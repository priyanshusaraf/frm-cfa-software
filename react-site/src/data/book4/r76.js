export default ({
  ...
  breakdown: [
    {
      ...
      points: [
        ...
        "Repo tenor is itself a choice, and lenders lean short. Overnight repo dominates because it hands the cash lender a fresh decision every morning: renew, or walk away. An OPEN repo splits the difference, lending for a day under a contract that simply renews each day until one side cancels, so the lender keeps the daily exit without re-papering the trade. Longer maturities out to several months exist, but demand thins as the term extends, which is the same preference for optionality read from the other end.",
  ...
  highYield: [
    ...
    { stars: 4, what: "Counterparty risk (minor, collateral-mitigated) vs. liquidity risk (persistent concern) in repos.", why: "A precise risk-prioritization fact." },
