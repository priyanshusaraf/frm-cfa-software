export default ({
  ...
  thinkLike: `<p>A treasury risk manager reading this material asks two questions of every report: (1) <em>What decision does this report exist to trigger?</em> A deposit tracker report showing the loans-to-deposit (LTD) ratio breaching its limit triggers a decision to either grow deposits or shrink the loan book. A funding concentration report showing one depositor above the ALCO's single-source limit triggers a decision to cap or actively reduce that relationship. A cash flow survival report showing a horizon under 30 days triggers activation of the contingency funding plan (R73). (2) <em>Is this number before or after mitigating actions?</em> This business-as-usual (BAU) vs. post-mitigation distinction runs through every report here. A bank that only has an 8-day survival horizon under BAU but 21 to 27 days after drawing on contingent funding and liquidating securities is in a materially different position than a bank whose post-mitigation number is still under 30 days, and a question can put exactly that on a graph with two lines, dashed for BAU and solid for after adjustments, then ask you to read off both horizons and compare each against the 30-day Basel III requirement.</p>
  <p>The FSA carve-outs (demand deposits treated as one-day tenor, derivatives excluded from the liquidity ratios except for pay-date coupons, undrawn commitments counted as an outflow) each stand alone well enough to be the one true statement among four. And the named reports are separable by content, so a stem can describe what a report contains and ask which of the seven-plus it is. Know that list cold, and know the cash flow survival report in particular: BAU against adjusted, and weekly against daily cadence.</p>`,
  ...
  highYield: [
    { stars: 4, what: "The cash flow survival report as the single most important liquidity stress report, tracking the 30-day Basel III horizon, shown as BAU vs. post-mitigation lines.", why: "The signature report of this material. It connects straight to R60's LCR, and the BAU-against-adjusted comparison is the shape a worked question takes." },
    { stars: 3, what: "FSA treatment rules: callable/demand deposits as one-day tenor, derivatives excluded (except pay-date coupons), undrawn commitments included.", why: "Three precise rules, each self-contained enough to be the one true statement among four." },
    ...
    { stars: 2, what: "Reporting cadence (daily/weekly/monthly/quarterly) and the full named-reports list, including the funding concentration report's single-source limit and the wholesale pricing/volume report's regulator-facing role.", why: "The reports are separable by content, so a stem can describe one and ask which it is." }
  ],
