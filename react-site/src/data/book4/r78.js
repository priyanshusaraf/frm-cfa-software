export default ({
  ...
  thinkLike: `<p>A bank treasurer or FX risk manager does not see a nonzero cross-currency basis as found money. It is a balance-sheet decision with a price tag attached.</p><p>Capturing the basis means borrowing on one side and lending on the other. Both legs sit on the bank's balance sheet, and both consume capital, funding and counterparty-credit-risk limits that are scarce and regulated (the leverage ratio, the liquidity coverage ratio, counterparty credit limits).</p><p>So the practitioner's question is never "is there a free lunch?" It is whether the basis, net of the capital charge and the funding cost of expanding the balance sheet to capture it, still clears the hurdle rate. Often it does not, and that is exactly why the basis survives.</p><p>At the macro level a central bank or treasury official watching the USD funding gap asks a parallel question. Not "do we have enough dollar assets?" but "what happens if we cannot roll over our dollar liabilities next week?" A funding gap only becomes a crisis at the rollover date, not before.</p>
  <p>Three kinds of question come out of this. The first is definitional and mechanical: separating FX swaps from cross-currency basis swaps on term, reconversion rate and interim payments, and separating the basis from the raw forward premium.</p>
  <p>The second asks why the arbitrage has not closed. These need BOTH halves of the story, the demand sources that opened the gap and the limits to arbitrage that keep it open. Half an answer scores nothing.</p>
  <p>The third turns on the asymmetry between domestic and foreign-currency lender-of-last-resort capacity. Phrased as "why is a USD funding gap worse than a domestic-currency one", the answer always traces back to the same fact: no central bank outside the Fed can create USD.</p>`,
