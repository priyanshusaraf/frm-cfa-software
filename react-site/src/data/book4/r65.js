export default ({
  ...
  eli5: `<p>Your car has three warning lights, and they are not interchangeable. The fuel gauge drops to a quarter tank: you have an hour to find any petrol station you like. The temperature needle creeps up over several minutes: pull off at the next exit. The brake warning screams: stop now.</p>
  <p>What separates them is not severity. It is how much runway each one gives you. The fuel gauge hands you time and choices. The brake light hands you neither, because by the time it fires, the graceful options are already gone.</p>
  <p>So you would not accept a dashboard with only the brake light, however serious that light is. And you would not accept only the fuel gauge, because it says nothing at all about the engine. You need instruments tuned to different speeds of failure, sitting side by side.</p>
  <p>A bank's early warning indicator system is that dashboard, pointed at its ability to pay its bills. Some indicators watch problems that build over weeks: loan losses climbing, the core deposit base thinning. Others watch problems that arrive in hours: overnight borrowing costs spiking, an intraday payment queue gridlocking.</p>
  <p>The green, amber and red stoplight on top is not a measure of how bad things are. It is a measure of how fast you have to move, which is exactly the question the driver is asking when they glance down.</p>`,
  thinkLike: `<p>A liquidity risk manager does not read an early warning indicator (EWI) framework as a list of ratios. It is a pipeline of five stages, and every stage has to work or the whole thing is decoration.</p><p>It starts with the <strong>measures</strong>. A metric earns its place only if it leads the event rather than lagging it, which already disqualifies most of the ratios a bank reports for other reasons.</p><p>Then the <strong>thresholds</strong>, where the green, amber and red boundaries get calibrated off historical volatility. The lookback is a real tradeoff: roughly a year, long enough to be statistically reliable and recent enough to still describe current conditions.</p><p>Then <strong>reporting</strong>, which is daily for most banks and intraday or hourly for trading-heavy ones.</p><p>Then <strong>integrated systems</strong>, so that lending, trading and treasury feed one dashboard instead of three disconnected spreadsheets that disagree with each other.</p><p>Then <strong>escalation</strong>. A named person or committee is obligated to act when a light turns amber or red. A threshold nobody owns changes nothing.</p><p>Two question shapes come out of this. One asks which characteristic makes an indicator useful, and the traps are plausible virtues that happen to be wrong here: accuracy, comprehensiveness, being backward-looking.</p><p>The other names a guidance document and asks which body issued it. OCC 2012 is the embedded-option one. BCBS 2008 is the general-deterioration one. BCBS 2012 is the intraday one. Fed SR 10-6 is the firm-tailored-triggers one. Learn the single distinguishing word for each (embedded options, general, intraday, firm-specific) rather than trying to recall paragraphs.</p>`,
  ...
  highYield: [
    ...
    { stars: 2, what: "Regulatory guidance sources and their specific focus (OCC embedded options, BCBS 2008 general vs. BCBS 2012 intraday, Fed SR 10-6 firm-specific).", why: "A specific list where a stem can name a topic and ask which guideline addresses it." },
