export default ({
  ...
  thinkLike: `<p>A bank's asset-liability management (ALM) desk, or its deposit-pricing committee, does not think about what is fair. It thinks about what the next dollar of funding costs, all-in, and whether that cost can be cleared on the asset side.</p>
  <p>Which means never taking a marginal-cost number at face value without two follow-up questions.</p>
  <p><em>Does the new rate apply only to new money, or to the whole existing balance too?</em> Posted rates on an account type like money market deposit accounts (MMDAs) typically reprice the whole balance. When they do, the effective marginal cost of the incremental funds is dramatically higher than the rate increase alone implies.</p>
  <p><em>Is there a cheaper alternative source for exactly this amount of money?</em> A bank facing a $10 million funding need should compare the marginal cost of raising it through its existing deposit product against issuing negotiable CDs or tapping wholesale funding, which are often cheaper precisely because they do not reprice the whole book.</p>
  <p>Three exam shapes come out of this. The first is classification: a bank waives a fee if the balance stays above some level, so which pricing method is that? (Conditional pricing.)</p>
  <p>The second is a marginal-cost <em>calculation</em>, where you must apply the new, higher rate to the ENTIRE base, old deposits plus new, rather than to the incremental amount alone. That is the single most common trap in the reading.</p>
  <p>The third is scenario questions on the social and regulatory material: deposit insurance limits, what Truth in Savings requires banks to disclose, why overdraft protection is controversial, what lifeline banking means. No calculation is involved, so these are checking whether you actually read that material.</p>`,
