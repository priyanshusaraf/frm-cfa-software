...
<p>The ILM never enters this calculation at all, not because PS Bank's loss history is unknown, but because Bucket 1 banks are, by design, exempt from having their loss history affect capital.</p>` }
  ...
  thinkLike: `<p>A risk manager building this number does not sit down and "model" operational risk the way a market risk desk models VaR. That flexibility is precisely what the Standardized Measurement Approach (SMA) was designed to remove.</p><p>The job is closer to careful bookkeeping under an audit-ready rulebook. Pull three years of income-statement data and compute the interest, lease and dividend component (ILDC), the services component (SC) and the financial component (FC) exactly as defined. Watch the absolute-value treatment on trading and banking-book P&L, where losses count just as much as gains. Determine the bucket, then run the marginal business indicator component (BIC) calculation the way a tax accountant works a bracket.</p><p>If the bank is large enough, there is a second half to the job. Pull ten years of internal loss data, curated against a strict inclusion and exclusion policy (no PP&E maintenance, no insurance premiums, no post-event upgrade costs), and use it to compute the loss component and the internal loss multiplier (ILM).</p><p>Three question shapes come out of this. The first is a pure BIC calculation, where the trap is applying the top bucket's rate to the whole business indicator instead of working it marginally.</p><p>The second is conceptual: what the ILM does, and when it is skipped, which is when a bank's losses run better or worse than average or when it has fewer than five years of data.</p><p>The third is a gross-loss inclusion or exclusion question, and the PP&E maintenance exclusion is the single most recycled distractor in the reading.</p><p>So the job reduces to two things. Get the mechanical bracket math right, and know precisely which loss items regulators count as real operational-risk losses rather than ordinary running costs.</p>`,
  ...
  quiz: [
    {
      ...
      answer: 1,
      why: "BIC is marginal: 12% on the first €1bn (€0.12bn) plus 15% on the remaining €12bn from €1bn to €13bn (€1.80bn) = €1.92bn. The tempting distractor €1.95bn comes from wrongly applying the flat Bucket 2 rate (15%) to the entire €13bn instead of only the portion above €1bn, exactly the marginal-vs-flat trap the bucket structure is built to catch."
    },
    {
