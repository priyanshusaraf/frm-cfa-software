export default ({
  ...
  why: `<p>Direct setup for R58's capital planning and the Basel capital math in R59-62. Understanding why stress test design is genuinely hard (scenarios must be extreme AND plausible simultaneously) prepares you for the practical complexity in R58's real supervisory regime. The concrete numbers here are also worth holding: the 19 SCAP banks, the $75B and $77B capital-raise figures, the Nevada versus North Dakota unemployment contrast, and the SCAP-to-CCAR disclosure timeline are all specific and checkable, unlike the softer conceptual material elsewhere in the book.</p>`,
  ...
  breakdown: [
    ...
    },
    {
      ...
      points: [
        "Mechanics: the starting balance sheet generates quarter 1's stressed income/loss; that produces the quarter-end balance sheet, which becomes the starting point for quarter 2, and so on: a sequential, quarter-by-quarter rollforward, not a single before-and-after comparison.",
  ...
  concepts: [
    {
      ...
      def: "SCAP (US, 2009): full bank-level disclosure, high transparency, and the first post-crisis macro-prudential test. CCAR (initial, 2011): only macro-scenario results published, not bank-level. CCAR (revised, 2012): back to near-SCAP level of bank-level detail.",
    ...
    },
    {
      ...
      def: "Scenarios must be extreme AND plausible simultaneously, which is genuinely hard because real risk is multi-factor (a scenario extreme in one dimension may be unrealistic when all dimensions are considered jointly).",
      intuition: "A supervisor's real job in scenario design is not picking one bad number. It is specifying the joint outcome of every relevant risk factor at once, such that the combination could actually occur in the real world.",
      example: "An increase in volatility can trigger credit markets freezing; high unemployment and falling equity prices tend to move together, so a scenario that shocks one without the other is internally inconsistent. Symmetrically, not everything can go bad at once: if some currencies depreciate, others must appreciate by arithmetic; if there's a 'flight to quality', the scenario must specify the safe-haven assets capital flees into, and even a scenario where U.S. Treasuries stop being the safe haven must identify what becomes the new one. The problem compounds for marked-to-market trading books, where a VaR system maps hundreds of thousands of positions to thousands of risk factors daily, making a coherent joint outcome across that many dimensions very hard to specify.",
    ...
    },
    {
      ...
      example: "Mechanically: the starting balance sheet generates quarter 1's stressed income/loss, which produces the quarter-end balance sheet, feeding into quarter 2, and so on, typically over a two-year horizon, with capital and liquidity ratios required to hold throughout. Concretely: July 2011 unemployment was 12.9% in Nevada versus 3.3% in North Dakota against a 9.1% national rate, and credit card loss rates (which are unusually unemployment-sensitive) tracked that huge regional spread rather than the national number. Product type matters too: during a recession auto defaults may rise while loss-given-default doesn't rise proportionally, because buyers needing a car shift toward used cars, propping up used-car collateral values. Industry matters as well: an airline loan collateralized by an airplane becomes very hard to sell in a depressed airline market, while healthcare loans are comparatively recession-resistant.",
      pitfall: "The same 'intermediate risk factor' mapping problem also appears in R46's ERM stress-testing taxonomy and R57's economic capital challenges, so treat it as a recurring translation problem rather than a one-off issue. Also: revenue modeling is much less mature than loss modeling, and noninterest income (fees, service charges) is especially hard to model despite being a growing share of U.S. bank income.",
      related: [{ r: 46, label: "R46: the same macro-to-micro mapping challenge previewed" }, { r: 58, label: "R58: where these mechanics become an actual supervisory process" }],
    ...
    },
    {
      ...
      related: [{ r: 58, label: "R58: the same horizon-plus-reserve logic in the real CCAR process" }],
  ...
  connections: {
    ...
    confused: [
      { what: "SCAP vs initial CCAR vs revised CCAR disclosure levels", how: "SCAP: full bank-level. Initial CCAR: only macro-scenario results, NOT bank-level. Revised CCAR: back to near-SCAP bank-level detail. Disclosure levels went full, then reduced, then restored, not a linear progression." },
  ...
  highYield: [
    ...
    { stars: 3, what: "The coherence problem: extreme AND plausible simultaneously, multi-factor risk.", why: "The conceptual core of why stress test design is genuinely hard, and a theme that recurs across the stress-testing readings." },
    ...
    { stars: 2, what: "Geographic/product/industry sensitivity examples (Nevada vs. North Dakota unemployment; used-car LGD; airline vs. healthcare collateral).", why: "These concrete illustrations get tested directly, as in 'which loan type is most sensitive to unemployment'." },
  ...
  recall: [
    { q: "Why did stress test disclosure levels move from SCAP's full transparency to initial CCAR's reduced disclosure, and then back to near-SCAP detail under revised CCAR?", a: "Because disclosure is a policy lever with costs on both sides, and supervisors were calibrating it round by round. Heavy bank-level disclosure is what restores market trust in a crisis, which is why SCAP published loss projections for all 19 banks and why the 2012 CCAR round went back to that granularity. But in calmer times the same disclosure invites window dressing (portfolio choices made to look good on this specific test) and crowds out private information production, since traders lean on the published numbers instead of researching banks themselves, which degrades the very market prices supervisors use as a signal. The 2011 pullback sits between those two pressures." },
  ...
  hooks: [
    ...
    { title: "Nevada isn't North Dakota", text: "A national unemployment number is an average hiding a 9.6-point regional spread (12.9% against 3.3% in July 2011). Treating every bank's credit card book as if it faced the national number is like giving every patient in a hospital the same prescription regardless of what is actually wrong with them." }
  ],

  summary: `<p><strong>Disclosure evolution</strong>: SCAP (2009, full bank-level) → CCAR initial (2011, macro-only) → CCAR revised (2012, near-SCAP detail). The path is non-monotonic. EBA Irish and Europe-wide (2011) went further still, disclosing bank-by-bank, asset-class, country, and maturity-bucket detail. <strong>Coherence problem</strong>: scenarios must be extreme AND plausible jointly across multiple factors, since a single-dimension extreme shock can be unrealistic when considered with all other variables (SCAP's 2009 market-risk scenario achieved coherence by reusing the actual 2007-2009 crisis experience rather than inventing a new hypothetical). <strong>Macro-to-micro translation</strong>: macro factors (unemployment, GDP, home prices) mapped to bank, product, and geography-specific outcomes. Nevada's 12.9% against North Dakota's 3.3% July 2011 unemployment shows why a national number misleads; used-car LGD dynamics and airline-versus-healthcare collateral show product and industry sensitivity. Revenue modeling remains far less mature than loss modeling. Balance sheet evolves quarter-by-quarter over a typical 2-year horizon, with capital/liquidity ratios required to hold throughout, and reserves must additionally be estimated for the year after the horizon ends (T-year test → T+1 years of reserve estimation). Heavy disclosure restores market trust in a crisis but carries window-dressing and crowded-out-private-information costs in normal times.</p>`,
  ...
  quiz: [
    {
      ...
      answer: 1,
      why: "The reading's core motivation is that static capital ratios, computed on today's balance sheet, don't reveal forward-looking resilience, and stress testing simulates an adverse scenario to answer that question instead. The 'ratios were computed correctly but set too low' answer is a tempting distractor because ratio levels are a real regulatory topic, but the point here is the tool's static nature, not its calibration; the 'no capital required' and 'no deposit insurance' answers are factually wrong (capital requirements and deposit insurance predate 2009)."
    },
    {
      ...
      answer: 1,
      why: "Coherence specifically means the joint outcome across multiple risk factors must be internally consistent and plausible, not just extreme on one dimension. The 'severe enough to test capital' answer describes severity alone, which is necessary but not sufficient for coherence, and a common trap since severity is easier to picture than joint plausibility. The 'identical scenario for every bank' answer describes the pre-2011 one-size-fits-all approach (a different issue), and the 'consistent disclosure format' answer describes disclosure, an unrelated topic in this reading."
    },
    {
      q: "In July 2011, national U.S. unemployment was about 9.1%. Which pairing of state-level unemployment rates illustrates why macro-to-micro translation matters for credit card losses?",
      options: [
      ...
      answer: 1,
      why: "The example is Nevada at 12.9% and North Dakota at 3.3%, against a 9.1% national rate: a roughly 9.6-point spread showing that credit card losses, which are unemployment-sensitive, vary far more by geography than a national average suggests. The other pairings are plausible-sounding but not the real figures."
    },
    {
      q: "A stress test has a stated horizon of two years (T=2). Under the balance-sheet modeling logic here, how many years of loan/lease loss reserves must the bank actually estimate?",
      options: [
      ...
      answer: 2,
      why: "At the end of the stress horizon the bank must estimate reserves needed to cover losses on loans and leases for the next year, so a T-year test requires T+1 years of reserve estimation, meaning a two-year test requires three years' worth. The 'two years, matching the stated horizon' answer tempts because it matches the headline horizon length, but it ignores the post-horizon reserve requirement."
    },
    {
      q: "Which of the following is a real judgment call a risk modeler must make when projecting a bank's balance sheet over the stress horizon?",
      options: [
    ...
    },
    {
      ...
      answer: 1,
      why: "SCAP had full bank-level disclosure across eight asset classes; the 2011 CCAR round pulled back to macro-scenario-only results (forcing the market to infer pass or fail indirectly, from dividend changes for instance); the 2012 CCAR round restored near-SCAP bank-level granularity. The 'increased steadily and monotonically' answer is the most common wrong intuition, assuming transparency only ever increases after a crisis, but the actual path is full, then reduced, then restored."
    }
  ],
