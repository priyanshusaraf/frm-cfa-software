export default ({
  ...
  teaches: `<p>This reading is the first of four building blocks in the operational risk management framework (ORMF): (1) <strong>identification</strong>, which is this reading, (2) measurement and assessment (R43), (3) mitigation (R44), and (4) reporting (R45). Once a risk has been found, a bank has to decide whether to accept it and price or capitalize for it, or reject it and mitigate it. None of that decision-making is possible until the risk has actually been surfaced. R42 teaches three things: (1) the <strong>top-down</strong> and <strong>bottom-up</strong> approaches to finding operational risks, and the specific tools that live under each; (2) the structured process for running <strong>scenario analysis</strong> and scenario workshops, which turn "what could go wrong" into a defined, assessable list of extreme events; and (3) the Basel operational risk taxonomy pushed one level deeper than R40's introduction, from Level 1 (7 broad categories) down to Level 2 (20 specific categories) and Level 3 (concrete examples), plus the newer ORX taxonomy that restructures it into 14 Level-1 categories.</p>`,

  why: `<p>Combining top-down (the strategic and emerging risks senior management sees) with bottom-up (the operational detail business units see) gives a more complete picture than either alone. A bank that only does one is structurally blind to an entire class of risk. What matters is less which tool is "top-down" versus "bottom-up" in isolation, and more what each tool is good at spotting, so you can answer questions like "which tool would the CRO recommend for X" (a question shape you'll see below). Scenario analysis matters because it is the bridge from qualitative risk identification into the quantitative assessment machinery of R43: a scenario workshop's output, a short list of extreme, plausible, financially severe events, becomes the raw material that gets modeled with fault trees, FAIR, and loss distributions in the next reading. And the ORX taxonomy's elevation of model risk and cyber risk to Level-1 status directly explains why R47-48 (cyber) and R53-54 (model risk) get entire dedicated readings later. Those risks earned their own spotlight specifically because they no longer fit neatly as sub-items buried inside a broader Basel Level-1 category.</p>`,

  intuition: `<p>Top-down identification is like a satellite view: it catches strategic and emerging risks a business unit embedded in daily operations might miss entirely. Bottom-up is like a ground-level walkthrough, catching the operational detail (a specific process gap, a specific control weakness) that senior management, working at altitude, simply can't see. Neither view alone is complete; the two are complements, not substitutes. Concretely: the top-down approach cascades down three levels, board and senior management, then divisions and business units, then individual business processes, using tools like brainstorming sessions, a "risk wheel," and exposure/vulnerability analysis. The bottom-up approach is not firmwide. It lives inside a specific business unit or process, typically run as part of a risk and control self-assessment (RCSA), and is done by the people actually doing the work: lower and middle management, not the board. Think of it this way. A bank expanding into a new automated financial-advisor product is a risk a satellite (senior management) would spot from altitude: this is a new strategic direction with IT and conduct risk attached. But the specific data-entry error pattern in the trade-confirmation process is something only the ground crew, the business unit doing the confirmations, would notice.</p>`,

  eli5: `<p>Imagine a hospital trying to prevent patient harm. The hospital board and chief medical officer periodically step back and ask big-picture questions: are we expanding into a risky new surgery type, is our IT system aging, is a pandemic coming? That's <strong>top-down</strong>, meaning strategic, infrequent, and done by people at the top who see the whole landscape but not the daily details. Meanwhile, each ward's nursing staff fills out a checklist every shift noting near-misses: a medication almost given at the wrong dose, a monitor that beeped late. That's <strong>bottom-up</strong>, meaning granular, frequent, and done by the people actually doing the work, catching things that would never be visible from the boardroom. Neither the board's big-picture scan nor the ward's daily checklist alone would catch everything; you need both. In banking, "the board" maps to senior management doing top-down risk identification (risk wheel, exposure/vulnerability analysis, emerging-risk scanning), and "the ward checklist" maps to business-unit-level bottom-up tools (RCSA, process mapping, loss and near-miss data analysis).</p>`,

  thinkLike: `<p>A risk manager treats identification as an inventory-building exercise, not a one-time event. The goal is to populate the bank's <strong>risk register</strong> (also called the risk inventory or risk universe) as completely as possible, because a risk that never gets identified can never be measured, mitigated, or reported. It just sits there as an unpriced tail exposure until it materializes as a loss. A practitioner asks, for every candidate risk-identification exercise, "which altitude am I working at, and what is this tool structurally blind to?" A risk wheel run in a top-down brainstorming session is good at surfacing causal chains (a weather event triggers a service outage which triggers reputational damage) but will never catch a specific process-level data-entry error. That needs process mapping or RCSA at the bottom-up level. The classic question shape is a scenario ("the CRO wants risk assessed at a more local, detailed level, so which tool is he LEAST likely to recommend?") that expects you to sort the tools into their top-down and bottom-up buckets rather than treat them as an undifferentiated list. On scenario analysis, think like a regulator worried about gameable results: the entire emphasis on neutrality, documented rationale, and structured generation-then-selection phases exists because regulators want scenario outputs that are reproducible and defensible across iterations, not a number a business unit backed into to minimize its own capital charge. On taxonomy, think like an analyst building a mapping table. Basel's 7 Level-1 categories are what regulators require for reporting; the ORX taxonomy's 14 categories are what many banks actually use internally, because specific risks (cyber, model risk, third-party failure) had grown too large and distinct to stay buried as sub-items. Recognizing WHY a risk gets promoted, namely that its scale and severity outgrew its old home, is more useful than memorizing the list itself.</p>`,
  ...
  breakdown: [
    {
      ...
      points: [
        ...
        "<strong>Analyzing emerging risks</strong>: split into predictable risks (cyber, regulatory, climate change, tracked via horizon scanning, including PESTLE analysis: political, economic, social, technological, legal, environmental) and unpredictable risks such as a global pandemic, where the only real defense is overall preparedness and resilience rather than specific forecasting."
      ]
    },
    {
    ...
    },
    {
      ...
      points: [
        ...
        "<strong>Selection</strong>: scenarios that produce the same effect via different causes get combined (damage to tangible assets from a weather event versus from civil unrest, say); scenarios with immaterial impact get dropped; scenarios with material financial impact are retained or added. A midsized bank might generate roughly 30 scenarios and keep about half after selection, with the count scaling up or down for larger or smaller banks.",
    ...
    },
    {
      ...
      points: [
        ...
        "<strong>Level 2</strong>: 20 more specific subcategories nested under the 7 Level-1 categories. EDPM and CPBP get the most Level-2 subcategories because they carry the highest frequency and severity of losses among financial firms.",
        "<strong>Level 3</strong>: concrete real-world examples nested under each Level-2 subcategory. Under EDPM, data entry errors from high transaction volume; under BDSF, a funding system crash that forces a bank to arrange $30 billion in emergency funding at above-market cost after the money markets have closed."
      ]
    },
    {
      title: "ORX taxonomy: what changed versus Basel",
      points: [
        ...
        "Motivation: practitioners found the same item classified inconsistently across banks. Cyber risk was sometimes filed under external fraud and sometimes under IT risk; third-party risk was sometimes stand-alone and sometimes under IT. The ORX taxonomy exists to create an 'international standard' that removes that ambiguity.",
        "The ORX taxonomy is not (yet) used for mandatory regulatory reporting. Basel Levels 1 and 2 remain the reporting standard; ORX is a widely used internal and analytical supplement."
      ]
    }
  ],
  ...
  concepts: [
    {
      ...
      def: "Top-down: analyzing exposures and vulnerabilities, the risk wheel, and analyzing emerging risks, run by the board, senior management, business-unit leaders, and department heads (legal and compliance, for instance), typically via brainstorming sessions held quarterly or semiannually, more often if the bank is in emerging or complex technology areas. Bottom-up: event and loss data analysis, risk and control self-assessment (RCSA), and process mapping, run at the business-unit or process level by lower and middle management, not the board.",
      intuition: "The single cleanest test to tell them apart: WHO is doing the identifying, not what tool is nominally involved. If it's senior management or the board looking firmwide, it's top-down. If it's the people actually running the process, it's bottom-up.",
      example: "A bank's decision to expand into automated financial advice (a strategic move) surfaces IT and conduct risk via top-down brainstorming. Meanwhile a specific spike in mismatched trade confirmations in the back office surfaces via bottom-up process mapping or RCSA, since senior management would likely never see this level of detail directly.",
      pitfall: "Combining both gives a more complete picture than either alone, so don't treat one as sufficient on its own. A CRO who wants risk assessed 'at a more local and detailed level' would recommend process mapping, near-miss analysis, or RCSA (bottom-up), and would NOT recommend a risk wheel (top-down, firmwide, brainstorming-based).",
    ...
    {
      name: "Risk wheel",
      def: "A top-down brainstorming tool that arranges common bank risks as interconnected 'cogs' in a circle (business objectives, labor market, reputation, technology, regulation, economy, natural events, data and information, business continuity) to visualize how one risk category causally triggers another.",
      example: "A weather-related disaster (natural events cog) causes a multi-day service outage (business continuity cog); if the outage persists, it damages the bank's reputation (reputation cog). Tracing this causal chain is proactive risk management: it gets at the root cause rather than reacting only to the symptom, which is the outage itself.",
    ...
    },
    {
      ...
      pitfall: "Don't dismiss near misses as 'nothing happened, so nothing to report'. The whole point is that something nearly did happen, and reporting and logging them is how a bank continually improves its control system without waiting for an actual loss to force the issue."
    },
    {
      name: "Scenario analysis & scenario workshops",
      def: "Scenarios are extreme, financially severe, plausible-but-unlikely events (natural disasters, global pandemics, major cyberattacks, major business disruptions) used to bridge risk identification into quantitative assessment. BCBS guidance defines this as 'low probability and high severity events, some of which could result in severe operational risk losses.' Neutrality is critical, since regulators want consistent results across iterations, and it is achieved through empirical evidence and documented, written rationale for each scenario's development.",
      example: "Scenario workshops run in two phases. Generation: facilitators, typically operational risk specialists rather than board members, run brainstorming with upper-management participants across business units, sometimes using 'silent voting' to avoid one strong personality dominating. Selection: combine scenarios that share an effect but differ in cause, drop immaterial scenarios, and add or retain scenarios with material financial impact. A midsized bank might generate around 30 scenarios and retain roughly half after selection.",
      pitfall: "External participants (outside risk specialists) are rarely used but are valuable specifically because they counter 'myopia', the tendency to overweight recent events and forget older ones, and because many of a bank's most significant losses (rogue trading, internal IT malfunctions) are actually internal rather than external, so an internally-biased panel can under-rate internal-cause scenarios.",
    ...
    },
    {
      ...
      def: "Three levels: Level 1 is the seven broad event categories (from R40: internal fraud, external fraud, EPWS, CPBP, DPA, BDSF, EDPM); Level 2 is 20 more specific categories nested under those seven; Level 3 is concrete examples nested under Level 2. Basel treats Levels 1 and 2 as mandatory regulatory reporting categories, so regulated banks must report operational risk using this structure.",
      example: "EDPM losses: small dollar amount per event, very high frequency, because financial firms process huge transaction volumes daily, so miscommunications and data-entry errors are common. Even a few days' delay finalizing a large FX transaction can be costly, since counterparties require compensation for use of funds. CPBP losses: infrequent, but very large, driven by client and counterparty disputes and regulatory fines for negligent business practices or breaches of advisory or fiduciary duty; litigation-heavy jurisdictions like the U.S. see especially high severity here. A concrete BDSF example: a bank's funding system crashes early in the day and doesn't come back until after money markets close (4:00pm ET), so the bank must arrange an extra $30 billion in emergency funding from counterparties at a much higher cost than its normal daily funding rate.",
  ...
  connections: {
    ...
    confused: [
      { what: "Top-down vs bottom-up identification", how: "Top-down catches strategic and emerging risks (satellite view); bottom-up catches operational, process-level detail (ground view). They are complements, not substitutes." },
      { what: "Basel taxonomy vs ORX taxonomy", how: "Basel has 7 Level-1 categories; ORX (2019) expands to 14 by promoting specific Level-2 risks (cyber, model risk, third-party failure, and others) up to their own Level-1 status." },
      { what: "Near misses vs. actual losses", how: "A near miss has zero dollar cost by definition. It's a warning signal caught by luck, not by a working control. Treat it as data about control weakness, not as something to ignore because 'nothing happened.'" }
    ]
  },
  ...
  recall: [
    { q: "Why might a bank's bottom-up risk identification process miss an emerging risk that top-down analysis would catch?", a: "Bottom-up identification (RCSA, process mapping, loss data analysis) is grounded in existing business unit operations and historical events, so it's poorly suited to spotting genuinely NEW, strategic, or emerging risks that haven't yet manifested in unit-level processes or loss data. Top-down analysis, done at a more strategic, enterprise level, is specifically designed to scan for these emerging and cross-cutting risks." },
    { q: "Why did the ORX taxonomy promote cyber risk and model risk to Level-1 status rather than leaving them as Level-2 categories under Basel's original scheme?", a: "These risks had grown significant and distinctive enough that treating them as sub-items buried under a broader Level-1 category (business disruption and system failures for cyber, or a modeling sub-item elsewhere) understated their importance and made them harder to track and report on their own. Promoting them to Level-1 gives them the visibility and dedicated attention their growing significance warrants, which is also why they get entire dedicated readings later in the book." },
    ...
    { q: "Why does regulatory guidance emphasize 'neutrality' in scenario analysis, and how is it achieved?", a: "Regulators require scenario analysis results to remain consistent across repeated iterations, so a process vulnerable to behavioral bias or inconsistent methodology would produce unreliable capital inputs that a business unit could steer. Neutrality is achieved through empirical evidence, documented explanations for how and why each scenario was developed, and detailed write-ups supporting the assumptions and methodology used." }
  ],
  ...
  hooks: [
    ...
    { title: "The $30 billion late-night funding scramble", text: "A funding system crashes before markets close; by the time it's back, money markets are shut. The bank has to scrounge $30 billion from counterparties at a premium rate: a single BDSF event, purely from timing, with no fraud or negligence involved." }
  ],

  summary: `<p><strong>Top-down</strong> (exposures/vulnerabilities, risk wheel, emerging risks: the strategic view, run by the board and senior management) versus <strong>bottom-up</strong> (event/loss data, RCSA, process mapping: the operational view, run by business units); combine both. Top-down tools: exposure/vulnerability analysis, risk wheel (causal 'cogs'), emerging-risk scanning (predictable via horizon scanning and PESTLE, unpredictable via preparedness). Bottom-up tools: event/loss data analysis (internal, external via ORX, and near misses, which are zero-cost early warnings), RCSA (inherent vs. residual risk), process mapping (tying controls to risks). <strong>Scenario analysis</strong>: extreme, plausible-but-unlikely events; neutrality via empirical evidence plus documented rationale; workshops run generation (brainstorm, silent voting) → selection (combine/drop/add) phases; roughly 30 scenarios generated and about half retained for a midsized bank. <strong>Basel taxonomy</strong>: Level 1 (7 broad categories: internal fraud, external fraud, EPWS, CPBP, DPA, BDSF, EDPM) → Level 2 (20 specific) → Level 3 (examples); EDPM small and frequent, CPBP infrequent and large, illustrated by a $30B emergency-funding BDSF example. <strong>ORX taxonomy</strong> (2019): expands to 14 Level-1 categories, promoting third-party failure, statutory reporting/tax, business continuity, data management, information security/cyber, and model risk, which directly explains why these topics earn dedicated readings later.</p>`,
  ...
  quiz: [
    {
      ...
      answer: 2,
      why: "Vulnerabilities are internal weak spots: obsolete processes, control weaknesses, or systems overdue for updating. They are not the things the bank has significant stake in, which are exposures like key suppliers, revenue drivers, or its regulator. Stand-alone, obsolete IT systems fit the definition of a vulnerability; the other three are examples of exposures."
    },
    {
      ...
      answer: 3,
      why: "The risk wheel is a top-down, firmwide brainstorming tool for tracing causal chains between broad risk categories, so it is not designed for local, granular detail. Process mapping, RCSA, and near-miss analysis are all bottom-up tools that operate at the business-unit or process level, which is exactly what the CRO wants."
    },
    {
      ...
      answer: 3,
      why: "Documents are often deliberately withheld from participants until after the generation phase, specifically to avoid biasing the initial brainstorming. Facilitators are typically operational risk management specialists rather than board members; participants are typically upper management rather than a full range of seniority; and assumptions rely on empirical evidence plus documented rationale, not exclusively on real-life data."
    },
    {
      ...
      answer: 1,
      why: "ORX promotes specific risks that were previously buried as Level-2 sub-items under Basel's broader Level-1 categories: third-party failure, statutory reporting and tax, business continuity, data management, information security and cyber, and model risk. It does not invent new risk types from scratch, does not split categories evenly, and is explicitly NOT (yet) used for mandatory regulatory reporting, which still relies on Basel."
    },
    {
      ...
      answer: 1,
      why: "EDPM losses (data entry errors, transaction miscommunications) occur at high frequency but each event is typically small in dollar terms, driven by financial firms' huge daily transaction volumes. CPBP losses (client and counterparty disputes, regulatory fines for negligent practices) are far less frequent but can be very large, especially in litigation-heavy jurisdictions like the U.S. Reversing the two, so that EDPM is infrequent and large, is the trap."
    },
    {
  ...
  sources: [
    ...
    { title: "BCBS: Revisions to the principles for the sound management of operational risk", url: "https://www.bis.org/bcbs/publ/d515.htm", note: "The supervisory expectations behind risk identification, scenario analysis, and the risk register discussed in this reading." }
  ],
