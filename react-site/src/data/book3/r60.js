export default ({
  ...
  concepts: [
    ...
    },
    {
      ...
      example: "Countercyclical buffer (CCyB): 0-2.5% of RWA, at national discretion, weighted across jurisdictions for international banks, dampens credit overheating. The reason it is set that way has a name, the cost-of-capital rationale: raising capital is easier and cheaper in good times than in bad, so financial stability comes cheaper if you build the buffer up when conditions are good and release it when they are not. G-SIB buffer: additional 1%-3.5% across five tiers (1%, 1.5%, 2%, 2.5%, 3.5%) for globally systemically important banks. The G-SIB list was first published in 2011 and had 29 institutions on it by 2018; no bank has ever landed in the top 3.5% tier, and as of 2018 only HSBC and JPMorgan Chase sat in the 2.5% tier. Since 2017, G-SIBs also face a leverage-ratio buffer equal to half their risk-based G-SIB buffer (on top of the ordinary 3% leverage ratio, and not counting the CCB).",
    ...
    },
    {
      ...
      def: "The Dodd-Frank Wall Street Reform and Consumer Protection Act, signed into US law in July 2010, is the umbrella statute for the whole post-crisis wave in the United States. Its provisions answer four separate failures the crisis exposed: nobody was watching the system as a whole (the FSOC and SIFI designation), firms could fail messily (living wills and extra capital for the systemically important), pay rewarded gains without pricing the losses those bets could cause (compensation reform), and whole markets ran opaque or unaccountable (OTC derivatives pushed to central clearing and swap execution facilities, rating-agency oversight, consumer protection through the CFPB, ability-to-repay rules, 5% securitization retention, and the Volcker Rule's ban on proprietary trading).",
  ...
  eli5: `<p>Imagine a household budget rule invented after a family nearly got evicted, not because they were broke on paper (their house and savings were worth plenty), but because all their cash was tied up and a sudden bill came due before their next paycheck. So the family adopts new rules: always keep enough cash in the checking account to cover 30 days of bills even if the paycheck is late (that's the LCR). Don't fund the mortgage with a payday loan that has to be renewed every week (that's the NSFR, matching how long your money is committed to how long your obligations last), keep a rainy-day fund on top of normal savings that you only touch in an emergency (the capital conservation buffer), and set up an "automatic pay cut" agreement with a rich uncle that kicks in without asking permission the moment the family's finances look shaky, converting his loan into a gift so no one else has to bail them out (a CoCo bond converting to equity). None of these rules make the family richer day to day. They exist purely so a temporary cash crunch never turns into a full collapse, which maps exactly to what Basel 2.5, Basel III's buffers, LCR/NSFR, and CoCos do for a bank: they don't add to net worth. They make sure a short-term cash crunch (illiquidity) can never turn into an actual failure the way it did for banks like Northern Rock and Lehman Brothers in 2007 to 09.</p>`,

  thinkLike: `<p>A bank treasurer is not primarily asking "are we solvent?" Basel II already forces that question through risk-weighted capital.</p><p>The treasurer is asking a second, separate question, one the 2007 to 09 crisis proved was just as dangerous to ignore: if funding markets freeze for 30 days, or for a full year, do we hold enough of the RIGHT kind of assets and funding to survive without a fire sale or a government rescue?</p><p>That is why this reading pairs two unrelated fixes. On the capital side sit Basel 2.5's stressed VaR, incremental risk charge and comprehensive risk charge, plus Basel III's tighter tiers and buffers. On the liquidity side sit the leverage ratio, the LCR and the NSFR.</p><p>Solvency and liquidity are different failure modes. They need different metrics, and a bank can die from either one independently. Lehman was largely a liquidity and confidence failure, arguably still technically solvent on paper for a time. Other institutions failed the other way around, undercapitalized on a risk-adjusted basis.</p><p>Three question shapes recur. The first is ratio arithmetic (LCR, NSFR, the market risk capital charge) where the trap sits in a haircut or a run-off rate: mortgages excluded from high-quality liquid assets, corporate debt admitted at 50%, wholesale funding running off faster than retail.</p><p>The second turns on who holds the discretion. Keep the MANDATORY capital conservation buffer separate from the DISCRETIONARY countercyclical buffer, because a stem that quietly swaps those two adjectives is easy to read straight past.</p><p>The third asks what changed and why: why stressed VaR is calibrated to each bank's OWN worst year rather than a fixed 2008 assumption, or why CoCos convert automatically rather than at bondholder discretion.</p><p>Expect the numbers themselves to be handed to you. No two sources agree on every haircut and run-off factor, so what you need cold is which direction each factor points, and the reason it points that way.</p>`,
  ...
  breakdown: [
    ...
    {
      title: "Potential CoCo conversion triggers",
      points: [
    ...
    {
      title: "Key Dodd-Frank reforms after the 2007 to 09 crisis",
      points: [
        ...
        "Volcker Rule: bars deposit-taking banks from proprietary trading, meaning trading their own capital for their own profit rather than on a customer's behalf, and limits their hedge fund and private equity stakes. The practical difficulty is that speculative proprietary trading can look very like legitimate market-making for customers."
      ]
    }
  ],
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        "The CCB is mandatory and always required in normal times; the CCyB is discretionary, set 0 to 2.5% by national regulators",
      ...
      answer: 1,
      why: "The CCB is a mandatory 2.5% of RWA built up in normal times and drawn down (with dividend restrictions) in stress. The CCyB is left entirely to national regulator discretion, ranging 0 to 2.5%. This exact conflation (treating the CCB as discretionary like the CCyB) is explicitly flagged in the source as a common exam trap."
    },
    {
  ...
  pdf: { book: 3, query: "Following the 2007 to 2009 financial crisis, the Basel Committee on Banking Supervision implemented reforms" }
});
