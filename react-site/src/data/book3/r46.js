export default ({
  ...
  eli5: `<p>Picture a family shop. The clerk behind the counter makes the day-to-day calls: this sale, this discount, this refund. She owns those decisions and their consequences.</p>
  <p>The store manager does something different. He does not make the sales; he sets the rules the clerk sells under, and he watches the pattern of sales for signs of trouble the clerk cannot see from where she stands.</p>
  <p>Once a year a bookkeeper comes in. The important detail is who hired her: the family owners, not the manager. Her job is not to check the sales. It is to check whether the manager's rule-setting and watching actually worked. A manager who audits himself is not a third check, he is the second one twice.</p>
  <p>If the family also brings in an outside accounting firm, that is a further layer sitting outside all three, which is why it is described as informal rather than as a fourth line proper.</p>
  <p>Now a separate question. Before opening a riskier new branch, the family asks what could realistically go wrong and whether they would survive it. They can ask it three ways. Nudge one number and see what breaks: what if rent doubles? Imagine a whole bad year at once: a recession hits the neighbourhood. Or start at the end, "we went bankrupt", and work backwards to find what chain of events would have had to happen.</p>
  <p>That third one is worth noticing, because it is the only one that does not require you to have guessed the disaster in advance. In finance terms: clerk, manager and bookkeeper are the three lines of defense, the outside firm is the informal fourth, and the three ways of imagining disaster are parameter, macroeconomic and reverse stress testing.</p>`,
  thinkLike: `<p>A risk manager reading this material treats ERM not as a compliance checklist but as an answer to a specific design question: "if a loss happens somewhere in this organization, how many independent sets of eyes had a chance to catch it before it became a crisis, and how far up the chain does accountability for missing it travel?" The three-lines structure is the organization's answer. A practitioner's instinct when handed a new committee, function, or hired firm is immediately to ask "which line is this, and does it report to management or to the board?", because that answer determines how much weight to place on its findings. That instinct is exactly what gets tested: a question describes a specific actor (an internal audit department, an externally hired audit firm, a risk committee, a compliance function) and asks you to classify it, usually with two superficially similar actors ("audit" appearing in both names) sitting in different categories.</p>
  <p>On the capital side, think like a manager deciding how much of the firm's balance sheet to lock away as a buffer against bad outcomes: regulatory capital (Pillar 1) is the floor a regulator imposes regardless of your specific business; economic capital is what you'd choose to hold even without a regulator looking over your shoulder, because it reflects your actual risk profile and target credit rating. The aggregation insight works the same way: a risk manager should push back against naively summing standalone capital requirements across risk types, because doing so ignores diversification, and should specifically remember that operational risk, being largely uncorrelated with market and credit risk, is a genuine diversification benefit rather than "yet another risk to add on top."</p>
  <p>On stress testing, the practitioner's frame is: match the testing approach to what you actually know. If you have a working model and just want to know its breaking point, shock a parameter. If you want to know whether the whole institution survives a realistic bad macro environment, run a regulator-issued scenario (like CCAR). If you're worried about a risk you can't even properly quantify, the "unknown unknown," start from the failure and reason backward, because forward-modeling an unmeasurable risk is a category error. That matching logic is what questions here turn on: given a described situation ("loss of a major client", say, or "shock GDP by -3%"), name which of the three approaches fits.</p>`,
  ...
  breakdown: [
    {
      ...
      points: [
        ...
        "Risk capital and stress testing (financial firms only): the fourth, industry-specific pillar: regulatory capital requirements and stress testing that financial firms must meet but non-financial firms generally do not."
      ]
    },
    {
