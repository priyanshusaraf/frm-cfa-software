...
<p>The December 2017 reforms (part of the same Basel III reform wave you saw take shape in R60) respond to this with one consistent philosophy applied across every affected risk type: RESTRICT internal models (take away or narrow the cases where banks are allowed to substitute their own model for the standard rule) and EXPAND/enhance the standardized approaches (SAs) so they are more risk-sensitive and don't need to be avoided in the first place. The <strong>output floor</strong> is the enforcement mechanism that makes this restriction bite even where internal models are still permitted: no matter how favorable a bank's internal model result looks, its final risk-weighted assets can never fall below 72.5% of what the standardized approach would require for the same exposures. Think of the output floor as a backstop bolted underneath every internal model a bank is still allowed to use, the model can still make the number more precise, but it can no longer make the number arbitrarily small.</p>`,
  ...
  thinkLike: `<p>A bank capital-management team reading this reform has stopped asking what the cleverest way to model its risk is. The question now is what the standardized-approach RWA will be, because that number sets a floor under everything the bank does.</p>
<p>Before 2017, an internal model that shaved RWA down aggressively was a competitive advantage: a lower capital charge and a higher return on equity. After the floor, a bank whose internal model result sits below 72.5% of its standardized RWA gets no benefit from that gap at all. The floor simply overrides it.</p>
<p>So capital planning shifts in two ways. You need your standardized-approach number cold, because it now caps your best-case outcome.</p>
<p>And for the risk types where internal models were removed altogether (A-IRB for large corporates and financial institutions, IRB for CVA), there is no modeling decision left to make. You are on F-IRB or the standardized approach by rule.</p>
<p>On the exam this is almost entirely a know-the-shape-of-the-reform question set rather than a fine-print one. Which risk types were restricted: credit, CVA and operational risk, NOT market risk and NOT the liquidity ratios.</p>
<p>Which specific sub-population lost A-IRB access: large and mid corporates and financial institutions, not all credit exposures.</p>
<p>And the exact mechanics and number of the output floor: 72.5%, computed only from standardized-approach building blocks, with IRB explicitly barred from entering the floor calculation for any risk type.</p>
<p>The most common trap is an answer choice that overgeneralizes a scoped restriction into a blanket one. Expect it, and check whether the restriction the question describes matches the specific carve-out the reform actually makes.</p>`,
...
<p>The standardized-approach number used to build the floor is itself assembled risk type by risk type: the plain SA for credit risk, SA-CCR for derivatives counterparty exposure (with the SA for credit risk then applied to the resulting counterparty exposure), SA-CVA or BA-CVA (or, alternatively, 100% of the bank's counterparty credit risk capital requirement) for CVA risk, SEC-ERBA, SEC-SA, or a flat 1,250% risk weight for securitization exposures, and the plain SA for both market risk and operational risk. No IRB-approach output is permitted to feed into any part of this floor calculation, for any risk type, that is true even for CVA risk, where the IRB option was removed from the framework entirely rather than merely excluded from the floor.</p>` }
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "The 2017 finalization tightened five areas at once, and the pattern is the same in each: less room for a bank's own model to set the number. Standardized credit risk got more granular and leans less on external ratings; IRB credit risk lost the advanced approach for large and mid corporates and financial institutions, with input floors underneath every parameter; CVA risk lost its IRB approach entirely; operational risk collapsed all prior approaches into one standardized measurement approach; and the leverage ratio gained a G-SIB buffer plus a refined exposure measure.",
