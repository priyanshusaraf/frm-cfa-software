export default ({
  ...
  why: `<p>This is the reading where op risk transitions from "here is a list of bad things that can happen" (R41, R42) to "here is how you attach numbers to those bad things." The loss distribution approach specifically is the direct ancestor of the standardized measurement approach (SMA) formula that closes Book 3 in R62: the SMA's business indicator and loss component are essentially a simplified, non-model-based replacement for what LDA tried to do by fitting frequency and severity distributions bank by bank. Even though the SMA now technically eliminates the <em>requirement</em> to model operational risk for regulatory capital, banks still use LDA-style thinking internally: for Pillar 2's ICAAP (internal capital adequacy assessment process), for stress testing, and for scenario analysis. So this reading matters both for its own mechanics (fault tree calculations are a classic numeric question type) and as the conceptual bridge to R62.</p>`,
  ...
  <p>Fault tree analysis multiplies INDEPENDENT control-failure probabilities together (an "AND condition": every listed control has to fail for the bad outcome to occur). But real controls are rarely fully independent. They're often designed together by the same team, so a flaw in one design philosophy shows up in several "independent" layers at once, and naive multiplication can dramatically UNDERSTATE the true joint failure probability. The Swiss cheese model captures this better as an intuition-builder (it is not itself a calculation method): imagine each control as a slice of Swiss cheese with holes in it that open, close, and drift around over time. A bad outcome slips through only when the holes in several slices happen to line up at the same moment. The defense is keeping the holes' locations uncorrelated across layers, which means designing each control independently enough that a single design flaw can't create holes in the same place in every layer at once.</p>`,

  visual: `<div class="widget" data-widget="bowtie" data-bowtie='{"event":"IT outage","causes":["Failed software deployment"],"preventive":["Change-management sign-off"],"impacts":["Delayed customer transactions"],"corrective":["Failover system","Customer-communication plan"]}'></div><div class="widget" data-widget="lossdist"></div>`,
  ...
  formulas: [
    {
      ...
      note: "Phishing 99% × firewall fails 5% × employee fails 10% × detection fails 3% × exit 1% = 0.0001485%. Assumes independence, which is often unrealistic.",
      derivation: `<p>A nonmalicious data-leakage scenario with five sequential conditions, each treated as independent:</p>
      <ul>
      ...
      \\[ 0.99 \\times 0.05 \\times 0.10 \\times 0.03 \\times 0.01 = 0.000001485 = 0.0001485\\% \\]
      <p>Now the part that matters more than the arithmetic. A per-employee probability is not the bank's exposure. Take the simpler generic case of four independent controls each failing with probability 5%: the joint failure probability is \\( 0.05^4 = 0.00000625 = 0.000625\\% \\). Apply that to an information-leakage scenario at a bank with 50,000 employees, assuming a consistent failure rate across them, and the bank-wide likelihood becomes at least \\( 0.000625\\% \\times 50{,}000 = 31.25\\% \\). A number that looks like a rounding error per person is close to a coin flip for the institution, so always check the exposure base (headcount, transactions, accounts) before calling a small probability immaterial.</p>`
    },
    {
      name: "LDA aggregate loss",
      math: "\\text{Aggregate loss} = \\text{Frequency} \\otimes \\text{Severity}",
      plain: "The total operational loss over a period is built by combining how MANY loss events occurred (frequency) with how LARGE each one was (severity), modeled as two separate distributions and then mixed together (convoluted), rather than as one single distribution fit to total losses directly.",
      ...
      <ol>
        ...
        <li>Repeat steps 1 to 3 millions of times to build up the full simulated aggregate loss distribution.</li>
      </ol>
  ...
  concepts: [
    {
      ...
      def: "BCBS minimums: at least 10 years of data, a minimum collection threshold of €20,000, internal losses classified to Basel event types, and occurrence and recovery dates reported. Four relevant dates per event: occurrence → discovery → reporting → accounting.",
      ...
      example: "A bank employee manipulates a suspense (transitory) account for years before anyone notices. The gap between occurrence and discovery here can stretch to years, which is exactly why the four-date framework exists: it separately measures how fast a problem is FOUND (occurrence to discovery, a visibility measure), how fast it's ESCALATED once found (discovery to reporting, a timeliness measure), and how long it takes to close out financially (reporting to accounting or settlement, which can itself run three to five years for incidents with legal or regulatory tails).",
      pitfall: "Boundary events occur in one risk class but are caused by another, for example a credit loss caused by human or operational error. Boundary events tied to CREDIT risk are treated as credit risk (already in RWA, so not double-counted in op risk); boundary events tied to MARKET risk stay in operational losses.",
    ...
    },
    {
      ...
      def: "RCSA (risk and control self-assessment) examines inherent risk (no controls) and residual risk (after controls), primarily qualitative and judgment-based, typically annual, or quarterly for very significant risks. A KRI measures exposure level (transactions processed per employee, say, or an increase in trader transaction limits). A KPI measures how effectively the bank operates (customer complaint count, average IT system downtime). A KCI measures how effectively controls operate (number of overdue business continuity plan reviews, error rates surviving two rounds of independent review).",
      ...
      pitfall: "Backtesting RCSAs against actual incidents tends to reveal UNDERESTIMATED likelihood and OVERESTIMATED severity, a specific and testable directional bias. Heatmaps (likelihood × impact → green/yellow/amber/red) are QUALITATIVE labels, not numbers to be multiplied together. It is mathematically wrong to compare a 'remote × extreme' risk (ratings 1×3=3) to a 'possible × low' risk (ratings 3×1=3) as if the equal product made them equivalent, because the ratings are ordinal labels, not cardinal numbers.",
    ...
    },
    {
      ...
      example: "Phishing received 99% × firewall fails 5% × employee fails to catch it 10% × detection fails 3% × information exits 1% = 0.0001485%. The simpler generic version makes the exposure point cleanly: four independent controls each failing 5% of the time give 0.05⁴ = 0.000625%, and at a bank with 50,000 employees on a consistent failure-rate assumption that becomes 0.000625% × 50,000 = 31.25% bank-wide. Exposure (headcount, transaction volume) matters as much as the raw per-event probability.",
      pitfall: "In reality, controls are RARELY fully independent, since they're often designed together by the same team. Conditional (Bayesian) probabilities give a more realistic estimate, because failure in one control makes failure in a related control more likely rather than equally likely. Scaling matters too: a tiny per-event probability times a large number of employees or transactions can still produce a material AGGREGATE likelihood.",
      related: ["FAIR model & the Swiss cheese model"]
    },
    {
      name: "Root-cause analysis, the 5-whys, and the bow tie diagram",
      def: "Root-cause analysis is performed by the first line of defense on significant incidents and near misses, with the second line reviewing and challenging it, looking for the deeper causes behind a loss rather than stopping at the symptom. The 5-whys technique operationalizes this: ask 'why did this happen?' at least five times in sequence, each answer prompting the next 'why', until you reach an underlying systemic cause rather than a surface-level trigger. The bow tie diagram visualizes the result: the risk event sits in the center, with CAUSES and PREVENTIVE controls laid out on the left side (before the event), and IMPACTS plus DETECTIVE/CORRECTIVE controls laid out on the right side (after the event).",
      ...
      intuition: "The bow tie shape literally mirrors the logic: left side is 'what could make this happen, and what stops it before it does', right side is 'once it happens, how bad does it get, and what limits the damage'. That makes it a natural tool for estimating both expected frequency (left side, driven by preventive-control strength) and expected severity (right side, driven by detective and corrective-control strength) in one diagram, and for spotting additional KRIs worth monitoring.",
      related: []
    },
    {
      ...
      example: "Swiss cheese model: each control layer has holes, and bad outcomes happen only when holes in multiple layers momentarily align. The defense is to keep control failure rates uncorrelated (design each layer independently, ideally by different teams) and maintain compensating controls that can catch what another layer misses, with periodic reviews to confirm the layers stay independent and reliable over time.",
      related: []
    },
    {
      name: "Loss distribution approach (LDA)",
      def: "Splits losses into frequency (discrete, usually Poisson, whose one parameter \\(\\lambda\\) equals both the mean and the variance) and severity (continuous, fat-tailed, usually lognormal), modeled independently then convoluted, often via Monte Carlo simulation, into an aggregate loss distribution.",
    ...
    },
    {
      ...
      def: "Scaling (size and inflation adjustments so external or old losses are comparable to the bank's current size and prices), cut-off mix (blend in external data above a threshold where internal data thins out, since a bank's own history of very large losses is usually sparse), filtering (explicit inclusion and exclusion rules to prevent manipulation of which losses enter the model).",
    ...
    },
    {
      ...
      example: "Business impact analysis (BIA) underpins step 2: a BIA report examines the operational and financial fallout from a disruption (lost or delayed sales, extra costs and penalties, lost customers) and recommends that recovery be prioritized for the processes with the biggest operational and financial stakes. Single points of failure (SPOFs), such as a key cybersecurity employee whose specialist knowledge is undocumented and hard to replace, or a supplier of IT, data, or internet services with no substitute, are treated as resilience KRIs. Where they can't be eliminated through backups and redundancy, they must instead be built explicitly into the impact-tolerance thresholds and stress tests.",
      pitfall: "Resilience is NOT the same as business continuity management (BCM). BCM covers every business process's continuity and recovery broadly; resilience is narrower, applying specifically to 'important business services' under non-extreme conditions and impact tolerances. 'Intolerable impacts' that breach tolerances fall OUTSIDE resilience's scope by definition: resilience only concerns itself with IBSs staying within tolerance, not with catastrophic breaches beyond it.",
  ...
  connections: {
    ...
    to: [
      { r: 62, why: "The LDA's frequency/severity split is the conceptual ancestor of the SMA's business indicator plus loss multiplier approach." },
    ...
    confused: [
      ...
      { what: "Fault tree analysis vs Swiss cheese model", how: "FTA assumes independence and multiplies probabilities, which often understates true risk; Swiss cheese explicitly models how correlated, aligned holes across layers create the failure. It is a more realistic mental model of the same joint-failure problem, but not itself a calculation method." },
      { what: "Resilience vs BCM", how: "BCM is broad (all processes); resilience is narrow (specifically 'important business services' within tolerance), and intolerable impacts fall outside resilience's defined scope." },
  ...
  misconceptions: [
    { wrong: "\"Fault tree analysis, by multiplying independent probabilities, gives an accurate estimate of joint control failure.\"", right: "Real controls are RARELY fully independent, since they are often designed together, so naive multiplication can significantly understate true joint failure probability. Conditional (Bayesian) probabilities are more realistic." },
    ...
    { wrong: "\"Extreme value theory works as well for operational risk as it does for market risk.\"", right: "EVT requires one dominant underlying cause generating losses, and op risk's heterogeneous loss drivers make it a poor fit, limiting its practical usefulness compared with market risk (R3)." },
    { wrong: "\"Operational resilience and business continuity management (BCM) are the same thing.\"", right: "BCM is broader (all business processes); resilience is narrower, focused specifically on 'important business services' and impact tolerances, and intolerable impacts fall outside resilience's defined scope." },
    { wrong: "\"A heatmap's likelihood and impact scores should be multiplied together for a precise risk score.\"", right: "Heatmap categories (remote/unlikely/possible/likely/highly likely × low/medium/high/very high/extreme) are QUALITATIVE labels combined into color zones (green/yellow/amber/red), not numbers meant for multiplication. Two risks with the same numeric 'product', say a remote/extreme risk and a possible/low risk, are not equivalent." },
  ...
  recall: [
    { q: "A bank's fault tree analysis multiplies five independent control-failure probabilities to estimate a 0.0001485% chance of a successful phishing attack. Why might this significantly understate the true risk?", a: "The analysis assumes all five controls (firewall, employee vigilance, detection systems, and so on) fail independently. In reality, controls are often designed together and share common vulnerabilities or blind spots, for instance when the same IT team designs both the firewall and the detection system. True joint failure probability, accounting for that correlation, is likely higher than the naive multiplication suggests." },
    { q: "Under the loss distribution approach, why are frequency and severity modeled separately before being combined?", a: "Frequency (how often losses occur) and severity (how large each loss is) are driven by different underlying processes and have different statistical shapes. Frequency is discrete and typically well-modeled by a single-parameter Poisson distribution, while severity is continuous and fat-tailed, typically lognormal. Modeling them separately, then convoluting them via Monte Carlo, captures the true aggregate loss distribution more accurately than trying to fit one distribution to the combined data." },
    { q: "A rogue employee's fraud causes a credit loss when a mismarked position leads to an uncollectible receivable. Is this loss captured in operational risk capital, credit risk capital, both, or neither?", a: "This is a boundary event tied to CREDIT risk, so it's treated as credit risk (already captured in credit RWA) and is NOT double-counted in operational risk capital, even though the root cause (mismarking) is operational in nature." },
    { q: "Why is extreme value theory (EVT) less useful for operational risk than for market risk?", a: "EVT's theoretical justification works best when there's ONE dominant underlying cause generating extreme losses, such as market crashes driven by a shared volatility regime. Operational risk losses come from genuinely heterogeneous causes (fraud, system failure, natural disaster, human error) with no single unifying generative process, which undermines EVT's core assumption and limits its practical fit." },
    { q: "What is the bow tie diagram used for, and how does its left side differ from its right side?", a: "It visualizes root-cause analysis around a single risk event placed at the center. The left side lays out the CAUSES of the event and the PREVENTIVE controls meant to stop it before it happens; the right side lays out the resulting IMPACTS and the DETECTIVE/CORRECTIVE controls meant to catch and limit damage after it happens, which lets an analyst estimate both frequency (left side) and severity (right side) in one picture." }
  ],

  hooks: [
    { title: "Two questions, one number", text: "LDA asks 'how often' (frequency) and 'how bad' (severity) as two SEPARATE questions, then mashes the answers together (convolution) into one aggregate distribution. The 99.9th percentile of THAT becomes your capital number." },
    { title: "Swiss cheese, not fault tree, for realism", text: "Fault tree analysis pretends the cheese slices' holes are randomly placed (independent). The Swiss cheese model admits the holes might actually LINE UP (correlated), which is a more honest picture of how failures cascade." },
  ...
  summary: `<p><strong>Loss data</strong>: at least 10 years, a €20,000 threshold, 4 dates (occurrence→discovery→reporting→accounting); boundary events tied to credit stay credit, tied to market stay operational. <strong>RCSA</strong> (inherent vs residual risk, qualitative, annual) tends to UNDERESTIMATE likelihood and OVERESTIMATE severity when backtested; alternatives are RCA and RRSA. <strong>KRI</strong> (exposure) / <strong>KPI</strong> (performance) / <strong>KCI</strong> (control effectiveness). <strong>Fault tree analysis</strong> multiplies independent probabilities, which is unrealistic since real controls correlate; <strong>root-cause analysis</strong>, the <strong>5-whys</strong>, and the <strong>bow tie diagram</strong> (causes and preventive controls on the left, impacts and detective/corrective controls on the right) dig into WHY and HOW BAD. The <strong>Swiss cheese model</strong> captures FTA's blind spot better. <strong>FAIR model</strong>: factors → measure → Monte Carlo combine. <strong>LDA</strong>: frequency (Poisson) × severity (lognormal), convoluted, 99.9th percentile = capital; UoM homogeneity trades off against data volume; copulas capture cross-UoM tail dependence. <strong>EVT</strong> poorly fits op risk's heterogeneous causes. <strong>Combining loss data</strong>: scaling, cut-off mix, filtering. Capital modeling now centers on Pillar 2's <strong>ICAAP</strong> since the SMA removed the LDA modeling requirement. <strong>Resilience</strong> (7 steps: IBS → tolerances → mapping → scenarios → remediate → communicate → annual self-assessment) is narrower than BCM.</p>`,

  eli5: `<p>Imagine your house has five separate locks and alarms guarding the front door: a fence, a gate lock, a motion sensor, a door lock, and a dog. If you assume each one works completely on its own and calculate the odds a burglar gets past ALL five by multiplying their individual failure chances, you'll get a tiny, reassuring number. But if the same lazy handyman installed the fence, the gate lock, AND the motion sensor all at once, a flaw in his work could quietly break all three at the same time. The real odds of a break-in are higher than your tidy multiplication suggested, because the "independent" defenses secretly share a weakness. That's exactly the trap fault tree analysis falls into when it multiplies probabilities as if control failures were unrelated, and exactly what the Swiss cheese model is designed to remind you of: your defensive layers have holes that drift around, and disaster strikes only when several holes happen to line up at once, which is more likely than pure multiplication implies whenever the layers aren't truly independent.</p>`,

  thinkLike: `<p>A quantitative operational risk analyst treats every "independence" assumption as a claim to be interrogated, not accepted at face value. When you see a fault tree with five multiplied probabilities, do two things in sequence: first, compute the naive product correctly; second, immediately flag that real controls sharing a designer, a vendor, or an IT platform are correlated, so the true joint failure probability is understated. Then check the SCALE of exposure (number of employees, transactions, accounts) before declaring a tiny probability "immaterial", because a tiny per-event chance times a huge exposure base can still be a large aggregate risk.</p>
  <p>When you see LDA, think in two separate modeling jobs rather than one: fit a frequency model (how many events this year: Poisson, one parameter doing double duty as mean and variance) and fit a severity model (how big is each one: lognormal, fat right tail) completely independently, THEN combine them via simulation. The thing worth knowing cold is WHICH distribution goes with WHICH piece (frequency = discrete = Poisson; severity = continuous and fat-tailed = lognormal), and the punchline number: the 99.9th percentile of the resulting aggregate distribution is the capital figure, not the mean and not the median. On the resilience side, think like a regulator auditing a bank's self-assessment: is the bank's list of "important business services" actually narrow and prioritized, or did it just relabel its whole BCM plan as "resilience"? Resilience explicitly excludes intolerable impacts (that's BCM's job) and applies only within impact tolerances for a curated list of IBSs.</p>`,
  ...
  quiz: [
    {
      q: "A fault tree analysis assumes four independent controls, each with a 5% chance of failing. What is the probability that all four fail at the same time, and what is the single biggest reason this figure likely understates real-world risk?",
      options: [
      ...
      answer: 0,
      why: "Multiplying independent probabilities gives 0.05×0.05×0.05×0.05 = 0.00000625 = 0.000625%, correctly computed under the (optimistic) independence assumption. The 1−0.95^4 distractor computes the probability that AT LEAST ONE control fails, an OR condition, not that ALL fail together, which is the AND condition FTA actually models. The key conceptual point is that real controls sharing a designer or platform are correlated, so 0.000625% likely understates the true joint failure probability."
    },
    {
      ...
      answer: 1,
      why: "Frequency counts whole loss events per year, so it needs a discrete distribution: Poisson, whose single parameter λ is both the mean and the variance. Severity is continuous, always positive, and fat-tailed (many small losses, a few huge ones), which lognormal captures well. Swapping the two, or using symmetric normal distributions for both, ignores the actual shape of operational loss data; the generalized-Pareto and uniform pairing belongs to the EVT peaks-over-threshold method, not standard LDA."
    },
    {
      q: "A bank's RCSA rated one operational risk as 'possible' likelihood with 'low' impact, and a separate risk as 'remote' likelihood with 'high' impact. A junior analyst assigns numeric scores (possible=3, low=1, remote=1, high=3) and concludes both risks are equally severe because 3×1 = 1×3 = 3. What is wrong with this reasoning?",
      options: [
        ...
        "Heatmap likelihood and impact ratings are qualitative ordinal labels, not cardinal numbers meant for arithmetic, so equal products don't imply equivalent risk",
        "The analyst used the wrong likelihood scale; heatmaps only use three categories, not five"
      ],
      answer: 2,
      why: "Heatmap categories are qualitative labels combined into color zones (green/yellow/amber/red), not numbers to be multiplied together. A remote/high risk and a possible/low risk are not truly equivalent just because an arbitrary numeric encoding produces the same product. Adding instead of multiplying is invalid for exactly the same reason. The five-point scales (remote/unlikely/possible/likely/highly likely and low/medium/high/very high/extreme) are standard, so the wrong-number-of-categories answer is factually wrong."
    },
    {
      ...
      answer: 2,
      why: "The BCBS rule is that boundary events tied to credit risk are classified and capitalized as credit risk, already captured in RWA, specifically to avoid double-counting the same loss in operational risk capital as well. Classifying it as operational risk alone, or as both, would double-count it, and boundary events are explicitly covered by classification rules rather than excluded from both frameworks."
    },
    {
      q: "Why does extreme value theory (EVT), despite its power in market risk (see R3), have limited practical use for operational risk?",
      options: [
      ...
      answer: 1,
      why: "EVT's block maxima and peaks-over-threshold methods are theoretically justified by extreme losses sharing one generative process, a single dominant cause. Operational risk's causes are genuinely heterogeneous, which undermines that assumption. A 20-year data requirement is not the reason, and EVT is a genuinely quantitative, distribution-fitting technique, which rules out both the normal-distribution-only and the purely-qualitative answers."
    },
    {
      ...
      answer: 1,
      why: "The risk event sits at the center; causes and preventive controls, aimed at stopping the event before it happens, are laid out on the left, while impacts and detective or corrective controls, aimed at limiting damage after the event occurs, are laid out on the right. That lets an analyst reason about frequency levers on one side separately from severity levers on the other. It is not a qualitative/quantitative split, nor is it risk-type-specific."
    }
  ],
