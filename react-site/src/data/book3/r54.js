export default ({
  ...
  teaches: `<p>What a "model" formally is (the Federal Reserve's SR 11-7 definition), the two-way split of model risk into <strong>execution risk</strong> and <strong>conceptual errors</strong>, the job of the model risk management (MRM) function and the four factors that set a model's risk tier, and, at the heart of the reading, three real, named case studies (the Gaussian copula CDO pricing model, the Barclays and Lehman bid spreadsheet, and NASA's Mars Climate Orbiter) that each illustrate a different one of three concrete failure modes: invalid assumptions, implementation error, and input measurement error.</p>`,

  why: `<p>Every quantitative risk model a bank runs (VaR, credit scoring, CDO pricing, capital models) is only as good as its assumptions, its code, and its inputs. This reading exists because "the model said so" has caused real, headline losses: a mispriced multi-trillion-dollar CDO market, a $1.1 billion trading-desk mistake in a bankruptcy auction, and a $125 million satellite gone. The skill worth building is taking a short narrative you've never seen before and bucketing it correctly into invalid assumptions, implementation error, or input measurement error. Those three are R53's "execution risk versus conceptual error" split, cut one level more granular so it maps directly onto real incidents.</p>`,
  ...
  concepts: [
    {
      ...
      def: "The Federal Reserve's SR 11-7 guidance defines a model as \"a quantitative method, system, or approach that applies statistical, economic, financial, or mathematical theories, techniques, and assumptions to process input data into quantitative estimates.\" A model's output is always an estimate or forecast, never a certainty, and it is subject to estimation error by construction. Model risk itself splits two ways: (1) execution risk, where the model doesn't function as intended because of errors in input data or in the coding of the model; (2) conceptual errors, where the assumptions underlying the model are invalid (don't represent reality) or the wrong modeling technique was used for the problem.",
    ...
    },
    {
      ...
      pitfall: "These three failure modes map onto R53's 'error vs. misuse' (execution risk vs. conceptual error) split, cut one level finer: invalid assumptions is a conceptual error, while implementation error and input measurement error are BOTH execution risk, split by whether the break is in the code and process (implementation) or in the data (input). Input measurement error is closer to a data-quality problem (the R43 loss-data concern) wearing a model-risk label than to a 'the model is broken' problem.",
      related: [{ r: 53, label: "R53: the errors-versus-misuse framing this reframes" }, { r: 43, label: "R43: data quality issues, the same root cause wearing a different label" }],
  ...
  connections: {
    ...
    confused: [
      { what: "Invalid assumptions vs implementation error", how: "Invalid assumptions: the DESIGN premise itself is wrong, as when defaults are assumed to stay low-correlated and in a crisis they don't (the Gaussian copula). Implementation error: the design might be fine, but the CODE, PROCESS, or EXECUTION doesn't correctly carry it out, as when a spreadsheet conversion step reveals rows meant to stay hidden (Barclays)." },
      { what: "Implementation error vs input measurement error", how: "Implementation error is a CODE, PROCESS, or LOGIC problem: the instructions were carried out wrong. Input measurement error is a DATA problem: the instructions and logic were carried out exactly right, but on the wrong numbers, as when correct navigation software is fed thruster data in the wrong units (Mars Climate Orbiter)." },
      { what: "Execution risk vs conceptual error (the two-way split) vs the three case-study failure modes", how: "Execution risk and conceptual error are the two BROAD categories from LO 55.a/R53. This reading's three failure modes REFINE execution risk into two sub-cases, implementation error (code and process) and input measurement error (data), while invalid assumptions IS the conceptual-error category, renamed for the case-study context." }
    ]
  },

  misconceptions: [
    { wrong: "\"All model risk tiers require the same validation rigor.\"", right: "Validation strictness scales with tier, which is driven by materiality of output, model complexity, whether it's client-facing, and whether it's used for regulatory compliance. Higher-tier models demand more frequent validation (every 2 to 3 years), comprehensive backtesting, and stricter documentation, data-quality, and version-control standards. But EVERY model, regardless of tier, still gets an annual review." },
    { wrong: "\"Input measurement error is fundamentally a model-design problem.\"", right: "It's closer to a data-quality problem (similar to R43's loss-data quality concerns) wearing a model-risk label, since the model's logic and assumptions could be entirely sound (as in the Mars Climate Orbiter's navigation software) while the data feeding it is simply wrong or mismeasured (units, in that case)." },
    ...
    { wrong: "\"An independent MRM team means model developers no longer need to watch for errors.\"", right: "MRM's existence should NOT create complacency among model users and developers, who remain the first line of defense. MRM review should also be continuous/ongoing, not just a scheduled periodic check, precisely because problems can emerge between reviews." }
  ],
  ...
  highYield: [
    ...
    { stars: 4, what: "The three named case studies (Gaussian copula/CDOs, Barclays/Lehman spreadsheet, NASA Mars Climate Orbiter) and which failure mode each illustrates.", why: "These cases get tested by name, so you need the specific mechanism, not just the label." },
  ...
  recall: [
    { q: "A pricing model correctly implements its designed logic in code, and the underlying theoretical assumptions are sound, but the market data feeding the model contains stale, mismeasured prices. Which failure mode is this, and how does it differ from an implementation error?", a: "This is an input measurement error: the model's code and assumptions are both fine, and the problem is purely in the DATA quality. An implementation error, by contrast, would mean the code itself doesn't correctly execute the intended (and otherwise sound) model logic, which is a coding and execution problem rather than a data problem." },
    { q: "Why would a highly complex, client-facing model used for regulatory capital calculations require stricter validation than a simple internal reporting tool?", a: "Model risk tiering scales validation rigor with materiality of output, complexity, client-facing exposure, and regulatory use, and all four point toward HIGH tiering for this model, which demands more frequent validation, comprehensive backtesting, and stricter documentation, data-quality, and version-control standards than a low-tier internal tool where errors have limited impact and no regulatory consequence." },
    ...
    { q: "In the Barclays bid for Lehman Brothers assets, why is the spreadsheet-to-PDF conversion error classified as an implementation error rather than an invalid assumption?", a: "Barclays' underlying intent (exclude 179 specific positions from the bid) was a sound plan, not a flawed premise. The failure was in EXECUTING that plan: converting the spreadsheet to PDF revealed the hidden rows instead of keeping them concealed. The process step, not the underlying logic or theory, broke." }
  ],

  hooks: [
    { title: "Three places to trip", text: "Design (wrong assumptions), build (wrong code or process), input (wrong data): a model can trip at any one of three stages along its pipeline, and each trip needs a different fix." },
  ...
  eli5: `<p>Imagine your GPS app getting you lost in three completely different ways. First: it assumes all the roads on its map are still open, but a bridge washed out last week and nobody updated the map, so the app's underlying assumption about the world is now wrong, even though the app itself works perfectly. Second: the app's routing logic is sound, but a software update accidentally swapped the "turn left" and "turn right" instructions, so the design is fine but the execution of it is broken. Third: the app and its map are both totally correct, but your phone's GPS chip is reporting your location a mile off. The logic is fine, the assumptions are fine, but the INPUT DATA feeding the system is wrong. In finance, the "GPS app" is a risk model (like a VaR model or the Gaussian copula CDO pricing model): the washed-out-bridge case maps to <strong>invalid assumptions</strong>, the swapped-instructions case maps to <strong>implementation error</strong>, and the bad-GPS-chip case maps to <strong>input measurement error</strong>: three different reasons the same model can lead you to a wrong, and sometimes very expensive, destination.</p>`,

  thinkLike: `<p>A model risk manager's first question on hearing about any model failure is never "was the model wrong?". It is "WHERE in the pipeline did it break: the assumption, the execution, or the input?" That single triage question determines the fix: a bad assumption needs a redesign and better disclosure to users about the model's limits; a bad implementation needs stronger code review, process controls, and testing; a bad input needs data-quality controls (unit checks, source validation, staleness checks) rather than touching the model itself at all. The same triage question also drives tiering decisions upfront: a risk manager sizing how much validation rigor a NEW model deserves is really asking "if this model breaks in one of those three ways, how much would it cost us, and how many people would it reach?", which is exactly the materiality, complexity, client-facing, and regulatory-use checklist.</p>
  <p>This material comes almost entirely as short case narratives you haven't seen phrased this way before: a spreadsheet bug, a unit-conversion slip, a correlation assumption that didn't hold, each demanding a classification. The trap is conflating the two-way split (execution risk versus conceptual error) with the three-way split (invalid assumptions, implementation error, input measurement error). Invalid assumptions IS the conceptual-error bucket, while implementation error and input measurement error are BOTH sub-cases of execution risk, split by whether the break is in the process and code or in the data.</p>`,
  ...
  quiz: [
    {
      q: "David X. Li's Gaussian copula CDO pricing model kept executing its own logic correctly while the CDO market collapsed. Which failure mode does it illustrate, and what specifically broke?",
      options: [
        "Implementation error: the correlation calculation was coded incorrectly",
        "Invalid assumptions: default correlations inferred from calm-market CDS prices were assumed to keep holding in a crisis",
        "Input measurement error: the CDS prices fed into the model were mismeasured",
        "Invalid assumptions: the model was sound for corporate credit but was applied to mortgage pools it was never built for"
      ],
      answer: 1,
      why: "The model's design premise was that CDS-implied correlations, calibrated when markets were calm and mortgage defaults were historically low-correlated, would keep describing reality. In 2008 correlations spiked and the model only caught up with a lag. The code was not buggy and the CDS prices themselves were accurate, which rules out the implementation and input options. The last option describes misuse, applying a sound model outside its domain, but the copula was built for exactly these mortgage pools: the premise inside it was the thing that failed."
    },
    {
      ...
      answer: 1,
      why: "This is the SR 11-7 definition verbatim in substance: a model produces an ESTIMATE, not a precise or error-free output, which is precisely why model risk exists. The 'always produces a precise, error-free output' answer is the tempting trap because it sounds like what a 'good' model should do, but the opposite holds: outputs are forecasts subject to estimation error by construction. A plain spreadsheet or a regulatory template isn't automatically a model unless it applies theory/assumptions to transform inputs into estimates."
    },
    {
      ...
      answer: 2,
      why: "The underlying plan (exclude specific positions) was sound; the failure occurred in EXECUTING that plan via the file-conversion step, a process and implementation error, exactly the Barclays and Lehman case. It is not invalid assumptions (the premise wasn't wrong) or input measurement error (no data was mismeasured): it is a pure execution failure. 'Model risk tiering failure' isn't a failure mode at all, just a distractor assembled from other vocabulary in the reading."
    },
    {
      ...
      answer: 0,
      why: "The four tiering factors are materiality of output, model complexity, whether it's client-facing, and whether it's used for regulatory compliance. The distractors swap in plausible-sounding but wrong factors: backtesting frequency and documentation length are CONSEQUENCES of tiering, meaning what higher tiers require, not inputs into setting the tier, and model age, developer seniority, and IT cost are not tiering criteria at all."
    },
    {
    ...
    {
      q: "An MRM team assigns a model to its highest risk tier. What does that most directly require?",
      options: [
        "The model is retired and replaced with a simpler alternative",
        "More frequent validation (roughly every 2 to 3 years) and comprehensive backtesting, on top of the annual review every model already gets",
      ...
      answer: 1,
      why: "Higher tiers mean MORE scrutiny, not less: more frequent full validation (every 2 to 3 years) plus comprehensive backtesting, layered on top of the annual review every model gets regardless of tier. High-tier models are not retired or exempted from documentation: that is backwards, since stricter documentation, data-quality, and version-control standards are exactly what higher tiers demand. Independent MRM review is required precisely because developers, however knowledgeable, are not independent of their own model."
    }
  ],

  sources: [
    { title: "Federal Reserve SR 11-7: Guidance on Model Risk Management", url: "https://www.federalreserve.gov/supervisionreg/srletters/sr1107.htm", note: "The original supervisory letter defining a 'model' and setting expectations for model risk management, and the source of the core definition used here." }
  ],

  pdf: { book: 3, query: "Models are sophisticated tools used widely in finance" },

  summary: `<p><strong>A model</strong> (per the Fed's SR 11-7) processes input data through theory-driven assumptions into an estimate, never a certainty. <strong>Model risk</strong> splits into execution risk (bad code or bad data) and conceptual errors (bad assumptions or wrong technique). <strong>Model risk tiering</strong> scales validation rigor with materiality, complexity, client-facing exposure, and regulatory use, run by an independent MRM team on a continuous basis. <strong>Three case-study failure modes</strong>: invalid assumptions (the Gaussian copula CDO model, a design-stage premise failure), implementation error (Barclays' Lehman bid spreadsheet, a code and process execution failure), and input measurement error (NASA's Mars Climate Orbiter, a data-stage unit mismatch, closer to a data-quality problem than a model problem). All three map onto R53's error-versus-misuse split from a different angle.</p>`
});
