export default ({
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "The two routes to understanding why a model decided what it decided. You either build a model simple enough to read directly (interpretable-by-design) or you keep the complex model and bolt an explanation onto it afterwards (post-hoc methods such as LIME and SHAP). The breakdown below sets out both, with the flaw of each.",
      intuition: "SHAP is worth understanding rather than just naming, because the game-theory language in it is literal. It treats each input variable, a borrower's debt-to-income ratio say, as a player in a cooperative game, and computes that variable's Shapley value: its fair share of credit or blame for pushing the model's output away from the average prediction, averaged over every order in which the variables could have been added. That is the same mathematics used to split a payoff fairly among coalition members, which is why the explanation it produces is an attribution rather than a rule.",
    ...
    },
    {
      ...
      related: [],
      memory: "Clusters with near-equal 'yes'/'no' splits are a RED FLAG (possible model error), not evidence of balanced or fair decision-making. Frequently tested reversal of intuition."
    }
  ],
  ...
  highYield: [
    ...
    { stars: 4, what: "SHAP clustering: near-equal yes/no clusters signal a MODEL ERROR, not fairness.", why: "A specific and counter-intuitive result, frequently tested reversal from the case study." },
  ...
  eli5: `<p>Imagine a loan officer who has processed a million applications and has genuinely excellent instincts. She is also completely tight-lipped. Ask her why she denied your application and she shrugs: "it felt wrong."</p>
  <p>Her accuracy is not the problem. The problem is that you cannot tell WHICH pattern her instincts absorbed. Maybe she learned that thin credit histories default more often, which is a real risk signal. Maybe she learned that a particular neighbourhood defaults more often, which is a proxy for something nobody is allowed to price on. From the outside, both look identical: a confident denial and a shrug.</p>
  <p>One fix is to fire her and hire someone who follows a simple written checklist instead. Every decision now comes with a reason. But the checklist cannot hold everything she knew, so you have traded accuracy for the ability to explain yourself. That trade is what "interpretable-by-design" means, and it is a real cost, not a free improvement.</p>
  <p>The other fix keeps her and hires a second person to watch her work. He cannot read her mind, so he reconstructs each decision afterwards: he takes an application she rejected, changes the income slightly, runs it past her again, and watches what flips. Do that across enough cases and you can say which inputs were actually carrying the decision. That is post-hoc explainable AI, and LIME and SHAP are two ways of doing it.</p>
  <p>He also keeps a diary: everything she has learned, every file she has seen, dated. That sounds like bureaucracy until a decision from eighteen months ago is challenged. She has learned continuously since then, so today's version of her cannot tell you what the old version was thinking. Only the diary can, which is why a self-learning model must archive its parameters and versions rather than just its outputs.</p>
  <p>So: the loan officer is the black-box model, the diary is the version archive that makes past decisions explainable, and the second person's reconstruction is what SHAP and LIME actually produce. Note what he produces is an approximation of her reasoning, not her reasoning itself. That gap is permanent, and it is why post-hoc explanation makes a model less opaque rather than transparent.</p>`,
  ...
  quiz: [
    ...
    {
      q: "Which of the following is NOT something an AI model developer can fully control or guarantee?",
      options: [
