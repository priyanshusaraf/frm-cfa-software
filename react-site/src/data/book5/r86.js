export default ({
  ...
  thinkLike: `<p>A risk manager doing risk budgeting never opens with "how much money should go into this position?". She opens with "how much of my risk budget will this consume, given what I already hold?"</p>
  <p>That reframing is the whole reading. It is also why the worked example matters more than its numbers: choosing between position X and position Y to add to an existing $500M position in W, the intuitively safer choice, X with the lower stand-alone VaR, is wrong.</p>
  <p>It is wrong because correlation with the existing book determines how much NEW risk a position adds. Risk in isolation is a property of the position; risk added is a property of the portfolio.</p>
  <p>Three question shapes follow. The first is the stand-alone versus incremental VaR trap, built almost exactly like the X and Y example, so learn the mechanic rather than the numbers.</p>
  <p>The second is a surplus-at-risk calculation for a pension fund, which can come either as the direct approach (VaR of assets minus expected surplus growth) or through the volatility of surplus via covariances. Both forms appear, so be ready for either.</p>
  <p>The third asks why active management risk usually does not matter much and when it suddenly does. The answer is that it matters when managers act the same way independently, every bond manager extending duration on the same rate call, which is when diversification across managers quietly stops existing.</p>`,
  breakdown: [
    {
    ...
    {
      title: "The global custodian question",
      points: [
        "The industry has been consolidating portfolios under a single GLOBAL CUSTODIAN, and the custodian industry has itself been consolidating into fewer, larger firms. One custodian holding everything is the easy route to centralized risk management: it can see the whole fund at once, and it can combine its own reports on position changes with market data to produce forward-looking risk measures rather than backward-looking statements.",
        "Firms that decline a global custodian do so because they believe they keep tighter control of their own risk measures that way, and can wire VaR systems more deeply into their own operations. Larger firms also have economies of scale, spreading the cost of a risk system across a big asset base, and they want tighter control where some assets are managed internally. The trade-off is therefore visibility against control, not cost against convenience.",
      ]
    },
    {
  ...
  concepts: [
    ...
    },
    {
      ...
      pitfall: "The central numeric lesson here: focusing on STAND-ALONE VaR alone can lead to the WRONG selection, what matters is the candidate's CORRELATION with existing holdings (its incremental contribution), not its isolated risk level.",
  ...
  highYield: [
    ...
    { stars: 3, what: "Buy-side vs sell-side differences in horizon, turnover, leverage, and VaR adoption.", why: "Foundational context, and as a comparison table." },
