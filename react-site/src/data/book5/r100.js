export default ({
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "The four building blocks of the crypto ecosystem, each covered in the breakdown below: unbacked crypto (Bitcoin, Ethereum), stablecoins, smart contracts and DeFi. They are worth learning as a stack rather than a list. Unbacked crypto is too volatile to price anything in, which creates the demand for stablecoins; stablecoins make on-chain contracting viable, which is what smart contracts automate; and DeFi is what you get when those contracts are composed into financial services with no intermediary.",
      ...
      pitfall: "The claim that algorithmic stablecoins are backed by real-world assets is simply false, and it is the one most likely to be dangled as true. Algorithmic stablecoins hold no such backing; that is the entire point of the design and the entire source of its fragility.",
    ...
    },
    {
      ...
      pitfall: "The three properties in the trilemma are security, decentralization and scalability, and no others. Expect plausible-sounding substitutes (throughput, privacy, immutability) offered in their place.",
    ...
    },
    {
      ...
      counter: "A stablecoin pegged to the dollar need not be convertible into dollars. TerraUSD maintained its 1:1 peg through convertibility into its paired token Luna, never into actual currency, which is exactly why the peg could evaporate.",
    ...
    },
    {
      ...
      def: "The gap between how crypto describes its governance and where power actually sits. Despite the decentralization marketing, decision-making concentrates among a small group in practice, and the breakdown below traces the specific mechanisms. The structural reason is economic rather than ideological: validation and mining carry largely FIXED costs, so scale pays, and anything that pays for scale concentrates.",
      ...
      counter: "PoS is often marketed as an improvement over PoW (less congestion, lower energy use), but PoS STILL leads to power/coin concentration, just through a different mechanism (fixed operational costs favoring large stakers) rather than solving centralization.",
    ...
    },
    {
      ...
      example: "Importing real-world data into a blockchain for a smart contract necessarily requires a third party, the oracle. A smart contract cannot fetch outside data for itself, which is why the oracle is a trust dependency the design was supposed to eliminate.",
      counter: "It might seem like decentralization would make oracles more trustworthy (no single point of failure), but decentralization here actually makes recourse HARDER when an oracle is compromised, since there's no central party to hold accountable.",
      pitfall: "Do not let the oracle fact become the trap. That smart contracts cannot directly access off-chain information is TRUE; the false statement in this family is always the one claiming algorithmic stablecoins carry real-world backing.",
    ...
    },
    {
      ...
      example: "Encouraging CBDCs is favoured over taxing transactions heavily, banning ICOs and DEXs, or imposing strict exchange and wallet regulation, because CBDCs reduce costs, enhance financial inclusion, and bolster financial-system integrity without the enforcement/innovation-stifling costs of the other options.",
      counter: "Banning crypto might seem like the most protective option, but bans have the unintended consequence of driving activity underground, arguably WORSE for oversight than regulation, which at least keeps activity visible.",
  ...
  highYield: [
    { stars: 4, what: "The scalability trilemma.", why: "Security, decentralization, scalability: pick only two. A near-guaranteed identification question." },
  ...
  summary: `<p>The crypto ecosystem is built from four layered elements: unbacked crypto (too volatile to use as money) → stablecoins (pegged, either asset-backed or algorithmic, to fix that volatility) → smart contracts (self-executing code automating transactions) → DeFi (financial services built from composed smart contracts). Structural flaws recur because the ecosystem is young: the scalability trilemma forces a permanent trade-off among security/decentralization/scalability; congestion and its offspring fragmentation degrade interoperability and security; stablecoin anchors are only as sound as their (sometimes yield-chasing) reserve backing; 'decentralization' is often a marketing claim undercut by real power concentration in PoW mining, PoS staking, governance tokens, and centralized exchanges (FTX, Mt. Gox); and the oracle problem means smart contracts are only as trustworthy as the off-chain data feeding them.</p>
  <p>Crypto poses distinct risks to four parties. Investors (volatility, fraud, exchange collapse), governments (AML/CFT, tax revenue, cryptoization), regulators (jurisdictional fragmentation), and traditional financial institutions (indirect exposure via customers, future tokenization-driven interconnection, competitive threat), addressed through a policy menu of containment, regulation, bans, and CBDC development, each with its own cost/benefit trade-off.</p>`,

  eli5: `<p>Your neighbourhood has always relied on one shopkeeper who keeps the master notebook of who owes whom. Everyone trusts the notebook because everyone trusts him.</p>
  <p>Now change exactly one thing: nobody trusts the shopkeeper any more. So the neighbourhood photocopies the notebook and gives a copy to every household. No page can be quietly erased, because everyone holds their own copy.</p>
  <p>But that creates a problem the shopkeeper used to solve. When copies disagree, which is the real one? So the neighbourhood pays a rotating crew of volunteers, in coins, to check each new entry before it is added and copied out.</p>
  <p>The crew can only handle so many entries an hour. A backlog forms, and people begin paying extra to jump the queue, which prices out anyone with a small entry to record.</p>
  <p>So some neighbours start a second notebook down the street. It clears entries faster, because it has fewer and less careful volunteers watching it. Now there are two notebooks nobody can reconcile.</p>
  <p>That chain is the whole story. The shared notebook is the blockchain, the crew is validators or miners competing for a block reward, the backlog is congestion, and the rival notebook is fragmentation: faster, and less secure and less decentralized for exactly that reason.</p>`,
  thinkLike: "<p>A risk manager evaluating anything in the crypto ecosystem should run one habitual test first: <em>which classic risk have I actually just relabeled?</em> Congestion and its fees are a liquidity-cost problem wearing new clothes; fragmentation is market segmentation and interoperability risk; a stablecoin losing its peg is a collateral/liquidity-quality failure identical in mechanism to a money-market fund breaking the buck; false decentralization is concentration and counterparty risk hiding behind marketing language; cryptoization is currency substitution/dollarization; and a CEX collapse (FTX, Mt. Gox) is custodial/counterparty risk with zero regulatory backstop. Once you've named the underlying classical risk, you already know the right due-diligence question to ask, is the collateral high-quality and transparent? who actually holds veto power? what's the recourse if this breaks?, without needing crypto-specific intuition at all.</p><p>Two question shapes recur: (1) taxonomy placement, where a stem describes something and asks which of the four risk-bearer categories it belongs to, or which of the structural flaws it illustrates, where the trap is blending two adjacent concepts (e.g., cryptoization vs. AML/CFT. Both 'government risk' but mechanistically distinct); and (2) 'least accurate / least likely' questions built around a single deliberately false statement embedded in a list of true ones (e.g., 'algorithmic stablecoins are backed by real-world assets' is the false one). Read every option as a candidate for being the deliberate false statement, not just the first one that sounds right.</p>",
  ...
  quiz: [
    ...
    {
      q: "Among containment, regulation, bans, and encouraging CBDCs, why is CBDC development the particularly attractive policy response relative to heavy taxation or outright bans?",
      options: [
