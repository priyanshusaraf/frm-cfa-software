export default ({
  ...
  eli5: `<p>Start with a household budget, where nothing is complicated yet. Before a single rupee moves, you decide two things: how bad a month can get before it stops being annoying and starts being a crisis, and what "a good month" would actually look like. That is the whole of planning, and notice it happens before you have spent anything.</p>
  <p>Then you split the paycheck into buckets. Rent, groceries, savings, each with a number attached and a job to do. That is budgeting, and it is not the same activity as the first one: planning set the boundary, budgeting decides how to live inside it.</p>
  <p>At the end of the month you open the bank statement and put it next to the plan. This is where you find out you spent three times your food budget on takeout without ever making a decision to do so. Nobody chose that; it accumulated. Monitoring exists because drift does not announce itself.</p>
  <p>Now change exactly one thing: the resource being allocated is RISK rather than money. An investment firm runs the same three steps in the same order. It decides in advance how much loss it can tolerate and what return would count as success. It then hands out risk, not just capital, across strategies and managers. And it checks realized risk against the budget often enough to catch a manager drifting away from the mandate.</p>
  <p>What that last step buys you is the whole point. Without it you find out about the drift the way you find out about the takeout: from the statement, after the money is gone.</p>`,
  ...
  hooks: [
    ...
    { title: "The value manager who isn't", text: "Style drift is the cautionary tale here: a manager can hit every aggregate risk number perfectly while quietly betting on something completely different than what clients signed up for, decomposition is the only way to catch it." },
  ...
  quiz: [
    ...
    {
      q: "Which of the following is one of the five risk planning objectives?",
      options: [
