export default ({
  ...
  eli5: `<p>A company car lease carries an unusual clause. If the driver ever fails a blood-alcohol test, the car is reassigned to a backup driver automatically. No lawsuit, no negotiation, no meeting. It simply happens, because both sides signed that clause when the lease was written.</p>
  <p>A CoCo bond is that pre-signed clause in financial form. Bondholders agree in advance that if the bank's health drops below a defined line, or a regulator declares it non-viable, their bond converts to stock or is written down on the spot.</p>
  <p>The reason anyone wants this is timing. A rescue negotiated during a crisis takes weeks the bank does not have. A clause agreed years earlier fires in an afternoon.</p>
  <p>Now the part Credit Suisse's investors learned the expensive way. They believed they held the lease and that shareholders were the backup driver, absorbing losses first, as equity normally does.</p>
  <p>The contract they had actually signed put them below the shareholders for that particular loss. Their AT1 bonds were written to zero while shareholders still received something in the merger.</p>
  <p>The lesson is not that CoCos are a trap. It is that the pecking order is a matter of contract, not of intuition about what "bond" and "share" usually mean.</p>`,
  ...
  breakdown: [
    ...
    {
      title: "Problems the Credit Suisse CoCo write-down created",
      points: [
  ...
  quiz: [
    ...
    {
      q: "Which feature is NOT part of the proposed reformed CoCo structure?",
      options: [
