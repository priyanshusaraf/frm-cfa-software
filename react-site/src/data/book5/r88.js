export default ({
  ...
  eli5: `<p>You are grading a personal trainer on how much weight their clients lost. Simple enough, until you notice the gym was packed some months and nearly empty in others.</p>
  <p>Average the results across every session and the crowded months dominate, even though the trainer did exactly the same job in every one. You have measured attendance, not skill.</p>
  <p>So there are two honest averages, answering two different questions. Weight each period by how many clients were in the room and you get what clients actually experienced. Treat every session equally and you get how good the trainer is.</p>
  <p>Those are the dollar-weighted and time-weighted returns. The first is the investor's real outcome, including the effect of when they put money in. The second isolates the manager, who did not choose the timing.</p>
  <p>The rest of this reading is a box of rulers for the second question. Sharpe, Treynor, Jensen's alpha, the information ratio and M-squared each define "how hard was the workout" differently, which is why they can disagree about the same manager.</p>
  <p>And a trainer who knows which ruler is being used can play to it: an easy programme one year and a brutal one the next flatters some measures and not others. A fund manager can game the Sharpe ratio the same way, by shifting risk mid-evaluation.</p>`,
  ...
  breakdown: [
    {
      ...
      points: [
        "Universe comparison: the crude alternative that comes before any of the ratios below. Sort every portfolio by investment style (small-cap growth, small-cap value, large-cap growth, large-cap value) and rank on raw return WITHIN its style bucket. Standardizing on style makes the ranking mean more than a naked return league table, but the method breaks the moment two funds in the same bucket carry genuinely different risk, which is the exact hole the risk-adjusted measures below exist to close.",
  ...
  formulas: [
    ...
    { name: "Information ratio", math: "\\text{IR} = \\dfrac{R_P - R_B}{\\sigma\\left(R_P-R_B\\right)}", note: "Surplus return over a BENCHMARK (not risk-free rate), divided by tracking error.",
      plain: "Also called the APPRAISAL RATIO, and both names appear in practice for the same quantity. The information ratio says: for every one unit of tracking-error risk (volatility of the return gap vs. the benchmark) the manager took on, how much surplus return over that benchmark did the active bets deliver?" },
  ...
  highYield: [
    ...
    { stars: 4, what: "Sharpe ratio manipulation via dynamic risk-shifting. The two-year vs. single-year worked example.", why: "A precise and memorable case. Frequently tested performance-manipulation mechanism." },
