export default ({
  ...
  intuition: `<p>SVB's core vulnerability was a mismatch hiding in plain sight. A depositor base concentrated almost entirely in one correlated group, tech and life-science startups and their venture capital (VC) backers, funded a long-duration held-to-maturity (HTM) securities portfolio: weighted-average duration 6.2 years, most maturities 10 years or longer as of December 2022.</p>
  <p>That worked beautifully while deposits kept growing. From a founding base of just $18 million in 1983, deposits reached $58 billion by 2019, then TRIPLED to $209 billion by 2021 as clients cashed out IPOs, secondary offerings, SPAC mergers and acquisitions and parked the proceeds at SVB.</p>
  <p>Then VC funding activity slowed in the second half of 2022, because rate increases had cut investor risk appetite. The SAME correlated depositor base began withdrawing SIMULTANEOUSLY to fund their own operating cash needs.</p>
  <p>And they did it exactly when the HTM portfolio had lost substantial value to those very same rate increases.</p>
  <p>What that is, mechanically, is <em>maturity transformation</em>: using short-term deposit funding to hold longer-term assets, which every bank does. What pushed it to a dangerous extreme is that both sides of the mismatch were driven by one underlying macro variable (interest rates, and the VC risk appetite that follows them) instead of being independent of each other.</p>
  <p>Then there is the accounting trap. HTM securities are carried at historical cost, which hides losses.</p>
  <p>But under the accounting rule then in force, selling even a PORTION of the HTM portfolio would have required RECLASSIFYING THE ENTIRE HTM PORTFOLIO to available-for-sale (AFS). That forces mark-to-market recognition of losses across the whole book at once, not just on the piece sold.</p>
  <p>So the rule created a powerful disincentive to address the problem early, and the losses compounded silently until forced action became unavoidable.</p>
  <p>By the end of 2022 that specific rule had been relaxed, and management was weighing a one-time transfer out of HTM that would no longer have triggered a full reclassification. By then the deposit-flight clock was already running out.</p>
  <p>The regulatory story matters just as much. A July 2018 rule change RAISED the asset threshold for Large and Foreign Banking Organization (LFBO) status, which carries stricter supervision, from $50 billion to $100 billion.</p>
  <p>That change alone delayed SVB's transition to stricter oversight by roughly three years. It would otherwise have crossed into LFBO status around 2018.</p>
  <p>When the transition finally came in February 2021, it arrived as a cliff event: a regional banking organization (RBO) one day, an LFBO the next, with no gradual ramp.</p>
  <p>Three things compounded that. A COVID-era examination pause. The ongoing RBO supervision cycle being allowed to run to completion first, which it did not do until July 2021. And a 2021 cultural shift at the Fed's Division of Supervision and Regulation (DSR) toward LESS regulatory burden on RBO banks.</p>
  <p>That last shift happened even as industry-wide banking assets grew 37% from 2016 to 2022 while regulatory supervisor headcount FELL 3% over the same period.</p>
  <p>Supervisors did formally flag governance as "Deficient-1" in August 2022, and began the multi-stakeholder Memorandum of Understanding (MOU) process. The MOU needed sign-off from the Federal Reserve Bank of San Francisco (FRBSF), the DSR, the Board of Governors' legal department, and the California Department of Financial Protection and Innovation (CDFPI). SVB failed before it could be finalized.</p>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "What broke when SVB crossed from Regional Banking Organization status ($10B to $100B in assets, lighter supervision under the Bank Exams Tailored to Risk programme) into Large and Foreign Banking Organization status. The transition was not a step but a gap: raising the threshold from $50B to $100B delayed stricter supervision by roughly three years, and when the new regime did arrive its reviews restarted rather than continued, so findings accumulated slowly while the balance sheet grew fast. The breakdown below traces the sequence.",
    ...
    },
    {
      ...
      def: "Six exposures that were each survivable alone and lethal together, listed in the breakdown below. The through-line is correlation: a depositor base drawn almost entirely from tech and life-science startups, venture capital and private equity meant the same shock that dried up funding for the depositors also drove them to withdraw, at the same time, in size, from a bank whose assets could only be sold at a loss.",
    ...
    },
    {
      ...
      def: "The supervisory findings that said SVB's governance was failing well before the run, and the record of them going unheeded. Its CAMELS management rating stayed at Satisfactory-2 through 2018, 2019 and 2020 while the underlying issues accumulated, and the three Matters Requiring Immediate Attention issued in the May 31, 2022 report are set out in the breakdown below. The pattern worth taking away is that the findings were correct and the escalation was not.",
    ...
    },
    {
      ...
      def: "Two measures of interest rate risk that answer different questions, and SVB watched the wrong one. Net interest income (NII) at risk measures SHORT-TERM earnings volatility under yield-curve moves including twists. Economic value of equity (EVE) measures the LONG-TERM change in the present value of assets against liabilities, which is where a portfolio of long-dated fixed-rate securities funded by demand deposits actually bleeds. A bank can look calm on NII while EVE is deteriorating badly, which is precisely what happened.",
