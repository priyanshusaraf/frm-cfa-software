export default ({
  ...
  teaches: `<p>Mapping is the bridge between the single-profit-and-loss (P&L) series machinery of Readings 1-4 and real, thousand-position portfolios. You'll see why mapping is necessary, the general-vs-specific risk trade-off, three fixed-income mapping methods of increasing precision (with a fully worked two-bond example), stress testing via undiversified Value at Risk (VaR), tracking error VaR against a benchmark (with a worked barbell-vs-bullet example), and how mapping extends to derivatives: forwards, forward rate agreements (FRAs), swaps, and options, using real worked numbers from an FX forward, a 6×12 FRA, and a short-horizon option.</p>`,
  ...
  concepts: [
    {
      ...
      intuition: "Mapping runs in two mechanical steps. First you measure the current market value of every position in the portfolio. Second, you split each position's market value across a small shared set of risk factors using factor exposures (a stock's return, for example, decomposes via its beta into a market-index piece plus a leftover firm-specific piece). Once every position has been rewritten this way, the portfolio's total exposure to each risk factor is just the column sum across all positions: a handful of numbers instead of thousands.",
  ...
  highYield: [
    ...
    { stars: 3, what: "Tracking error VaR vs absolute VaR can diverge (barbell vs bullet example).", why: "This is the trap worth watching for: low own-risk does not mean low divergence from the benchmark." },
  ...
  <p>Three question shapes recur here. One is the "which of the following is NOT a reason for mapping" or "NOT one of the three fixed-income methods" form: memorize the exact lists so a plausible-sounding decoy (like "present value mapping") doesn't fool you. Another is a numeric principal-mapping or ranking question using a two-bond setup structurally identical to the $100M/$100M worked example: practice reproducing the weighted-average-life calculation cold. The third is a conceptual trap pairing two things that sound like they should move together but don't: specific risk treated as fixed, or minimizing tracking error confused with minimizing absolute VaR (the barbell-vs-bullet case). Expect a scenario description that asks you to identify which of these traps it illustrates, rather than asking you to name the trap directly.</p>`,
  ...
  breakdown: [
    ...
    },
    {
      ...
      points: [
        ...
        "FRAs decompose into an implied borrowing leg and an implied investing leg over the two relevant periods (a 6×12 FRA implies borrowing for 6 months and investing for 12 months).",
  ...
  quiz: [
    ...
    {
      q: "A portfolio manager builds a barbell portfolio (heavily weighted at the very short and very long ends of the curve, little in the middle) that is duration-matched to a bulleted (mid-curve concentrated) benchmark. Compared to the benchmark, what typically happens to the barbell's absolute VaR and its tracking error?",
      options: [
