export default ({
  ...
  why: `<p>If markets truly believed lognormal returns (BSM's core assumption — the assumption that the stock/currency price is lognormally distributed with one constant volatility for the whole life of the option), every option on the same underlying and maturity would trade at the same implied volatility regardless of strike. They don't — and the specific pattern of disagreement is a direct, tradeable readout of what the market believes about fat tails, skewness, and jump risk that the plain BSM formula cannot see. A trader who ignores the smile and prices every strike off one flat volatility will systematically misprice away-from-the-money options — mispricing that shows up immediately as an arbitrage opportunity for anyone using the market's actual (smile-consistent) prices instead. This is BSM's constant-volatility assumption failing exactly the way Reading 11 warned it would for bonds (where a single flat rate-volatility number also proved too crude) — now the same failure shows up for options generally.</p>`,

  intuition: `<p>Start from put-call parity, the formula \\(c + Xe^{-rT} = p + S_{0}\\) that relates a European call price \\(c\\) and put price \\(p\\) at the same strike \\(X\\) and maturity \\(T\\). This equation is derived purely from no-arbitrage — buy the call, sell the put, and you have synthetically replicated a forward contract, regardless of what distribution the underlying actually follows. Because it holds no matter what the true distribution is, if the ACTUAL market call price deviates from what BSM predicts by some dollar amount, put-call parity forces the actual market put price to deviate from its BSM prediction by that exact same dollar amount (same sign, same size) — because both sides of the parity equation must still balance. Since implied volatility is just "the volatility number that, plugged into BSM, reproduces the market price," and both option prices moved by the identical dollar amount, the implied volatility you back out for the call and the implied volatility you back out for the put at that same strike MUST be equal. This is not an empirical finding you could someday falsify — it's arithmetic.</p>
  <p>The shape of the smile itself is diagnostic — it tells you what the market believes about the true distribution of the underlying, distinct from BSM's lognormal assumption. A <strong>smile</strong> (typical of currency/FX options, e.g. options on EUR/USD) means the market prices deep in-the-money AND deep out-of-the-money options with higher implied volatility than at-the-money options — fatter tails on BOTH sides than lognormal — driven by the fact that exchange-rate volatility is not constant and jumps between regimes (e.g. a central bank suddenly shifting policy), raising the odds of an extreme move in either direction; this effect is weaker for long-dated options because regime shifts average out over a longer horizon. A <strong>skew/smirk</strong> (typical of equity index options, e.g. options on the S&P 500) means implied volatility is HIGHER for low-strike options (in-the-money calls, out-of-the-money puts) and LOWER for high-strike options (in-the-money puts, out-of-the-money calls) — an inverse relationship between implied vol and \\(X/S_{0}\\) that translates into a left-skewed implied distribution: traders believe a big down move is more likely than a big up move of the same size. Two separate causes are given for this: the leverage effect (a mechanical, real change in volatility — as a firm's equity value falls, its debt-to-equity ratio rises, which genuinely increases the riskiness of the remaining equity) and "crashophobia" (a name coined by Mark Rubinstein after the 1987 crash — market participants pay a fear premium for deep out-of-the-money puts, which act as crash insurance, purely because they are afraid of another crash, independent of whether volatility itself has actually changed; support for this story is that the skew steepens when markets fall but doesn't flatten symmetrically when markets rise). A <strong>frown</strong> means the market is pricing in an anticipated binary/news event (e.g. a pending court ruling, an FDA drug approval decision, an election) that will send the price sharply one way or the other — making the outcome bimodal rather than fat-tailed, so at-the-money options (which straddle the "coin flip" outcome) get bid up relative to away-from-the-money options — the mirror image of a smile's shape.</p>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "Smile (currency/FX options, e.g. EUR/USD options): implied volatility is higher for BOTH deep in-the-money and deep out-of-the-money strikes than for at-the-money strikes — fatter tails on BOTH sides than lognormal, caused by exchange-rate volatility itself not being constant (it jumps between regimes, e.g. around central-bank surprises), less pronounced for long-dated options because regime shifts average out over time. Skew/smirk (equity options, e.g. S&P 500 index options): implied volatility falls as \\(X/S_{0}\\) rises — higher for low-strike options (in-the-money calls, out-of-the-money puts), lower for high-strike options (in-the-money puts, out-of-the-money calls) — a left-skewed implied distribution, caused by the leverage effect and crashophobia. Frown (anticipated binary event, e.g. a pending court ruling or drug-trial result): at-the-money implied vol is HIGHER than away-from-the-money — the opposite shape of a smile, reflecting a bimodal (two-outcome, not fat-tailed) distribution.",
      example: "GARP's own module-quiz phrasing: an empirical distribution with a fatter RIGHT tail than lognormal generates higher implied volatility for HIGH strike prices — meaning in-the-money calls and out-of-the-money puts would be priced as 'expensive' relative to flat-vol BSM. Flip the logic for a left-skewed (equity) distribution: the fat tail is on the low-strike side instead.",
      pitfall: "'Crashophobia' is about the PRICE of protection (deep OTM puts carry a fear premium because everyone remembers October 1987) — NOT a claim that actual realized volatility rises when prices fall. Don't conflate the leverage effect's story (volatility genuinely does rise, mechanically, as leverage rises) with crashophobia's story (a risk-aversion premium baked into pricing, independent of whether volatility itself changes). GARP's own module quiz flags this exact trap directly.",
    ...
    },
    {
      ...
      def: "The volatility smile plots implied vol against \\(X/S_{0}\\) by default. Traders sometimes swap the x-axis variable: X alone (just the strike price — less stable, because it depends on the current price level and needs re-plotting every time the underlying moves), \\(X/F_{0}\\) (strike divided by the forward price for the underlying with matching maturity — often preferred since the forward price is the theoretical expected future price, making it a cleaner definition of 'at-the-money'), or option delta (lets traders study the smile even for options that aren't plain European/American calls and puts, since delta is defined for a wider range of payoffs).",
    ...
    },
    {
      ...
      def: "Standard BSM delta assumes implied volatility stays fixed as the equity price changes — it only models effect (1): moving along the existing smile curve (i.e., as the equity price changes, \\(X/S_{0}\\) changes, which moves you to a different point on the SAME curve in the chart). But there is a second effect the standard formula ignores entirely: (2) the WHOLE curve shifting up or down, because equity price and volatility are typically negatively correlated (when equity prices fall, the leverage effect and crashophobia both push implied vol up — this is the same story as the skew concept above, but now viewed dynamically rather than as a static snapshot).",
      pitfall: "Effect (2) DOMINATES effect (1) in practice, per the source text. The minimum variance delta is the delta measure that incorporates this whole-curve shift; it comes out LOWER than the standard BSM delta, which ignores the shift entirely — a common trap is assuming standard BSM delta already captures this, or (as one GARP distractor puts it) assuming the minimum variance delta is HIGHER than BSM delta. It is not — it is lower.",
      related: ["Smile, skew, and frown"],
      memory: "Standard delta only walks along the smile; minimum variance delta also accounts for the smile itself moving — and that whole-curve move is the bigger effect, which is why minimum variance delta comes out lower."
    },
    {
