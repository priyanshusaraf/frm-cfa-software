export default ({
  ...
  teaches: `<p>This reading surveys correlation as a risk in its own right, not just an input you plug into a Value at Risk (VaR) formula. It covers five things. It starts with the portfolio mean/variance foundation from Markowitz, with a worked numeric example computing variance, covariance, and correlation from raw price data, so you see where a correlation number actually comes from. Then wrong-way risk (WWR) in credit default swaps (CDS), using a concrete example of a U.S. investor buying protection on French government bonds from Deutsche Bank. Third, correlation options and correlation swaps, including the exchange option, the quanto option, and a fully worked correlation-swap payoff. Fourth, how correlation feeds directly into the VaR capital charge banks must hold. Fifth, the correlation-trading blowup that arrived three years before the crisis: hedge funds long the collateralized debt obligation (CDO) equity tranche and short the CDO mezzanine tranche, who lost money on <em>both</em> legs when GM and Ford were downgraded to junk in May 2005, plus concentration and systemic risk. Readings 8 and 9 go deep on the empirics (how correlation actually behaves over time) and the modeling machinery (copulas) that this reading only gestures at.</p>`,
  ...
  formulas: [
    ...
    },
    {
      ...
      note: "The Basel Committee on Banking Supervision multiplier applied to 10-day VaR for trading-book assets (stocks, futures, options, swaps marked to market).",
  ...
  concepts: [
    ...
    {
      name: "The CDO tranche trade of May 2005",
    ...
    {
      name: "The wider 2007-2009 correlation story: systemic risk and market scale",
      ...
      memory: "2007 to 2009 in one line: correlations that looked low and stable in a calm market snapped toward 1 exactly when everyone needed them to stay low."
    },
    {
  ...
  misconceptions: [
    ...
    { wrong: "\"The CDO tranche trade lost money because the long leg alone went bad.\"", right: "Both legs lost simultaneously. Mezzanine correlation fell (lowering its spread, hurting the short position) while equity spreads rose (hurting the long position). The 'hedge' failed on both sides at once." },
  ...
  highYield: [
    { stars: 5, what: "The May 2005 correlation-trading mechanism: which leg moved which way and why.", why: "A frequently tested scenario question requiring you to track two simultaneous, counter-intuitive moves." },
    ...
    { stars: 4, what: "Concentration ratio + default correlation jointly determining EL (the worked $5M/$2.5M example).", why: "A clean numeric example that gets varied constantly in practice questions." },
    ...
    { stars: 3, what: "Correlation swap payoff calculation (fixed vs realized correlation, notional scaling).", why: "A direct plug-and-chug calculation that shows up with different notionals and asset counts. Practice recomputing realized correlation from pairwise inputs." }
  ],
  ...
  recall: [
    ...
    { q: "In the GM/Ford tranche trade, why did the mezzanine (short) leg lose money instead of gaining?", a: "Mezzanine tranche correlation FELL after the GM/Ford downgrade, which lowered mezzanine spreads. A short position profits when spreads RISE; falling spreads meant the short leg lost money too, the opposite of what the 'hedge' was supposed to do." },
  ...
  hooks: [
    { title: "Two lost legs", text: "The May 2005 tranche trade: 'long equity, short mezz, correlation-hedged' sounds safe. Remember it as TWO SEPARATE LOSSES: equity spreads UP (long leg loses), mezz spreads DOWN (short leg loses too). A hedge where both legs lose is the whole lesson." },
  ...
  <p>Two question shapes recur. The first is plug-and-chug numeric work: compute a correlation swap payoff from given pairwise realized correlations and a fixed strike, or compute 10-day VaR (and the resulting 3x capital charge) from a two-asset covariance matrix. These reward simply doing the arithmetic correctly and knowing which formula piece changes with which input. The second is scenario and direction questions, most often the GM/Ford tranche trade: you are given a downgrade event and asked which leg gained, which lost, and why. The trap is assuming the "hedged" position must have offset, when the entire teaching point is that it didn't. A third, subtler pattern is the "except" question: which correlation-sensitive strategy behaves opposite to the general rule (worse-of options), or which sector behaves opposite to the general default-correlation pattern (energy, largely uncorrelated with the rest of the market). The one item that breaks the pattern you just memorized is exactly the one worth double-checking.</p>`,
  ...
  breakdown: [
    ...
    },
    {
      ...
      points: [
        "Static correlations: measured as a fixed value for a specific time period. Examples are the covariance matrix behind VaR, copula correlations for CDOs, and the binomial default correlation model.",
        "Dynamic correlations: measure how comovement changes over time. Examples are pairs trading, deterministic correlation approaches, and stochastic correlation processes (the subject of Reading 8's empirical deep-dive)."
      ]
    },
    {
      ...
      points: [
        ...
        "Exception: option on the worse-of two assets (payoff = min of the two prices). Here LOWER correlation → LOWER option price, because dispersion raises the chance that at least one asset performs badly, which directly hurts a worse-of payoff.",
    ...
    {
      title: "The May 2005 CDO tranche trade: both legs, in order",
      points: [
        "Setup: hedge funds go long the CDO equity tranche (first-loss piece, roughly the first 3% of pool defaults, highest spread received) and short the CDO mezzanine tranche (the next 4%, from 3% to 7% of pool defaults, lower spread paid), believing the position is correlation-hedged.",
    ...
    },
    {
      ...
      points: [
        ...
        "Default correlation measures whether those names tend to default together, so a lower default correlation means the exposures are more independent.",
  ...
  quiz: [
    ...
    {
      q: "In the pre-crisis CDO tranche trade, hedge funds were long the equity tranche and short the mezzanine tranche. After GM and Ford were downgraded to junk in May 2005, what happened to each leg?",
      options: [
    ...
    },
    {
      ...
      answer: 1,
      why: "An option on the worse-of two assets pays off based on the minimum of the two prices; lower correlation raises dispersion, which raises the chance that at least one asset performs badly, hurting rather than helping a worse-of payoff, so its price falls as correlation falls. Exchange options, quanto options, and basket options all follow the general rule (lower correlation → higher price)."
    },
    {
      ...
      answer: 1,
      why: "At default correlation 1.0, the two $6M loans behave exactly like a single $12M loan, so both default together with the same 5% probability, so EL = 0.05 × $12,000,000 = $600,000. $300,000 (the tempting distractor) mistakenly applies the 5% PD to only one $6M loan instead of the combined $12M exposure; $150,000 and $30,000 come from further arithmetic slips on the notional or the PD."
    },
    {
      ...
      answer: 2,
      why: "With default correlation held at 1.0, the two smaller loans always default together in the worst case, replicating the single large loan exactly, so expected loss is unchanged, as the worked $5M example shows ($250,000 in both the one-loan and correlation-1.0 two-loan cases). A lower concentration ratio only helps if default correlation ALSO falls, and it does not fall automatically just because the loan was split. That is why the answers claiming expected loss falls proportionally, falls to zero, or rises are all wrong."
    }
  ],
