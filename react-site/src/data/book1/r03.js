export default ({
  ...
  teaches: `<p>Everything in Readings 1 and 2 is calibrated to the bulk of the data, the 'normal' 95% of days. But the days that bankrupt institutions are 1-in-1000 events that barely appear in any sample. Extreme Value Theory (EVT) takes a different statistical approach: fit a distribution <strong>only to the extreme observations</strong>, using theorems that tell you what shape tails must converge to no matter what the parent distribution looks like.</p>
  <p>Two frameworks do this: the <strong>Generalized Extreme Value (GEV)</strong> distribution, built from block maxima (the single worst observation per period), and <strong>Peaks-Over-Threshold (POT)</strong>, which models every observation beyond a cutoff u and converges to the Generalized Pareto distribution (GPD). Both share the same tail-index parameter, <strong>\\(\\xi\\)</strong>. This reading closes with multivariate EVT: joint extremes across assets, and the curse of dimensionality that comes with it.</p>`,
  ...
  formulas: [
    { name: "POT VaR (Generalized Pareto)", math: "\\text{VaR} = u + \\dfrac{\\beta}{\\xi} \\times \\left[ \\left(\\dfrac{n}{N_{u}} \\times (1-c)\\right)^{-\\xi} - 1 \\right]",
      plain: "Value at Risk (VaR), the loss you'd need to exceed at confidence c, equals the threshold u plus an adjustment: how many multiples of the scale \\(\\beta\\) you add on top of u, stretched by the shape \\(\\xi\\) and by how much rarer confidence level c is than the threshold-crossing frequency \\(N_u/n\\).",
      derivation: "<p>Example: \\(\\beta = 0.75\\), \\(\\xi = 0.25\\), \\(u = 1\\%\\), \\(N_u/n = 5\\%\\) (so \\(n/N_u = 20\\)), confidence \\(c = 99\\%\\) (so \\(1-c = 0.01\\)).</p><p>Step 1: form the ratio inside the brackets. \\[ \\frac{n}{N_u}\\times(1-c) = 20 \\times 0.01 = 0.2 \\]</p><p>Step 2: raise it to the power \\(-\\xi\\). \\[ 0.2^{-0.25} \\approx 1.495 \\]</p><p>Step 3: subtract 1, then scale by \\(\\beta/\\xi = 0.75/0.25 = 3\\). \\[ 3 \\times (1.495 - 1) \\approx 1.486 \\]</p><p>Step 4: add the threshold. \\[ \\text{VaR} = 1\\% + 1.486\\% \\approx 2.49\\% \\]</p><p>The whole adjustment term is zero only if \\(N_u/n = 1-c\\) exactly: VaR collapses to u only when the confidence level you're testing happens to equal the exceedance frequency you calibrated on. Any tighter confidence pushes VaR above u.</p>",
    ...
    { name: "POT Expected Shortfall", math: "\\text{ES} = \\dfrac{\\text{VaR}}{1-\\xi} + \\dfrac{\\beta - \\xi u}{1-\\xi}",
      plain: "Expected Shortfall (ES), the average loss on the days VaR is breached, is just VaR scaled up by a factor that depends only on \\(\\xi\\), plus a small correction term. You never need to re-run the whole POT VaR calculation to get it.",
      derivation: "<p>Continuing the worked example (\\(\\text{VaR} \\approx 2.486\\%\\), \\(\\beta = 0.75\\), \\(\\xi = 0.25\\), \\(u = 1\\%\\)):</p><p>\\[ \\text{ES} = \\frac{2.486\\%}{1-0.25} + \\frac{0.75\\% - 0.25 \\times 1\\%}{1-0.25} = \\frac{2.486\\%}{0.75} + \\frac{0.5\\%}{0.75} \\approx 3.31\\% + 0.67\\% \\approx 3.98\\% \\]</p><p>Because \\(1-\\xi = 0.75 < 1\\) whenever \\(\\xi>0\\), dividing by it always inflates VaR. That's the algebraic reason ES/VaR \\(>1\\) exactly when the tail is fat (Fréchet). If \\(\\xi=0\\) (Gumbel), the formula is undefined in this exact form, because the underlying GPD becomes the exponential distribution and the ES/VaR gap shrinks toward a fixed constant instead of growing with \\(\\xi\\).</p>",
    ...
    { name: "GEV shape cases", math: "\\xi > 0:\\ \\text{Fréchet}\\qquad \\xi = 0:\\ \\text{Gumbel}\\qquad \\xi < 0:\\ \\text{Weibull}",
      ...
      note: "Fréchet means fat tails (markets). Weibull means bounded tails and is rare in finance." }
  ],
  ...
  concepts: [
    {
      ...
      related: [{ r: 1, label: "R1: standard error explodes where data thins" }]
    },
    {
    ...
    },
    {
      ...
      def: "Model EVERY observation exceeding threshold u. The Gnedenko-Pickands-Balkema-deHaan (GPBdH) theorem: as u grows, excess losses converge to the Generalized Pareto distribution (GPD) with scale \\(\\beta\\) and shape \\(\\xi\\).",
      ...
      example: "Worked mechanics: (1) compute \\((n/N_u) \\times (1-c)\\), the correct orientation the formula uses, (2) raise that ratio to the power \\(-\\xi\\), (3) plug it into the bracket and scale by \\(\\beta/\\xi\\), (4) feed the resulting VaR into the linear ES formula.",
      pitfall: "The threshold trade-off: u too high means few exceedances and noisy parameters, but an accurate approximation. u too low means lots of data, but the GPD approximation breaks because observations aren't genuinely extreme. This trade-off shows up on the exam constantly.",
  ...
  connections: {
    from: [
      ...
      { r: 2, why: "Even the smartest weighted historical simulation is trapped inside the sample's worst day. EVT extrapolates past it." }
    ],
    to: [
  ...
  misconceptions: [
    ...
    { wrong: "\"\\(\\mu\\) and \\(\\sigma\\) in GEV are the mean and standard deviation.\"", right: "They are location and scale parameters, related to but not identical to the mean and standard deviation." }
  ],
  ...
  highYield: [
    ...
    { stars: 5, what: "The threshold u trade-off (bias vs variance).", why: "Tested directly and conceptually; know both directions of the trade-off cold." },
  ...
  recall: [
    ...
    { q: "State the two failure modes of threshold choice in POT.", a: "u too high: too few exceedances, so imprecise parameter estimates (variance). u too low: exceedances aren't genuinely extreme, so the GPD approximation is invalid (bias)." },
  ...
  quiz: [
    ...
    { q: "A test on your return data fails to reject the null hypothesis ξ = 0. What should you conclude and do?",
      ...
      answer: 1,
      why: "This is one of the three explicit decision rules for choosing between ξ = 0 and ξ > 0: if a statistical test can't reject ξ = 0, you use the Gumbel assumption. Defaulting to ξ>0 when in doubt only applies when you have no test result to lean on, and here you do have one, and it says ξ = 0. Weibull is rare in finance and not a default." },
    ...
    { q: "Given the VaR ≈ 2.49% computed above (β = 0.75, ξ = 0.25, u = 1%), what is the correct way to get Expected Shortfall?",
      ...
      answer: 1,
      why: "ES under POT is a simple linear function of VaR: given VaR, β, ξ, u, you compute it directly (≈2.49/0.75 + 0.5/0.75 ≈ 3.98%) without re-running the VaR derivation. Recomputing from scratch wastes exam time and is a trap worth avoiding. Setting ES = VaR contradicts 'ES ≥ VaR' from Reading 1, and the remaining option isn't the formula at all." },
    ...
    { q: "Two markets each independently have a 1-in-100 extreme day. Roughly how often do both markets have a joint extreme day on the same day, and what does this imply for multivariate EVT?",
      ...
      answer: 2,
      why: "For independent 1-in-100 events, joint occurrence is 100×100 = 1-in-10,000, the curse-of-dimensionality arithmetic that makes multivariate EVT hard. Adding frequencies misapplies probability rules for independent joint events; 1-in-1,000,000 is the three-variable case, not two; and assuming correlation rather than independence changes the calculation entirely." }
  ],
