export default ({
  ...
  <p>Along the way you also pick up two "plumbing" questions every practitioner has to answer before using any of these models. What do you do when the model predicts a negative interest rate? And should you calibrate the model to match today's market prices exactly (arbitrage-free), or let it stand on its own assumptions and tell you when the market looks wrong (equilibrium)?</p>`,
  ...
  formulas: [
    ...
    },
    {
      ...
      note: "The annualized volatility \\(\\sigma\\) gets scaled down to the length of one tree step by the square root of time, the same scaling used everywhere in this book (VaR horizon scaling, option volatility, etc.).",
    ...
    },
    {
      ...
      note: "As \\(T\\to\\infty\\), \\(e^{-kT}\\to 0\\) and \\(E[r_T]\\to \\theta\\), so the expected rate decays exponentially from today's rate toward the long-run anchor \\(\\theta\\).",
    ...
    },
    {
      ...
      note: "The number of years it takes to close half the distance between today's rate and \\(\\theta\\). A larger k gives a shorter half-life, so shocks fade faster.",
  ...
  concepts: [
    ...
    },
    {
      ...
      pitfall: "Negative rates are not simply a modeling glitch to be ignored. A small negative rate can be economically rational if the safety/convenience of holding cash is valuable enough to depositors (a real phenomenon seen when European and Japanese policy rates went negative).",
    ...
    },
    {
      ...
      pitfall: "This is a genuine multi-step calculation, not a shortcut. What gets tested is whether you understand WHY averaging + re-solving probabilities is necessary (because the model's variance and mean must still be matched even after forcing recombination), not just that 'you average the middle nodes.'",
  ...
  breakdown: [
    {
      ...
      points: [
        ...
        "Vasicek (mean reversion): dr = k(θ−r)dt + σdw. It actively pulls the rate back toward long-run mean θ at speed k; the only one of the four whose tree does not naturally recombine."
      ]
    },
    {
      ...
      points: [
        ...
        "Only one risk factor (the short rate itself), which implies parallel shifts only, contradicting observed non-parallel curve moves.",
        "Always a positive probability of negative rates, and it gets worse the longer the horizon and the lower the starting rate.",
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: 1,
      why: "Observed volatility term structures are hump-shaped (rise, then fall), which a flat-volatility model like Model 1 cannot reproduce. 'Declining with maturity' is what Vasicek (not the observed market) produces, a common mix-up. Strictly increasing and U-shaped are not the pattern described in the source."
    },
    {
      ...
      answer: 2,
      why: "There are exactly two fixes: switch to a non-negative distribution, or floor negative nodes at zero. Increasing k only applies within Vasicek and reduces (does not eliminate) the chance of extreme moves. It is not one of the two named general-purpose fixes, and it isn't even available in Model 1, where the negative-rate problem is first introduced."
    },
    {
      ...
      options: [
        ...
        "Neither, since relative value analysis requires a historical simulation, not a term structure model"
      ],
      answer: 1,
