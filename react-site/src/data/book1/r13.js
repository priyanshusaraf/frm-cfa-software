export default ({
  ...
  concepts: [
    {
      ...
      example: "\\(r_{0}=6\\)%, \\(\\sigma =1.2\\)%/yr, monthly steps (dt=1/12): SD of monthly change = 1.2%\\(\\times \\sqrt{1/12}\\) = 0.346%. After 2 periods: upper node 6.692%, middle 6% (unchanged — no drift!), lower 5.308%. A single realized \\(dw=0.2\\) after one month produces \\(dr=1.2\\%\\times 0.2=0.24\\)%, so the new rate is \\(6\\%+0.24\\%=6.24\\)%.",
      pitfall: "Four named limitations: (1) flat volatility term structure (real markets show a hump — rising then falling with maturity); (2) only one risk factor (parallel shifts only — contradicts observed non-parallel curve moves); (3) always positive probability of negative rates (worse for longer horizons, lower starting levels — e.g. a 10-year horizon with \\(\\sigma =1.2\\%\\) gives a terminal SD of \\(1.2\\%\\times \\sqrt{10}=3.79\\)%, easily putting a 6% starting rate within two standard deviations of zero); and (4) the no-drift assumption alone forces a downward-sloping predicted par-yield term structure, purely from the convexity effect of Reading 12 (there's no drift term to offset the convexity pull-down).",
    ...
    },
    {
      ...
      example: "(1) Use a distribution that is always non-negative — lognormal or chi-squared — so negative rates are mathematically impossible; the cost is that this can introduce unwanted skewness or distort the model's implied volatilities elsewhere. (2) Keep the original tree but force any negative node to a value of zero ('zero-flooring'); this only changes the small part of the distribution that would have gone negative, leaving the rest of the tree untouched — generally the preferred fix because it's less invasive. Whether either fix even matters depends on the use case: coupon-bond valuation (which depends on the average rate path) is fairly insensitive to a small chance of a brief negative rate, while option models with asymmetric payoffs are much more sensitive to it.",
    ...
    },
    {
      ...
      pitfall: "Better fits typical upward-sloping curves, but calibrated \\(\\lambda\\) can be unrealistically large if interpreted as pure risk premium, and assuming ONE constant risk premium forever is a stretch for long horizons — it's more defensible over shorter windows, where it's easier to argue expected rates keep rising year over year than over decades.",
      related: ["Ho-Lee — time-dependent drift"]
    },
    {
      name: "Ho-Lee model — time-dependent drift",
      def: "\\(\\lambda (t)\\) can differ every period — more flexible, and each \\(\\lambda_t\\) is calibrated against the observed market rate for that maturity.",
      example: "\\(r_0\\) is set to today's observed one-period spot rate. \\(\\lambda_1\\) is then solved so the model's two-period tree rate matches today's observed two-period market rate. \\(\\lambda_2\\) is solved next, using \\(r_0\\) and \\(\\lambda_1\\), so the model's three-period rate matches the observed three-period market rate — and so on outward, one maturity bucket at a time, so the whole calibrated tree reproduces the entire observed curve exactly.",
  ...
  misconceptions: [
    ...
    { wrong: "\"All four drift models produce recombining trees.\"", right: "Model 1, Model 2, and Ho-Lee all recombine. Vasicek does NOT recombine naturally — because its drift adjustment depends on distance from \\(\\theta\\), which differs by path." },
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: 1,
      why: "A constant λ(t) = λ for all t is exactly Model 2's definition — Ho-Lee nests Model 2 as this special case. It does not reduce to Model 1 (that requires λ = 0, not just constant), and it has no relationship to Vasicek's mean-reverting structure."
    },
    {
    ...
    },
    {
      ...
      answer: 2,
      why: "The reading gives exactly two fixes: switch to a non-negative distribution, or floor negative nodes at zero. Increasing k only applies within Vasicek and reduces (does not eliminate) the chance of extreme moves — it's not one of the two named general-purpose fixes, and it isn't even available in Model 1, where the negative-rate problem is first introduced."
    },
    {
