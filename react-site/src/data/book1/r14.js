export default ({
  ...
  breakdown: [
    {
      ...
      points: [
        ...
        "Black-Karasinski, lognormal plus mean reversion: combines Vasicek-style mean reversion (in log-space) with lognormal's time-varying volatility. Like Vasicek, its tree does not naturally recombine and needs the time step recalibrated between periods to force recombination."
      ]
    },
    {
      title: "Basis-point vol vs. yield vol vs. can-go-negative, by model",
      points: [
        "Model 1 / Vasicek (normal): basis-point vol constant, independent of r. Yield vol NOT constant. CAN go negative, which is the flaw the rest of the reading fixes.",
        ...
        "Trap to remember: CIR has NEITHER volatility measure constant; lognormal has ONLY yield vol constant. Neither model has constant basis-point vol, and that is the whole point of making volatility level-dependent."
      ]
    }
  ],
  ...
  formulas: [
    ...
    { name: "Lognormal model", math: "d \\ln(r) = \\lambda (t)dt + \\sigma dw", note: "Constant YIELD volatility \\((\\sigma )\\); basis-point volatility still increases linearly with r \\((=\\sigma r)\\). Cannot go negative.",
      ...
      derivation: "<p>Set \\(a_1=0\\) (no drift for this step) and \\(dt=1\\), so \\(\\ln r_1 = \\ln r_0 + \\sigma\\). Exponentiating both sides: \\[ r_1 = r_0\\, e^{\\sigma} \\] Using the small-\\(x\\) approximation \\(e^{x}\\approx 1+x\\): \\[ r_1 \\approx r_0(1+\\sigma) \\] So volatility acts as a PERCENTAGE of the current rate. Worked example: if \\(\\sigma = 20\\%\\), the up-node rate is \\(r_0\\,e^{0.20}=1.2214\\,r_0\\), which the \\(1+\\sigma\\) approximation rounds to \\(1.20\\,r_0\\). Either way the move is a PERCENTAGE of the current short rate (roughly 20% above it), not 20 percentage points above it. This is exactly the sense in which lognormal drift is \\`\\`multiplicative'' rather than \\`\\`additive'' like Ho-Lee's \\(dr=\\lambda(t)dt+\\sigma dw\\), which adds a fixed amount to \\(r\\) directly regardless of \\(r\\)'s level.</p>"
    }
  ],
  ...
  concepts: [
    {
      ...
      example: "Worked example: \\(r_0=5\\%\\), \\(\\lambda=0.24\\%\\), \\(\\sigma=1.50\\%\\), decay \\(e^{-0.3t}\\), shock \\(dw=0.2\\), one-month step. The rate change works out to \\(\\approx 0.31\\%\\), giving \\(r_1\\approx 5.31\\%\\). That is slightly less than the \\(5.32\\%\\) you'd get with constant (undecayed) volatility, because the exponential decay has already begun shrinking \\(\\sigma\\) after one month. Especially useful for pricing caps/floors, whose value depends critically on forecasted \\(\\sigma(t)\\) at multiple future dates.",
    ...
    },
    {
      ...
      example: "Worked CIR example: \\(r_0=5\\%\\), \\(\\theta=24\\%\\), \\(k=0.04\\), \\(\\sigma=1.50\\%\\), \\(dw=0.2\\), one-month step \\(\\to dr\\approx 0.13\\%\\), \\(r_1\\approx 5.13\\%\\). Worked lognormal example: if \\(\\sigma=20\\%\\), the up-node rate is \\(r_0e^{0.20}=1.2214\\,r_0\\), roughly 20% above the current rate rather than 20 percentage points above it.",
      ...
      pitfall: "Don't confuse 'basis point volatility' (volatility of the level change dr) with 'yield volatility' (volatility of the percentage change). CIR has NEITHER constant: the parameter \\(\\sigma\\) in \\(\\sigma\\sqrt r\\) is a fixed number, but that is not the same thing as yield volatility, which works out to \\(\\sigma/\\sqrt r\\) and so falls as rates rise. The lognormal model, by contrast, has CONSTANT yield volatility even though its basis-point volatility still increases with the rate level. This exact distinction has been directly tested.",
    ...
    },
    {
      ...
      intuition: "Working in log-space is a trick: it lets you reuse the additive machinery (trees, normal-distribution math) you already built for Ho-Lee and Vasicek, then translate the result back to rate-space by exponentiating; exponentiating an additive shock automatically converts it into a percentage (multiplicative) shock on the level.",
      example: "Lognormal with mean reversion is the Black-Karasinski model, which combines mean reversion (Vasicek-style, but in log-space) with time-varying volatility: \\(d\\ln r = k(t)[\\ln \\theta(t) - \\ln r]dt + \\sigma(t)dw\\), so \\(\\ln r\\) reverts to \\(\\ln[\\theta(t)]\\) at speed \\(k(t)\\), with time-varying volatility.",
  ...
  highYield: [
    { stars: 4, what: "CIR vs lognormal: which volatility measure (basis-point vs yield) is constant in each.", why: "The single most precisely tested distinction in this reading, and it has been directly tested before." },
  ...
  hooks: [
    ...
    { title: "Log-space addition, rate-space multiplication", text: "Lognormal's drift adds in log-space but multiplies in rate-space, just as adding exponents multiplies the underlying numbers. Ho-Lee skips the log entirely and adds directly." },
    { title: "The volatility that protects against zero", text: "CIR and lognormal both shrink volatility as the rate approaches zero. It is like a car automatically slowing down as it nears a cliff edge, mechanically prevented from going over (negative)." }
  ],

  summary: `<p><strong>Model 3</strong>: time-decaying volatility (\\(dr=\\lambda(t)dt+\\sigma e^{-\\alpha t}dw\\); useful for caps/floors); can match Vasicek's terminal SD under specific calibration but still implies PARALLEL shifts (different from Vasicek's nonparallel shifts), and matching one statistic is not the same as being the same model. <strong>CIR</strong> \\((dr=k(\\theta - r)dt+\\sigma \\sqrt{r}\\cdot dw)\\): basis-point vol rises with \\(\\sqrt{r}\\) (decreasing rate), yield vol NOT constant, cannot go negative. <strong>Lognormal</strong> (d ln \\(r=\\lambda (t)dt+\\sigma dw)\\): basis-point vol rises linearly with r, yield vol CONSTANT \\((\\sigma )\\), cannot go negative. The CIR/lognormal distinction on WHICH volatility is constant is the most tested fact here. <strong>Additive vs multiplicative drift</strong>: Ho-Lee is additive on the rate; lognormal is additive in log-space, hence multiplicative on the actual rate. Black-Karasinski (lognormal + mean reversion) inherits Vasicek's non-recombining tree problem.</p>`,
  ...
  quiz: [
    ...
    {
      q: "A market maker is deciding whether to use the CIR model instead of a simple constant-volatility model. When might they reasonably stick with the simpler model?",
      options: [
        "Never, since CIR should always be preferred since it prevents negative rates.",
