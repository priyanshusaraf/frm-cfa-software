export default ({
  ...
  thinkLike: `<p>A practitioner reading this chapter isn't memorizing trivia. They're building a mental checklist of "assumptions I am quietly making every time I quote a VaR number." Before presenting a VaR figure to a risk committee, an FRM-minded risk manager asks: What horizon did I use, and does it match this portfolio's actual liquidity (a 10-day convention is meaningless for a book that takes three months to unwind)? Did I hold volatility constant, and if so, am I understating risk in a calm period that is about to turn volatile? Have I priced in the cost of actually exiting these positions (exogenous liquidity), and, if the position is large or exotic, have I separately modeled how much my own selling will move the price against me (endogenous liquidity)? And critically, am I reporting VaR alone, or also expected shortfall, given that VaR can make a diversified, merged portfolio look artificially riskier than its parts?</p><p>Expect this material to show up almost entirely as one-liner recognition and paired-contrast questions: which of these is the drawback of VaR, which liquidity type applies to a large illiquid position, top-down finds X while bottom-up finds Y. The single highest-yield sentence here, worth memorizing verbatim, is that VaR is not subadditive. It's the direct justification for FRTB's later switch (R16) from VaR to expected shortfall.</p>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      related: [{ r: 2, label: "R2: volatility-weighted and filtered historical simulation respond to this" }]
    },
    {
