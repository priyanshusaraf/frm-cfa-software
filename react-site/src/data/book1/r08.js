export default ({
  ...
  <p>The reading is built around one landmark empirical study. Researchers took the 30 common stocks in the Dow Jones Industrial Average (Dow) and, every month from 1972 to 2017 (534 months), computed the full 30×30 pairwise correlation matrix: 900 correlations per month, so 900 × 534 = <strong>480,600</strong> monthly correlation observations in total. They sliced those observations by the state of the U.S. economy, defined using gross domestic product (GDP) growth, and separately fit statistical distributions to the correlation values themselves. Everything you need for the exam, the level/volatility split, the mean-reversion identity, the best-fit distributions, comes directly out of that one dataset. Understanding how it was built makes the results much easier to remember than if you just tried to memorize them as isolated facts.</p>`,
  ...
  <p>Put yourself in the seat of a risk manager who has just computed a correlation matrix for a trading book (for value at risk, or VaR, for a correlation swap, for pricing a basket option). The number you get today is a point estimate of something that systematically shifts with the business cycle, mean-reverts over time in a mathematically predictable way, and is <em>not</em> normally distributed, so confidence intervals built assuming normality will misstate your risk. This reading is the empirical justification for treating "the correlation" as a moving, imperfectly normal target rather than a fixed input.</p>`,
  ...
  formulas: [
    {
      ...
      note: "The stochastic (random-shock) term of the full mean-reversion process is dropped for this LO; only the deterministic pull back toward the mean matters for this learning objective.",
      ...
      derivation: `<p>The full continuous-time mean-reversion process (the type used later for interest rates, e.g. the Vasicek and Cox-Ingersoll-Ross (CIR) models in R13 through R14) has the form \\( dS_t = a(\\mu - S_{t-1})\\,dt + \\text{(stochastic term)} \\). For this reading's purpose, estimating the mean reversion rate empirically, the stochastic term is dropped and \\( \\Delta t = 1 \\) is assumed, leaving:</p>
  ...
  concepts: [
    {
      ...
      example: "Using the Dow 1972 to 2017 study's exact figures: recession gives level 37.0%, volatility 80.5%. Normal gives level 33.0%, volatility 83.0% (the peak). Expansion gives level 27.5%, volatility 71.2% (the trough for both statistics).",
  ...
  highYield: [
    { stars: 3, what: "Level-vs-volatility three-state split (recession 37.0%/80.5%, normal 33.0%/83.0%, expansion 27.5%/71.2%).", why: "The signature counter-intuitive result on this topic, and precisely because it's easy to mis-remember under pressure, it's a favorite exam target." },
  ...
  thinkLike: `<p>A risk manager does not treat "what is the correlation?" as a single-number question. The question is which regime I am in, and how far this month's estimate might wander from that regime's typical level before I recalibrate.</p>
  <p>That means holding two separate mental dials. One is the current LEVEL: which regime (recession, normal or expansion) sets my baseline expectation. The other is the current VOLATILITY of that level: how much noise to expect around the baseline before treating a change as meaningful.</p>
  <p>Take a manager calibrating VaR or stress-testing a correlation-sensitive book (a correlation swap, a basket option, a collateralized debt obligation tranche) during a recession. They should expect correlations to sit high, and to keep moving around a lot while they sit there. Recession correlation volatility is 80.5%, elevated in absolute terms even though it is not the peak.</p>
  <p>The same manager in a "normal" macro environment should expect slightly MORE month-to-month churn than that, 83.0%, even though the average correlation LEVEL is lower. So widen the confidence interval around any single correlation estimate rather than trusting it too literally.</p>
  <p>One question shape puts the obvious wrong answer (recession means both the highest level AND the highest volatility) next to the correct, counter-intuitive pairing (recession is the highest level, normal is the highest volatility).</p>
  <p>Another hands you a regression output, a slope coefficient, and asks you to convert it into a mean reversion rate, an expected next-period value, or a one-period autocorrelation. All three come from the same underlying number via the a + AC = 1 identity.</p>
  <p>A third swaps "normal distribution" in as a distractor on the equity correlation best-fit question, banking on normal being the intuitive default guess for any statistical distribution.</p>`,

  breakdown: [
    {
      title: "Correlation level vs. volatility across the three economic states (Dow, 1972 to 2017)",
      points: [
    ...
    },
    {
      ...
      points: [
        "Equity correlation (480,600 Dow observations, 1972 to 2017): best fit Johnson SB; mean reversion rate ~79% (high, snaps back to the mean quickly); normal, lognormal, and beta are explicit poor fits.",
  ...
  quiz: [
    {
      q: "Based on the 1972 to 2017 study of Dow stock correlations, which statement correctly characterizes correlation levels and correlation volatilities across recessions, normal periods, and expansions?",
      options: [
    ...
    },
    {
      ...
      answer: 1,
      why: "Default probability correlation had the highest correlation volatility of the three correlation types studied, at 88%, despite having a low mean reversion rate (~30%) similar to bonds. Bond correlation volatility (64%) is actually the lowest of the three, which is a common point of confusion since bonds also have the lowest mean reversion rate."
    }
  ],
