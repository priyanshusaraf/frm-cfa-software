export default ({
  ...
  concepts: [
    ...
    },
    {
      ...
      example: "Three named reasons BSM cannot be used for bonds. It assumes no upper bound on the underlying's price, but a bond has a maximum value, reached when interest rates hit zero, at which point a zero-coupon bond is worth exactly its par/face value and a coupon bond is worth the sum of its remaining coupons plus par. It assumes a constant risk-free rate, but bond payoffs are literally driven by changing rates: self-contradictory, since the very thing the model treats as fixed is what's generating the bond's value changes. And it assumes constant volatility, but bond price volatility must fall to zero as the bond approaches maturity (pull-to-par: a bond maturing tomorrow can only be worth its redemption value, so there's essentially no price uncertainty left).",
