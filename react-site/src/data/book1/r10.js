export default ({
  ...
  <p>Concretely, this reading walks through two worked trades a real fixed-income desk runs. The first is a nominal Treasury bond ("T-bond") shorted against a purchase of Treasury Inflation-Protected Securities ("TIPS"), where the T-bond's yield is a <em>nominal</em> yield (includes expected inflation) and the TIPS yield is a <em>real</em> yield (inflation stripped out). The second is an illiquid 20-year euro interest rate swap hedged with a blend of liquid 10-year and 30-year euro swaps. In both cases the naive approach, matching DV01 (dollar value of a one-basis-point move) one-for-one, turns out to be quietly wrong. Regression analysis on historical yield data is the fix.</p>`,
  ...
  formulas: [
    ...
    },
    {
      ...
      <p>Worked example: a trader shorts \\(F_{N} = \\$100\\,\\text{million}\\) of a nominal T-bond and hedges with TIPS. Regression of nominal yield changes on real (TIPS) yield changes gives \\(\\beta = 1.0198\\): the nominal yield moves 1.0198bp for every 1bp move in the real yield. Applying \\(\\beta\\) to the DV01-neutral face amount produces a regression-hedge face amount of approximately \\(\\$82.55\\,\\text{million}\\) of TIPS (versus a somewhat smaller, unadjusted DV01-neutral amount of about \\(\\$80.95\\,\\text{million}\\): \\(\\$82.55\\text{M} / 1.0198 \\approx \\$80.95\\text{M}\\)). Because \\(\\beta\\) was close to 1 in this example, the regression hedge did not move far from the naive DV01-neutral hedge. But the adjustment direction and size are only known once the regression is actually run.</p>`,
    },
    {
  ...
  concepts: [
    ...
    {
      name: "R² and standard error of the regression as hedge-quality readouts",
      ...
      intuition: "A naive DV01-neutral hedge gives you a hedge SIZE but never tells you how RELIABLE that hedge actually is. R² and the standard error of the regression fill that gap. They let a trader estimate the volatility of the hedged portfolio's P&L in advance and decide whether the hedge is good enough to trade, before ever putting the position on.",
      related: ["Regression hedge & hedge adjustment factor β"]
    },
    {
      name: "Two-variable regression hedge",
  ...
  quiz: [
    ...
    {
      q: "A risk manager insists the hedge adjustment factor \\(\\beta\\), once estimated from a five-year regression, can be treated as a fixed constant going forward. What is the correct critique?",
      options: [
        "\\(\\beta\\) is a known-unrealistic simplification; best practice re-estimates it over multiple time windows and compares results rather than trusting one fixed value",
        "\\(\\beta\\) should never be estimated from historical data at all, only from theoretical bond math",
        "The critique is unwarranted: \\(\\beta\\) is provably constant over any horizon",
        "\\(\\beta\\) only needs re-estimation if the two-variable hedge (not the single-variable hedge) is being used"
      ],
      answer: 0,
      why: "The constant-\\(\\beta\\) assumption is a known limitation; best practice re-estimates \\(\\beta\\) over multiple windows and compares results. The 'β is provably constant' choice directly contradicts that. The 'never estimated from historical data' choice misdescribes where \\(\\beta\\) comes from: it IS estimated from historical data, that's the whole method. The 'only needs re-estimation for two-variable hedges' choice wrongly restricts the limitation to only the two-variable case, but it applies to any regression hedge."
    }
  ],
