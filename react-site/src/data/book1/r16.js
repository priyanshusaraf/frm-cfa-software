export default ({
  ...
  <p>This matters beyond the exam because FRTB is not a hypothetical academic proposal. It is the actual rulebook (finalized by the Basel Committee in 2019, with a global implementation deadline banks were working toward through the early-to-mid 2020s) that determines how much capital a bank must hold against its trading book. Get the capital number wrong and a bank is either dangerously under-capitalized (it can't absorb a real loss) or needlessly over-capitalized (it can't lend or trade competitively). Every design choice in this reading exists because the old Basel I / Basel II.5 regime got one of those two outcomes wrong in a specific, identifiable way.</p>`,

  intuition: `<p>The canonical motivating case makes the whole chapter's purpose vivid: a $950M bond portfolio, 2% annual default probability, discretized as 3-in-5 chance of $0 loss, 2-in-5 chance of total ($950M) loss conditional on being in the bad tail. At 95% confidence, VaR = $0, because the default probability is below 5% and the threshold loss is therefore zero. VaR says "no problem." But ES = 40% × $950M = $380M. ES correctly captures that IF you're in the tail, the loss is catastrophic. This gap between a reassuring VaR and an alarming ES is exactly the scenario FRTB was designed to prevent regulators from missing.</p>
  ...
  <p>Under a normal distribution, 99% VaR and 97.5% ES are numerically very close \\((2.326\\sigma\\) vs \\(2.338\\sigma\\) above the mean). They diverge sharply under fat-tailed distributions, which is the whole point of preferring ES. This is also why FRTB didn't need to make banks report a dramatically different number for "normal," well-behaved risk factors. The real bite of the change shows up exactly in the skewed, jump-prone risk factors (like credit and default risk) where VaR's blindness was hiding the most danger.</p>`,

  eli5: `<p>Imagine a smoke detector that only ever tells you "yes, there is smoke right now" or "no, there is no smoke right now." That's VaR: a single yes/no threshold. Now imagine a smoke detector that, once it detects smoke, also estimates how big the fire probably is. That's Expected Shortfall. If your house has a 2% chance per year of catching fire, a "does it happen more than 5% of the time" detector (VaR) will sit there silently saying "all clear" every single year, because a 2% chance never crosses its 5% alarm threshold. Yet when the fire does happen, it burns the house down. The ES-style detector instead asks "if the bad case happens, how bad is it," and reports the full loss it would face. FRTB replaces the yes/no smoke detector (VaR) with the severity-estimating one (ES) as the regulatory standard, precisely because banks (like homeowners) need to know not just "will something bad happen more than 1-in-20 times" but "how much would we lose if it does."</p>`,
  ...
  <p>This reading gets tested in three recurring shapes. The first is a numeric VaR-vs-ES calculation structurally identical to the $950M example, with the portfolio size and default probability swapped so that you have to redo the arithmetic. The second is a "which of these two rules governs X" trap, pitting backtesting against P&L attribution, or 97.5% ES against the 99.9% jump-to-default VaR; the distractor merges two genuinely separate regulatory tests into one. The third asks what \\(ES_{k}\\) holds fixed in the liquidity waterfall, which rewards memorizing the exact nested structure (each successive \\(ES_{k}\\) retires one more category from being re-shocked) rather than a vague sense that "there are five buckets."</p>`,
  ...
  formulas: [
    ...
    },
    {
      ...
      <li>\\(ES_{2}\\): 10-day shock applied to categories 2 to 5 only; category 1 is held fixed (its risk is already captured in \\(ES_1\\)'s shorter horizon).</li>
      <li>\\(ES_{3}\\): 10-day shock applied to categories 3 to 5; categories 1 and 2 held fixed.</li>
      <li>\\(ES_{4}\\): 10-day shock applied to categories 4 to 5; categories 1 to 3 held fixed.</li>
      <li>\\(ES_{5}\\): 10-day shock applied to category 5 only; categories 1 to 4 held fixed.</li>
      </ul>
  ...
  concepts: [
    {
      ...
      intuition: "VaR only checks whether the 5%-worst outcome crosses a threshold; with only a 2% default probability, the 5th-percentile outcome is still a 'no default' scenario, so the threshold itself sits at $0 loss. ES instead averages across everything inside that worst-5% tail. Within that tail, the source's discretized schedule says 2-out-of-5 sub-scenarios are a total $950M loss, so the conditional average comes out to $380M.",
      example: "Change the numbers and the mechanic still applies: an $825M portfolio with a 3% default probability (a common exam variant) still gives 95% VaR = $0 (3% < 5%), but a different conditional tail split (here 3-in-5 sub-scenarios of total loss) gives ES = 3/5 × $825M = $495M. Always redo the arithmetic for the specific numbers given; don't memorize $380M as a universal answer.",
      pitfall: "This is the single clearest illustration in the whole curriculum of why VaR can say 'no problem' while ES screams danger. Memorize the mechanism (default probability below the confidence threshold → VaR reads zero), not just the punchline.",
    ...
    },
    {
      ...
      def: "Five categories (10/20/40/60/120 days) replace the single 10-day horizon applied to everything. The Basel Committee defines a liquidity horizon (LH) as the time required to execute transactions that extinguish an exposure to a risk factor, without moving the price of the hedging instruments, in stressed market conditions. The internal models-based approach (IMA) computes a waterfall of shocks \\(ES_{1}\\) through \\(ES_{5}\\).",
      intuition: "In plain terms, a liquidity horizon is how long it would realistically take to fully hedge or exit a position in a crisis without your own selling pushing the price against you. That is why the categories stretch from 10 days for the most liquid risk factors out to 120 days for the least liquid.",
      example: "Investment-grade sovereign credit spreads are assigned a 20-day horizon; non-investment-grade corporate credit spreads, being far less liquid in a stressed market, get a 60-day horizon. Large-cap equity typically sits at the short (10-day) end. A 120-day horizon corresponds to roughly six months of trading days, 60 days to one quarter, 20 days to one month, and 10 days to about two weeks, so the categories map directly onto how quickly a desk could realistically unwind that type of position under stress. \\(ES_{1}\\) shocks ALL categories together; \\(ES_{2}\\) holds category 1 fixed and shocks 2-5; and so on. The components are then combined, scaled by the square root of the horizon differences (a direct echo of the \\(\\sqrt{t}\\) scaling rule from Reading 1's return-horizon conventions).",
    ...
    },
    {
      ...
      pitfall: "Once classified, reclassification is heavily restricted to extraordinary circumstances only, such as a firm-wide change in accounting practice. The restriction exists specifically to shut down regulatory arbitrage, where banks moved assets between books to get more favorable capital treatment. Even in an allowed reclassification, any capital BENEFIT from the new category is disallowed: the bank must keep computing capital under the stricter of the two methods.",
  ...
  connections: {
    ...
    confused: [
      ...
      { what: "97.5% ES (general market risk) vs 99.9% VaR (jump-to-default)", how: "Two entirely separate measures within FRTB, for two conceptually different risk types (continuous spread risk (ES) vs discrete default risk (VaR)) never merge them into one number." },
  ...
  highYield: [
    ...
    { stars: 4, what: "Backtesting (99%/97.5%, 12/30 exception limits) vs P&L attribution (±10% D/A, 20% variance ratio) as two separate audits.", why: "A precise and easily conflated pair of rules, and the distinction between them is what a question turns on." },
    { stars: 4, what: "97.5% ES (general market risk) vs 99.9% VaR (jump-to-default) as separate, coexisting measures.", why: "The 'don't merge these two frameworks' trap is explicitly flagged and reliably tested." },
  ...
  breakdown: [
    ...
    },
    {
      ...
      points: [
        ...
        "Credit risk and securitizations: the incremental risk charge splits credit spread risk (handled via ES) from jump-to-default risk (its own 99.9% VaR, one-year horizon); securitizations move from bank-specific internal models to a single standardized approach."
      ]
    }
  ],
  ...
  quiz: [
    {
      ...
      answer: 0,
      why: "A 3% default probability is below the 5% tail cutoff at 95% confidence, so the 95th-percentile loss outcome still falls in the 'no default' bucket. VaR = $0, exactly as in the canonical $950M/2% example. ES instead averages across the whole tail: 3-in-5 of the tail's sub-scenarios are a total loss, so ES = 3/5 × $825M = $495M. The tempting distractor, which reports VaR = $495 million and ES = $0, simply swaps which number is zero, forgetting that the threshold-crossing measure (VaR) is the one that reads zero, not ES."
    },
    {
      ...
      answer: 0,
      why: "Basel II.5 bolted a 250-day stressed VaR onto Basel I's existing 10-day 99% VaR, creating a two-measure system. FRTB does not stack a third layer on top. It eliminates that whole VaR-plus-stressed-VaR pairing and replaces it with one measure: 97.5% ES computed over a stressed 250-day window. The choice claiming Basel II.5 keeps its stressed VaR while FRTB adds ES on top is the classic trap: it assumes FRTB is purely additive, when its central design move is a replacement, not an addition."
    },
    {
      ...
      answer: 1,
      why: "The waterfall structure retires one more category from being re-shocked at each step: \\(ES_{1}\\) shocks all five, \\(ES_{2}\\) holds category 1 fixed and shocks 2 to 5, \\(ES_{3}\\) holds 1 to 2 fixed and shocks 3 to 5, \\(ES_{4}\\) holds 1 to 3 fixed and shocks only 4 to 5, and \\(ES_{5}\\) holds 1 to 4 fixed and shocks only category 5. The choice that holds categories 1 through 4 fixed and shocks only category 5 describes \\(ES_{5}\\), not \\(ES_{4}\\). That is a common off-by-one trap when memorizing the waterfall."
    },
    {
    ...
    {
      q: "Under FRTB's treatment of credit risk, why is jump-to-default risk measured using a 99.9% VaR with a one-year horizon rather than the 97.5% Expected Shortfall used for general market risk?",
      options: [
      ...
      answer: 1,
      why: "Credit spread risk moves continuously day to day and fits naturally into the same ES framework used for general market risk. Default, in contrast, is a discrete jump: the issuer either defaults or does not. FRTB therefore keeps it as a separate charge (99.9% VaR, one-year horizon) rather than blending it into the continuous ES calculation. The claim that jump-to-default risk is always smaller in dollar terms is wrong on the facts, since such losses can be severe. The 'irrelevant formality' and 'ES cannot be computed' choices misstate the framework. Both risks are taken seriously, and ES computation is not the obstacle."
    },
    {
      ...
      answer: 1,
      why: "FRTB's trading-book test requires proving more than mere intent: (1) the bank must genuinely be able to trade the asset, and (2) the trading desk must physically manage its risk day to day. The choice resting on intent to trade plus an investment-grade rating relies on exactly the mere intent that pre-FRTB rules allowed and that enabled regulatory arbitrage. FRTB deliberately raised the bar past intent. The 'held to maturity AND amortized cost' choice describes banking-book, not trading-book, treatment."
    }
  ],
