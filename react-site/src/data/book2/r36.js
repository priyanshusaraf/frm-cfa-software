export default ({
  ...
  <p>The metric family is just the list of sensible questions you can ask about that curve. "What is this trade worth on average at that date, counting both directions?" gives <strong>expected mark-to-market (expected MtM)</strong>, which can be negative. "Same date, but only counting the scenarios where they owe me" gives <strong>expected exposure (EE)</strong>, which is never negative and is always at least as large as expected MtM, because the losing half has been dropped rather than averaged in. "Same date, but the bad tail rather than the average" gives <strong>potential future exposure (PFE)</strong>, a high-confidence worst case at that date. "Across the whole life, what is the single worst PFE?" gives <strong>maximum PFE</strong>, one point picked off the PFE curve. "Across the whole life, what is my average EE?" gives <strong>expected positive exposure (EPE)</strong>, the single summary number a risk report quotes.</p>
  <p>EPE is worth pinning down properly, because its name points at the wrong thing and the usual shortcuts get it wrong. <strong>The distinction that matters is the time dimension.</strong> EE is a profile: it gives you a different value at every future date, so it is a curve. EPE collapses that entire curve into one number by averaging it over the life of the trade, weighting each date by its share of the interval. So EE is a function of time and EPE is a scalar. Against PFE the difference is the same one plus the statistic: PFE is a quantile at a single date, EPE is a mean across all of them.</p>
  <p>The word POSITIVE is NOT what separates EPE from PFE, and it does not mean "floored at zero" either, since EE and PFE are both already floored at zero for the same reason (a counterparty defaulting while you owe them costs you nothing). It is there to pair EPE against <strong>expected negative exposure (ENE)</strong>, the identical construction built from the negative side of the distribution. Your negative mark-to-market is your counterparty's exposure to you, so EPE is your side and ENE is theirs. That is the positive-versus-negative the terminology is actually pointing at.</p>
  <p>EPE exists as its own metric for a concrete reason: regulation and pricing both need ONE number, not a curve. For derivatives the exposure at default that goes into a portfolio credit model is not observable the way a loan's is, so it is replaced by EPE scaled by an alpha factor (R38), and the same single figure feeds the credit value adjustment in R37. And <strong>effective EE</strong>, with its average <strong>effective EPE</strong>, is EE forced never to fall as time passes, which exists so that a short trade cannot look safe just because it matures inside a year when in practice it gets rolled into a fresh one.</p>
  ...
  <div class="widget" data-widget="payment-frequency"></div>
  <div class="widget" data-widget="exercise-date"></div>
  ...
  concepts: [
    ...
    },
    {
      ...
      intuition: "Every profile you will ever be asked to draw is these four factors switched on or off, so learn them as switches rather than as a list. Future uncertainty always pushes UP, because the further out you look the wider the distribution of where the market could be. Periodic cash flows always pull DOWN, because each settlement removes a future payment from the trade that is left. Whichever force is winning at a given date decides whether the curve is rising or falling there, and the peak of a swap is simply where they cross. The third factor, combination, means a product with two risk drivers gets two shapes added together rather than blended. The fourth, optionality, adds the value of a choice nobody has made yet, and it drops out the moment the choice is made.",
    ...
    },
    {
      ...
      intuition: "Do not memorise seven pictures. Ask two questions of the contract and the picture follows. First: does principal change hands, and when? If a large notional lands at maturity (foreign exchange forward, cross-currency swap) nothing settles that risk early, so the curve climbs into the end. If principal is at risk from day one (bond, loan, repo) the curve starts high near notional instead of at zero. Second: does anything settle along the way? Regular payments amortize the remaining trade, which is what bends the interest rate swap back down and gives it its hump, and what makes a loan's exposure decay rather than sit flat like a bond's. Options are the exception to both questions: you paid up front and hold only a claim, so exposure is just the option's value and it can never turn into an obligation.",
    ...
    },
    {
      ...
      intuition: "The formula is doing something simpler than it looks. Adding up n trades that all move together gives you n times the risk of one, and that is the no-benefit case the factor compares against. Adding up n trades that move independently gives far less than n times, because the ones going against you partly cancel the ones going for you, which is the ordinary diversification result and is why the factor lands at 1 over the square root of n when correlation is zero. Correlation is the dial between those two worlds. What surprises people is that zero is not the best case: negative correlation is, because trades that reliably move opposite each other cancel deliberately rather than by luck. The only thing stopping you going further is arithmetic, since n trades cannot all be mutually opposed, which is what the floor at minus one over n minus one is saying.",
    ...
    },
    {
      ...
      intuition: "The name hides a simple question: how long are you actually unprotected? Not zero, even under a daily collateral agreement, because the last time collateral genuinely changed hands is earlier than you think. You have to notice the exposure, call for margin, wait out the grace period, decide the counterparty has failed, hedge what you are holding, and only then close it out. The market is moving through all of it. So the MPoR is the window your collateral does not cover, and everything you compute about collateralized exposure is really a statement about that window: exposure over it scales with the square root of its length, which is why shortening it is worth more than it looks and why regulators impose a floor on how short you may assume it is.",
    ...
    },
    {
      ...
      intuition: "These three sit together because they answer one question in sequence: you have posted collateral, so what actually happens to it? Segregation decides whether it is held apart from the receiver's own assets, which decides whether you get it back if they fail. Rehypothecation decides whether they may re-use it in their own trades, which is efficient for them and is exactly what makes it unrecoverable for you when things go wrong. The tension is real, not a technicality: collateral that is segregated is safe for the poster and idle for the system, while collateral that can be re-used is efficient for the system and a credit exposure for the poster. Which is why the same collateral that removes your counterparty risk can hand you a new one, pointing the other way.",
  ...
  connections: {
    ...
    confused: [
      ...
      { what: "PFE vs EPE", how: "The look-alike pair, and the real difference is the TIME dimension, not the sign. PFE is potential FUTURE exposure: a quantile at ONE date. EPE is expected POSITIVE exposure: the time-average of the whole EE curve, so a scalar rather than a profile. Saying the middle word out loud (FUTURE against POSITIVE) keeps the two NAMES apart, but do not mistake it for the concept: POSITIVE is not what separates EPE from PFE, it pairs EPE against expected negative exposure (ENE)." },
  ...
  misconceptions: [
    { wrong: "\"EPE is called expected POSITIVE exposure because, unlike PFE, it cannot be negative.\"", right: "The name is not doing that job. EE and PFE are already floored at zero for the same reason EPE is: a counterparty defaulting while you owe THEM costs you nothing, so the losing outcomes are dropped rather than counted as negatives. What actually separates EPE from PFE is the time dimension and the statistic. EE is a curve with a value at every future date; PFE is a quantile read off one of those dates; EPE is the mean of the whole EE curve over the life of the trade, so it is a single number rather than a profile. The word POSITIVE pairs EPE against expected negative exposure (ENE), the same construction built from the other side of the distribution, which is your counterparty's exposure to you." },
  ...
  hooks: [
    ...
    { title: "Say the middle word", text: "A name mnemonic, not the concept. Potential FUTURE exposure is a tail at one date. Expected POSITIVE exposure is a time-average across the whole trade, and its POSITIVE pairs it against expected negative exposure, not against PFE. The look-alike pair stops looking alike the moment you say the distinguishing word out loud." },
