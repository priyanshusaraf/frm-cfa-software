export default ({
  ...
  <p>Two practical habits follow. First, when you meet a new product, do not reach for a formula. Ask which of the four factors are switched on. Single payout at the end? Exposure climbs. Regular settlement? It gets capped. Two risk factors stacked? Add two shapes. An exercise decision pending? Add its value until the decision is made. That question answers most graph problems without arithmetic. Second, when numbers do appear, they are almost always one of two templates: the netting factor, where the work is knowing the two limits, or the MPoR scaling, which is square-root-of-time volatility multiplied by a z-score. Being able to say in words why the shape or the formula behaves as it does is what turns a slow reconstruction into a fast answer.</p>`,
  ...
  breakdown: [
    {
      ...
      points: [
        ...
        "Potential future exposure (PFE): a high-confidence worst case at one specific future date, the tail equivalent of EE.",
        "Maximum PFE: the highest PFE value over the stated time frame, one point picked off the PFE curve.",
        "Expected positive exposure (EPE): the average of EE through time, the single number used to summarize a trade.",
        "Expected negative exposure (ENE): the exact opposite of EPE, built from negative future values, which is the counterparty's view of the same trade.",
        "Effective EE and effective EPE: EE forced to be non-decreasing, and its average. Both exist to capture rollover risk on transactions under one year."
      ]
    },
    {
