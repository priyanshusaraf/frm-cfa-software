export default ({
  ...
  intuition: `<p>Foreign currency default is the intuitive case. The country cannot print dollars or euros to pay foreign-currency debt, because it does not control that printing press.</p><p>Local currency default is the puzzle. A country CAN print its own currency, so why would it ever default instead? There are three answers.</p><p>A gold standard used to constrain the printing. Before 1971 the currency had to be backed by gold reserves, which capped how much of it could be issued.</p><p>A shared currency strips out independent monetary policy entirely. Greece in its 2015 crisis could not unilaterally print euros to pay its debt, because that decision sits with the European Central Bank rather than with Athens.</p><p>And sometimes the INFLATION cost of debasing the currency is judged worse than defaulting outright. Printing money to pay debt just shifts the pain onto everyone holding that currency: more currency chasing the same goods devalues every unit of it.</p><p>That third reason is really a cost-benefit tradeoff, and one dimension governs it. The more that foreign-currency debt is funding local-currency assets, the worse printing gets, because devaluing the currency crushes the local asset values while the foreign debt itself does not shrink.</p>
  <p>Once you're past that puzzle, the material builds outward: what actually makes one country riskier than another (four broad sources: growth stage, politics, legal system, economic concentration), who measures that risk and how (rating agencies, using a committee process much like corporate ratings), why that measurement is imperfect (herd behavior, slow updates, upward bias), and what two market-based numbers investors watch instead when they don't trust the rating alone (the default spread on the country's bonds, and the CDS spread on insuring against its default).</p>`,
  ...
  formulas: [
    {
      ...
      derivation: `<p>Worked example: on July 1, 2022, a 10-year dollar-denominated Brazilian government bond yielded 6.00%, while the 10-year U.S. Treasury bond yielded 3.02%. Since the Treasury is treated as risk-free (same currency, same maturity, so the only difference left is default risk):</p>
    ...
    },
    {
      ...
      derivation: `<p>Worked example: an investor holds $20 million notional of 5-year Peruvian government bonds and buys 5-year CDS protection on them at a spread of 170 basis points (1.70%):</p>
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "Six levers move a country's default risk, and they split cleanly into what it owes and what it can collect. On the owing side: the level of indebtedness (measured against GDP, and counting obligations to its own citizens, not just foreign lenders) and pension and social-service commitments. On the collecting side: the size of tax receipts and, separately, how stable those receipts are. Then two that cut across both: political risk, and implicit backing from stronger countries or blocs.",
    ...
    },
    {
      ...
      def: "Each of the big three rates more than 100 countries and publishes two ratings per country: a local currency rating and a foreign currency rating. The local one is usually at least as high, because a country can print its own currency but not someone else's. Agencies link them by notching the local rating UP from the foreign one, or the foreign rating DOWN from the local one. A eurozone member, having given up its printing press, sees them converge. S&P rates probability of default; Moody's rates probability AND expected severity. An analyst drafts the rating; a committee of roughly 5 to 10 people then argues the factors (political risk, economic structure, fiscal flexibility, external liquidity) and votes.",
    ...
    },
    {
      ...
      def: "Five recurring failures. Ratings carry an upward bias; agencies herd, so three ratings are less independent than they look; updates lag a developing crisis; agencies then overreact and cut hard, deepening the crisis they are reacting to; and clusters of changes inside one year point at a first rating that was simply wrong. The causes behind that last one are worth naming: government-supplied data of uneven quality, thin analyst resources on a low-revenue product, and revenue that comes from subsovereign ratings instead.",
    ...
    },
    {
      ...
      def: "Two market-based alternatives to a rating, both faster and both noisier. The default spread is the yield gap between a risky sovereign bond and a risk-free bond of the same currency and maturity. The CDS spread is the annual premium paid for protection against that sovereign defaulting. Each updates in real time where a rating updates in months, and each pays for that speed by bundling liquidity and demand effects in with credit risk. CDS spreads move first of the three: in the 2009-2010 European crisis, Greek CDS spreads widened well ahead of any rating action.",
      ...
      counter: "Neither market-based measure has been proven definitively superior to the other for predicting default, CDS spreads are not shown to beat default spreads at forecasting sovereign risk, despite reacting faster to news. And the default spread has a limitation of its own: it is far less meaningful on LOCAL currency bonds, because there is no local risk-free security to measure it against, so a yield gap between two countries may just be a gap in expected inflation rather than in default risk. Even the dollar-denominated version leans entirely on the assumption that U.S. Treasuries are default-risk-free, which is what makes the spread interpretable at all.",
      related: [30],
  ...
  quiz: [
    ...
    {
      q: "Following a sovereign default, which of the following consequence magnitudes is most accurate?",
      options: [
