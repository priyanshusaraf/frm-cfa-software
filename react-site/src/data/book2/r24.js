export default ({
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "Each of the big three rates more than 100 countries and publishes two ratings per country: a local currency rating and a foreign currency rating. The local one is usually at least as high, because a country can print its own currency but not someone else's. Agencies link the two either by notching the local rating UP from the foreign one, or by notching the foreign rating DOWN from the local one. A eurozone member, having given up its own printing press, sees the two converge. S&P rates probability of default; Moody's rates probability AND expected severity. The rating itself is drafted by an analyst and then voted on by a committee of roughly 5 to 10 people.",
