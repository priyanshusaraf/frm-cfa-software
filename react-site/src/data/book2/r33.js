export default ({
  ...
  concepts: [
    ...
    {
      name: "Close-out clauses versus acceleration clauses",
      def: "Two contractual responses to a counterparty going bad, and they differ in how far they go. An acceleration clause makes future payments immediately due when a credit event occurs, a ratings downgrade being the standard trigger, but the contracts stay alive: you are demanding your money sooner, not walking away. A close-out clause goes the whole distance. It terminates every transaction under the agreement, values each one at replacement cost, and nets them into a single amount owed one way. Close-out is the mechanism that makes close-out netting work at all, and it is the reason a master agreement is one contract rather than a folder of separate trades.",
      intuition: "Think about what each does to your position at the moment a counterparty starts to look shaky. Acceleration pulls the timing forward: the obligations still exist, still reference the same trades, you simply get paid now instead of later. That is useful when the worry is that the counterparty will not survive to the payment dates, but you remain their counterparty afterwards. Close-out severs the relationship: nothing survives, every trade collapses into one net settlement figure, and you are free to replace the hedges elsewhere. That is why the two are not interchangeable answers even though both are triggered by deteriorating credit.",
      example: "A downgrade trips a clause in your agreement. Under acceleration, the swap payments scheduled over the next three years become payable at once, but the swap itself is still on your books and the counterparty is still your counterparty. Under close-out, the swap and everything else under the master agreement terminate, each is marked at replacement value, the positives and negatives net down to one number, and one party pays the other that single amount.",
      pitfall: "The exam trap is treating acceleration as a milder close-out. It is not a weaker version of the same action but a different action altogether: acceleration changes WHEN you are paid and leaves the contracts standing, close-out ENDS the contracts. Only close-out produces the single net figure that close-out netting depends on, so a question asking which mechanism delivers netted settlement on default is asking about close-out, never acceleration.",
      related: [{ r: 32, label: "R32: close-out netting as a counterparty-risk mitigant" }],
      memory: "Acceleration moves the clock forward. Close-out ends the game and settles one number."
    },
    {
      ...
      pitfall: "Break clauses are theoretically valuable but have never been popular in practice, for two reasons. A break clause is really just a discrete (point-in-time) form of collateralization, and continuous collateral posting achieves the same protection more smoothly. Then there is the banker's paradox: for a break clause to actually protect you, you'd need to exercise it EARLY, before the counterparty's credit has visibly deteriorated; but firms are reluctant to exercise early because doing so damages the relationship and can itself trigger the very distress they're trying to avoid, so in practice break clauses tend to get exercised too late (if at all) to be useful.",
      related: [{ r: 32, label: "R32's toolkit list previews termination features as one exposure-mitigation lever" }],
    ...
    },
    {
      ...
      related: [{ r: 35, label: "R35 formalizes compression vs. multilateral offset as two distinct paths to a leaner derivatives book" }],
  ...
  breakdown: [
    ...
    {
      title: "Netting's advantages (why firms want it): four reasons",
      points: [
