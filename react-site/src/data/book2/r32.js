export default ({
  ...
  thinkLike: `<p>A counterparty risk manager's job starts with one habit worth drilling. For every OTC trade, ask which of us could end up owing the other, and whether that answer changes if the market moves. Not who owes whom right now.</p>
  <p>Keeping that question live is most of the work. It is also exactly what a question probes when it describes a transaction and asks whether the risk is bilateral or unilateral, or whether a specific mitigant (walkaway, netting, collateral) actually helps in the MtM scenario given. Recall the Ondine and Scarbo case: the right mitigant depends entirely on whether your own MtM is currently positive or negative.</p>
  <p>The second habit is to treat CVA and credit limits as two independent gates a trade must clear, rather than one combined check.</p>
  <p>They come apart in practice because the two desks want opposite things. A CVA desk wants fewer, larger counterparty relationships, because netting works better that way. Credit officers want more, smaller ones, because that is what diversification means to them.</p>
  <p>So when a question gives a trade's status against the CVA charge and separately against the counterparty's credit limit, expect the answer to turn on those being genuinely separate constraints, with a trade that clears one and fails the other.</p>`,
  breakdown: [
    {
