export default ({
  ...
  thinkLike: `<p>A counterparty risk manager's job starts with a mental habit worth drilling: for every OTC trade, ask "which of us could end up owing the other, and does that answer change if the market moves?", not "who owes whom right now?" Keeping that question live is most of the work, and it is exactly what a question probes when it describes a transaction and asks whether the risk is bilateral or unilateral, or whether a specific mitigant (walkaway, netting, collateral) actually helps in the SPECIFIC MtM scenario given (recall the Ondine/Scarbo question: the "right" mitigant depends entirely on whether your own MtM is currently positive or negative).</p><p>The second habit: treat CVA and credit limits as two independent gates a trade must clear, not one combined check, GARP loves scenario questions where a trade clears one gate and fails the other, precisely because in practice trading desks and credit risk committees really do run these as separate, sometimes conflicting, processes (CVA desks want fewer, bigger counterparty relationships for netting efficiency; credit officers want more, smaller ones for diversification). When you see a question describing a trade's status against "the CVA charge" and separately against "the counterparty's credit limit," expect the answer to hinge on treating these as genuinely separate constraints.</p>`,
