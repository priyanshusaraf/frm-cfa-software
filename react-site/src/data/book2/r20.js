export default ({
  ...
  thinkLike: `<p>A credit portfolio manager does not ask "will this loan default?" as a yes or no. They ask how much of the loan's risk is idiosyncratic, this borrower's own bad luck, and how much is systematic, something that would hit the rest of the book at the same time.</p>
  <p>That split is the whole framework. In a large, well-diversified portfolio the firm-specific piece washes out: good luck on one loan offsets bad luck on another.</p>
  <p>The residual does not. The piece correlated with the rest of the book survives no matter how many more loans you add, which is why adding loans stops helping at some point.</p>
  <p>So the job in capital allocation is pricing and limiting that undiversifiable residual, not the gross unexpected loss of any single loan read in isolation.</p>
  <p>Three question shapes follow. Pure calculation, plugging into EL = EA×PD×LR and UL = EA√[PD·σ²LR + LR²·σ²PD], where the step people miss is that σ²PD = PD(1−PD) under the binomial assumption.</p>
  <p>Directional traps, where PD and the recovery rate both change and you must sign the net effect on EL. A PD decrease and an RR increase both lower EL, so they reinforce rather than cancel, which is the opposite of what the question is inviting you to assume.</p>
  <p>And the qualitative one: what happens to portfolio UL as correlation falls. UL_P is below the sum of the individual ULs whenever ρ is under 1, and equals it only in the unrealistic case where every pairwise correlation is exactly 1.</p>`,
  formulas: [
    ...
    { name: "Unexpected loss (single asset)", math: "UL = EA \\times \\sqrt{PD\\cdot \\sigma_{LR}^{2} + LR^{2}\\cdot \\sigma_{PD}^{2}}", note: "\\(\\sigma_{PD}^{2}\\) = PD(1−PD) under a binomial default assumption.",
      ...
      derivation: `<p>UL is simply the standard deviation of the same terminal loan value / loss variable used to derive EL. This is nothing more elaborate than applying the ordinary definition of standard deviation to the horizon-date value of the risky asset. Starting from \\(Var(L) = E[L^2] - (E[L])^2\\) and treating \\(PD\\) and \\(LR\\) as independent random variables (each with its own mean and variance, \\(\\sigma_{PD}^{2}\\) and \\(\\sigma_{LR}^{2}\\)), algebra reduces the variance of the loss to:</p>
  ...
  concepts: [
    ...
    },
    {
      ...
      example: "A 20-asset portfolio has 190 unique correlation pairs, computed as \\(n(n-1)/2\\); a 100-asset portfolio has 4,950, exactly why practitioners collapse pairwise correlation into one representative number instead of estimating each pair. In the worked example, a two-loan portfolio's UL_P falls as the assumed pairwise correlation drops, moving from \\(\\rho=0.3\\) to \\(\\rho=0.1\\) shrinks UL_P to $346,118, even though portfolio EL is completely unchanged (correlation has zero effect on EL_P, since expected loss simply sums regardless of how the two loans co-move).",
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        "EL_P stays exactly the same; UL_P falls (to $346,118 in the worked example)",
