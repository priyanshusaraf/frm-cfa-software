export default ({
  ...
  teaches: `<p>This reading makes that one sentence precise before the book gets quantitative. <strong>Credit risk</strong> is defined as the probability that one party (a creditor) loses money because a counterparty fails to honor a financial obligation. That failure can happen for three different reasons: an <strong>inability</strong> to repay (the money genuinely isn't there), an <strong>unwillingness</strong> to repay (a dispute over whether the obligation is even valid, e.g., a sovereign government contesting a debt), or <strong>nontimeliness</strong> (the money eventually arrives, but late, which still costs the creditor lost interest income).</p>
<p>It then draws three careful legal/financial distinctions (insolvency vs. default vs. bankruptcy), catalogs the seven transaction types that create credit risk, and maps which kinds of entities carry which kinds of exposure, using real institutional examples (JPMorgan's derivatives book, Melvin Capital's GameStop short, Swiss Re/Munich Re reinsurance) rather than abstract categories. Nothing here is hard math. The exam risk is sloppy vocabulary applied to a concrete scenario.</p>`,
...
<p>It also gives you the three-part checklist every credit officer runs before extending exposure: (1) the amount of credit risk, (2) the probability of counterparty default, and (3) the recovery amount and timing of payment receipt. That sequence reappears, formalized, as PD, LGD, and EAD once the book turns quantitative in R20.</p>`,

  intuition: `<p>Think of insolvency, default, and bankruptcy as three separate clocks that don't have to tick together. <strong>Insolvency</strong> is a balance-sheet STATE (liabilities > assets, i.e., negative equity). You can be insolvent and still pay every bill on time, because insolvency is about what you owe versus what you own, not about whether cash is moving on schedule. <strong>Default</strong> is a contractual EVENT (you failed to meet an obligation). It can happen even without insolvency, e.g., a company with plenty of assets simply refuses to pay because it disputes the validity of the contract.</p>
<p><strong>Bankruptcy</strong> is a legal PROCEEDING, typically triggered after default, in which a court steps in to negotiate among the entity's management, creditors, and other stakeholders. The court resolves the situation via liquidation (Chapter 7 in the U.S.: the company is wound down and its assets sold off) or reorganization (Chapter 11: the company keeps operating under a court-supervised restructuring of its debts). A firm can sit in the "insolvent" box for years without ever crossing into default or bankruptcy, and a firm can default (say, on a single missed coupon due to a genuine cash-timing dispute) while remaining perfectly solvent on paper.</p>`,

  eli5: `<p>Imagine you lend your neighbor your lawnmower every weekend and he promises to give it back by Sunday night. <strong>Insolvency</strong> is like finding out your neighbor actually owes more money to the bank than his house and car are worth combined. He's technically "underwater," but he might still hand your lawnmower back right on time every single Sunday, because being underwater on his overall finances doesn't automatically mean he misses this one specific promise to you.</p>
<p><strong>Default</strong> is the moment he actually doesn't return the mower on time. Maybe he genuinely can't (it broke), or maybe he's flatly refusing (he thinks you promised to fix it first and he's not giving it back until you do). Either way, the specific promise got broken. <strong>Bankruptcy</strong> is what happens if his money troubles get so bad that a judge has to step in and referee between him and everyone he owes, deciding whether he sells everything off (liquidation) or gets a structured payment plan while keeping his stuff (reorganization). In finance terms: insolvency is the balance-sheet condition, default is the broken promise, and bankruptcy is the court process. None of the three automatically implies either of the others.</p>`,

  thinkLike: `<p>A credit officer evaluating any new exposure (a loan, a lease, a derivatives line, a trade credit arrangement) runs through the same three-question checklist every time, regardless of the instrument: how much am I exposed for, what's the probability the counterparty doesn't pay, and if they don't pay, how much do I get back and how long does it take? That three-part frame (amount, probability, recovery) is the seed of everything quantitative that follows in this book. It becomes exposure at default (EAD), probability of default (PD), and loss given default (LGD, the complement of the recovery rate) once the math shows up in R20.</p>
<p>The second habit a good risk manager builds here is refusing to assume "credit risk" means "loans." A treasury desk placing corporate cash in a bank deposit, a company prepaying a supplier, an insurer holding a reinsurance recoverable on its books for a decade: all of these are credit exposures, and missing one because it doesn't look like a loan is a classic blind spot. The exam tests this by handing you a scenario (a balance sheet, a repo, a receivables question) and asking you to classify what is happening using precise vocabulary. Is the entity insolvent, defaulted, or bankrupt? Which of several plausible-sounding "credit risk mitigants" would a corporation actually use for receivables? (Insurance, factoring, and documentary credit; not derivatives, which hedge market risk rather than counterparty nonpayment on the receivable itself.)</p>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "Insolvency: liabilities > assets (negative equity), while the firm may still be paying every bill on time. Default: fails to meet a contractual obligation, due to inability OR unwillingness to pay (can happen without insolvency, e.g., a dispute). Bankruptcy: a legal proceeding, typically triggered after default, in which a court negotiates among the entity's management, creditors, and other stakeholders. It is resolved via Chapter 7 (liquidation/dissolution) or Chapter 11 (reorganization/restructuring).",
      pitfall: "A company with liabilities exceeding assets that is STILL PAYING everyone on time is insolvent, not in default and not bankrupt. The exam loves handing you a balance sheet with negative equity and a confidently-paying management team, and the answer is 'insolvent,' full stop. Example from the source: Acquaria Corporation has $280M in assets and $320M in debt, but management expects to keep meeting payment obligations. That is insolvency, not default or bankruptcy, because insolvency describes the balance sheet, and default/bankruptcy have not (yet) occurred.",
  ...
  recall: [
    ...
    { q: "Name the three ways a corporation can mitigate its accounts-receivable credit risk, .", a: "Buying insurance on the receivables, selling the receivables to another company (factoring), and securing foreign transactions through documentary credit (a bank guarantee of payment). Derivatives are not on this list, they hedge market risk, not a customer's nonpayment risk." },
    { q: "What are the three things a creditor generally wants to assess about any credit exposure?", a: "(1) The amount of credit risk, (2) the probability of counterparty default, and (3) the recovery amount and timing of payment receipt, the conceptual precursor to EAD, PD, and LGD." }
  ],
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: 2,
      why: "There are exactly three receivables-risk mitigants: insurance, factoring, and documentary credit. Derivatives hedge market risk (e.g., commodity price moves), not a customer's failure to pay an outstanding invoice, so they are not a receivables-risk tool here."
    },
    {
