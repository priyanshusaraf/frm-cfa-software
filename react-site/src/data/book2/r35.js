...
<p>Three question shapes recur here. (1) Waterfall position: "which resource is drawn on FIRST", or "which layer absorbs a loss of this size", which is the same knowledge as knowing the order but asked one layer at a time. (2) Precise definitional pairs: initial margin vs. variation margin, compression vs. netting, tear-up versus forced allocation, VMGH versus auctions, where the trap is conflating two mechanisms that sound similar but differ in what triggers them and who bears the cost. (3) "Which of these is, or is not, an advantage of central clearing", where the answer that tempts is almost always the overclaim that central clearing "eliminates" counterparty risk (it only reduces and concentrates it).</p>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "The five things a CCP can do once a member has actually defaulted, roughly in the order it reaches for them. It first macro-hedges the defaulter's book so the position stops bleeding, then auctions the hedged sub-portfolios to surviving members, who have every reason to bid seriously because a failed auction pushes the loss further down the waterfall toward their own money. If that is not enough, it pays only part of the variation margin gains it owes other members (variation margin gains haircutting, on the logic that the defaulter's losses are somebody else's gains), tears up unmatched contracts to restore a matched book, or forcibly allocates specific positions to specific members.",
