export default ({
  ...
  visual: `<div class="widget" data-widget="party-flow" data-flow='{"title":"Securitization structure: pool to SPV to tranched notes","parties":[{"id":"orig","label":"Originator","x":0.12,"y":0.5},{"id":"spv","label":"Issuer (SPV)","x":0.5,"y":0.5,"kind":"accent"},{"id":"inv","label":"Investors","x":0.88,"y":0.5},{"id":"trustee","label":"Trustee / servicer","x":0.5,"y":0.12,"kind":"muted"},{"id":"rating","label":"Rating agency","x":0.5,"y":0.88,"kind":"muted"}],"flows":[{"from":"orig","to":"spv","label":"sells assets"},{"from":"spv","to":"orig","label":"cash","dashed":true},{"from":"spv","to":"inv","label":"tranched notes"},{"from":"inv","to":"spv","label":"cash","dashed":true},{"from":"trustee","to":"spv","label":"oversight"},{"from":"rating","to":"spv","label":"rates notes"}],"caption":"The originator sells its loan pool to a bankruptcy-remote SPV (the true sale), which funds the purchase by issuing tranched notes to investors. The trustee and servicer administer the cash flows, and a rating agency rates the notes. The SPV exists so the assets are legally separated from the originator, so investors are exposed to the pool, not to the originator own credit."}'></div>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      example: "A useful analogy: a revolving structure resembles a corporate revolving line of credit from a bank, the corporation doesn't amortize the balance down gradually; it's required to pay the line down to zero periodically in a lump sum, then can draw on it again. Credit card ABS work the same way structurally.",
      related: []
    },
    {
  ...
  breakdown: [
    ...
    },
    {
      ...
      points: [
        ...
        "Subordinating note classes: junior (class B) notes don't receive principal until senior (class A) notes are fully redeemed or rating-agency performance tests are met, and the collateral pool has to pass those tests over a period of time before any principal flows to the subordinate notes.",
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: 1,
      why: "DSCR = NOI / total debt payments = $89,572,500 / $87,958,000 ≈ 1.02. A DSCR above 1.0 does mean the pool covers its debt service, but 1.02 is explicitly flagged in the source as 'a very low DSCR for mortgages' since typical residential pools run 2.5 to 3.0, so the 'comfortably high, typical of residential' characterization is the trap; the DSCR ≈ 0.98 answer gets the direction of coverage wrong."
    },
    {
