...
<p>You need to know: (1) the seven parameters every CSA defines, threshold, initial margin, minimum transfer amount, rounding, haircut, credit quality linkage, and credit support amount; (2) the mechanics, who calculates the numbers (the valuation agent), what can be posted (collateral types), and how disputes over those numbers get resolved; (3) the structural choice between a one-way CSA (only one side ever posts) and a two-way CSA (both sides post, but possibly on different terms); (4) collateral's darker side, rehypothecation and segregation, and the six distinct risks (market, operational, liquidity/liquidation, funding liquidity, default, FX) that collateralization itself introduces even as it reduces credit exposure; and (5) the post-2011 G20/regulatory push for mandatory bilateral variation and initial margin on non-cleared OTC derivatives.</p>`,
  ...
  concepts: [
    ...
    {
      name: "Overcollateralization (and its mirror, undercollateralization)",
      def: "Collateral almost never lands exactly on the exposure. Overcollateralization is the state where the collateral one party holds EXCEEDS what it is owed; undercollateralization is the opposite, where a real uncollateralized exposure is left standing. Neither is an accident or an error. Both are produced on purpose by the CSA parameters: initial margin pushes the balance deliberately into overcollateralization, while threshold, minimum transfer amount and rounding deliberately leave small amounts uncovered so that collateral is not called over trivial moves.",
      intuition: "Follow the two parameters that pull in opposite directions. A threshold is an agreed amount of exposure that does not have to be collateralized at all, so any exposure below it sits uncovered: undercollateralization by design. Initial margin is collateral posted up front regardless of whether anyone is currently exposed, so it overcollateralizes the receiving party from day one. Run both at once and they fight, which is exactly why a CSA using meaningful initial margin normally sets the threshold to zero. Minimum transfer amount and rounding then add a little more slack at the edges, because calling for a trivial amount of collateral costs more in operational effort than the risk it removes.",
      counter: "Here is the part that is easy to miss and is the reason this matters at all: overcollateralization does not remove risk, it REVERSES the direction of it. If you have posted more collateral than you owe, the excess is your asset sitting in someone else's hands. Should THEY fail, you are now an exposed creditor for that surplus, and you are exposed to a party you may have thought of as the risky one. The exposure did not disappear, it changed sides. That is precisely why segregation and limits on rehypothecation exist: they decide whether the excess you posted is still recoverably yours if the holder goes under.",
      pitfall: "Do not read overcollateralization as simply safer. It is safer for the party HOLDING the collateral and creates a new exposure for the party that POSTED it. Also keep this sense of the word separate from the securitization one, where overcollateralization means a pool whose collateral exceeds the claims written against it (101 mortgages backing 100 mortgages of bonds) as an internal credit enhancement. Same word, different mechanism, different reading.",
      related: [{ r: 35, label: "R35: initial margin at a CCP, held against the same problem" }],
      memory: "Threshold leaves a gap under; initial margin pushes it over. Over does not mean safe, it means the exposure switched sides, which is why segregation matters."
    },
    {
...
<p>The exam tests this by making you trace numbers through the CSA machinery: given an exposure, a threshold, and possibly an initial margin, compute the credit support amount and say whether margin gets called or posted. It also loves scenario questions that hinge on relative credit quality (who negotiates a one-way CSA, and in whose favor?) and on naming which of the six collateral-related risks (market, operational, liquidity/liquidation, funding liquidity, default, FX) a described situation illustrates, so practice mapping a short story ("the bond posted as collateral just got downgraded below the CSA's eligibility criteria") to the correct risk category (default risk) rather than a vaguer one (market risk).</p>`,
  ...
  quiz: [
    ...
    {
      q: "Which pair of CSA parameters are additive, meaning exposure must exceed the SUM of both before any collateral actually moves?",
      options: [
