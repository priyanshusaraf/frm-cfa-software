export default ({
  ...
  thinkLike: `<p>A CVA desk trader treats every new derivative not as "priced at its textbook value" but as "priced at its textbook value minus what it costs to insure against this specific counterparty not paying." Before agreeing to a trade, the desk asks: what is the ΔEE (the extra expected exposure) this trade adds to everything else already on the books with this counterparty, given the netting agreement in place? A large standalone CVA on a trade that happens to offset existing exposures can be nearly free to add; a small standalone CVA on a trade that compounds existing exposure can be expensive. This is why incremental CVA, not standalone CVA, drives the actual pricing decision, and it's exactly the kind of "which number do you use, and why" distinction a question probes, usually by describing a scenario (new trade vs. attribution of an existing book) and asking which metric applies.</p>
  ...
  concepts: [
    {
      ...
      def: "CVA (credit value adjustment) is the expected cost of counterparty credit risk, expressed as a dollar (or spread) reduction from a derivative's risk-free value: risky value = risk-free value − CVA. Unlike a bond or loan, a bilateral derivative (e.g., an interest rate swap with fixed and floating legs) can owe money in either direction depending on where the market moves, so pricing its counterparty risk is harder than pricing a one-way instrument. One naming trap: the source prints CVA as both \"credit value adjustment\" and \"credit valuation adjustment\". They are the same thing. Do not go hunting for a difference.",
  ...
  highYield: [
    ...
    { stars: 4, what: "Wrong-way risk: definition, classic scenarios (2007-09 monolines, Lehman FX swap), and the higher-credit-quality-amplifies-WWR counterintuitive fact.", why: "A rich conceptual area that lends itself to scenario stems, so it comes up often." },
    ...
    { stars: 3, what: "The four WWR modeling approaches and each one's chief weakness.", why: "A clean enumerable list: a stem can describe an approach and ask its chief weakness or best use case." },
  ...
  quiz: [
    {
      q: "A swap has an expected positive exposure (EPE) of 6% and the counterparty's credit spread is 375bps. Using the quick running-spread approximation, what is the CVA spread?",
