export default ({
  ...
  intuition: `<p>Retail lenders have an advantage corporate lenders often lack: they can act preemptively. They can tighten underwriting standards or raise cutoff scores before a downturn fully arrives.</p>
  <p>What buys them that option is granularity. Every new loan is a fresh decision point, and a bank underwriting thousands of new mortgages a month can simply say "no" more often starting tomorrow.</p>
  <p>Corporate lenders are usually locked into large exposures already on the books. A $500 million syndicated loan to one borrower cannot be tightened after the fact the way next month's mortgage originations can, so the warning signs arrive too late to act on.</p>
  <p>But the advantage evaporates in the "dark side" scenario. A systemic shock such as a housing crash hits asset VALUES and DEFAULT RATES at the same time: collateral values fall exactly as borrowers default more.</p>
  <p>No amount of granularity protects against that double blow. Granularity diversifies away idiosyncratic risk, the risk that one specific borrower stops paying for reasons unique to them. It does nothing against systematic risk, a factor that pushes every borrower in the pool toward default at once.</p>
  <p>The 2007-08 subprime crisis is the textbook case. In the run-up, lenders offered products borrowers could not really afford: high loan-to-value mortgages to borrowers with weak credit.</p>
  <p>So when home prices fell, loan balances exceeded property values for a huge swath of the pool simultaneously, and defaults spiked at exactly the moment recovery values collapsed.</p>`,

  formulas: [
    {
      name: "Implied probability of default from loss rate and LGD",
      math: "\\text{PD} = \\dfrac{\\text{Loss rate}}{\\text{LGD}}",
      note: "Example: a portfolio subgroup with a 3% loss rate and 75% LGD implies PD = 3%/75% = 4%.",
  ...
  concepts: [
    {
      ...
      def: "Retail banking takes deposits from and lends to consumers and small businesses (home mortgages, home equity lines of credit, installment loans such as auto loans and credit cards, and small business loans), so credit risk is its biggest exposure but nowhere near its only one. Five others sit alongside it: operational risk, business risk (including volume risk, since mortgage origination swings with rates), reputation risk, interest rate risk, and asset valuation risk on collateral such as a leased car's assumed residual value.",
    ...
    },
    {
      ...
      pitfall: "A pooled model is NOT the same as a credit bureau (FICO) score, only credit bureau scores run on the standardized 300 to 850 scale; pooled and custom models use their own internal scales built from their own data.",
      related: []
    },
    {
    ...
    },
    {
      ...
      def: "Two linked tools for asking whether a scorecard actually separates the customers who default from the ones who do not. The cumulative accuracy profile (CAP) is the chart: it plots the cumulative share of the risk-ranked population against the cumulative share of defaults captured, against a perfect-model line and a random 45-degree line. The accuracy ratio (AR) is that chart collapsed to one number, the observed curve's lift over random as a fraction of the perfect model's lift, so closer to 1 is better.",
      ...
      example: "A scorecard is never a 'set it and forget it' tool: a bank must monitor scorecard performance on a regular, ongoing basis, because the underlying applicant population and the lender's product mix both keep shifting, a scorecard trained on last decade's applicants can silently lose discriminatory power as the pool it now sees changes underneath it.",
      related: []
    },
    {
  ...
  hooks: [
    { title: "The double blow", text: "The dark side isn't one bad thing. It is two bad things at once: home prices fall (less recovery) AND defaults rise (more losses) simultaneously. That simultaneity is what makes 'small loans' add up to a systemic crisis." },
