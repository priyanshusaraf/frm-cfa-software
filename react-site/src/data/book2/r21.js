export default ({
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "Three ways to predict default, distinguished by what evidence they lean on: judgmental models lean on expert human assessment (the 5C framework), empirical models lean on statistical patterns mined from historical loan data, and financial/market models lean on economic theory plus live market prices. Only the third family is restricted to corporate borrowers, since it needs traded securities to read prices off.",
    ...
    },
    {
      ...
      def: "Three models that inherit Merton's question (how likely is this firm to default?) while each dropping a different piece of his machinery. Moody's-KMV keeps the structural frame but replaces the normal distribution with a historical one and moves the default point to short-term debt plus half of long-term debt. CreditMetrics drops firm-level structure for rating-migration statistics on comparable peers. CreditRisk+ drops capital structure entirely and treats default as a rare Poisson event.",
    ...
    {
      name: "RAROC",
      def: "Risk-adjusted return on capital = loan revenue / capital at risk. Loan revenue = loan value × (spread s + fees f − expected losses l − operating costs c) × (1 − tax rate x). Capital at risk can be estimated two ways: via duration, \\(\\Delta L = -\\dfrac{D\\times L\\times \\Delta i}{1+i}\\) (the estimated drop in the loan's market value if rates rise by \\(\\Delta i\\)), or via unexpected loss, \\(\\alpha \\times LGD\\times EAD\\) \\((\\alpha \\approx 2.6\\sigma\\) at 99.5% under a normal assumption, but 5 to 6\\(\\sigma\\) in practice since loan-loss distributions are skewed, not normal).",
      example: "Worked example: a $1,000,000 loan carries a 0.1% commission, a 0.65% spread over the bank's cost of capital, 0.2% operating costs, and an 11.1% tax rate, loan revenue = $1,000,000 × (0.0065 + 0.001 − 0.002) × (1 − 0.111) = $4,889.50. The loan has duration 3.75, current rate 10.5%, and rates are expected to rise 1.5%: capital at risk = 3.75 × $1,000,000 × 0.015 / 1.105 ≈ $50,905. RAROC = $4,889.50 / $50,905 ≈ 9.61%. The loan is profitable as long as the bank's own cost of capital is below 9.61%.",
  ...
  thinkLike: `<p>A bank credit officer sizing up a single corporate loan is answering two separate questions in sequence, and each of them gets tested on its own.</p><p>The first is regulatory: how much capital am I legally required to hold against this specific loan? That is the IRB formula, and a good risk manager holds its comparative statics as reflexes rather than lookups.</p><p>A riskier, higher-PD borrower takes a LOWER asset-correlation input, because its trouble is usually company-specific.</p><p>A bigger borrower takes a HIGHER one, because its fate tracks the whole economy.</p><p>And a long-maturity loan to a currently-safe borrower needs MORE capital, not less, because that borrower has more room to fall before the loan matures.</p><p>The second question is commercial: even after holding that capital, does this loan make the bank money? That is RAROC, and the mental model is short. Compute the after-tax margin the loan throws off, divide by whatever capital the first question said you must set aside, and compare the resulting percentage to the bank's OWN cost of capital rather than to some external benchmark.</p><p>The distinction that gets tested here is between "profitable" and "RAROC is merely positive". A 9% RAROC against an 11% cost of capital is a loan that should never be booked, even though the raw number looks fine in isolation.</p><p>Underneath both questions sits Merton. A risk manager reads a firm's equity price as continuously updated market information about how close that firm is to default, because equity IS a call option on the firm's assets, and a call option's price falls as the underlying gets closer to being worthless.</p>`,
