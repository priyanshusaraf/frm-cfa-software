export default ({
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "The most common counterparty stress test, and the one whose limits get tested. You reprice every counterparty's portfolio under a scenario (a 25% equity fall, a rate shock, a credit event), rank counterparties by stressed current exposure, and report the largest to senior management. Its two shortcomings both come from the same omission: credit quality was never in the exercise. So the results cannot be meaningfully summed across counterparties, and they say nothing at all about wrong-way risk.",
    ...
    },
    {
      ...
      def: "Most institutions stress only unilateral credit value adjustment (CVA), which prices the chance the counterparty defaults on them. Bilateral CVA (BCVA) adds the mirror case, the debt value adjustment (DVA), which is the value of the institution's own option to default. Two things change when you go bilateral: the DVA term runs off expected NEGATIVE exposure (the counterparty's view of the same book), and each term is weighted by the OTHER party's survival probability, since you only benefit from your own default if the counterparty is still around to be owed money.",
    ...
    },
    {
      ...
      def: "Four ways counterparty stress testing goes wrong in practice, and all four are about the measure being combined or extrapolated carelessly rather than about the scenario itself. Counterparty results are usually not aggregated with loan and trading-book stress tests at all. When they are combined, institutions reach for current exposure instead of expected exposure or expected positive exposure. Current exposure is least reliable exactly where it is most used, on at-the-money derivatives whose sign flips on a small move. And delta-sensitivity linearization breaks down under large stress moves, because delta is itself nonlinear.",
      pitfall: "This is the most literal 'list' item in the topic: the exam can ask you to identify which of these four is being described in a scenario, so know all four by their distinguishing feature (new/unaggregated discipline; wrong exposure measure used; at-the-money instability; delta linearization error) rather than a vague 'stress testing has flaws.'",
      related: [],
  ...
  recall: [
    ...
    { q: "What two changes does the BCVA formula make relative to the unilateral CVA formula, and why is each one necessary?", a: "It adds a DVA term using expected NEGATIVE exposure (ENE, from the counterparty's perspective) because the institution can also default and owe the counterparty nothing further. And each term must be weighted by the relevant counterparty's SURVIVAL probability, the institution can only realize the DVA benefit if the counterparty is still alive to have been owed money, and the counterparty can only realize the CVA loss if the institution is still alive to have owed it money." },
