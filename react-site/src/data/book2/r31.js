export default ({
  ...
  intuition: `<p>Counterparty risk in derivatives is unusual among credit risks because the exposure is uncertain in DIRECTION. With a loan you know you are the lender and roughly what is owed. A derivative's value can flip between positive and negative for either party as markets move.</p><p>Picture an interest rate swap. Today you might be in the money, meaning the counterparty owes you if the contract closed out now. If rates move against you over the next year, you could just as easily end up owing them.</p><p>That bilateral, market-driven uncertainty is why derivatives credit risk needs its own toolkit: VaR-style exposure models, netting, collateral, central clearing. Loan-style credit risk models only ever have to worry about one direction of loss.</p><p>The whole reading is organized around one question, asked at increasing levels of institutional sophistication: <em>who bears the risk that my counterparty can't pay, and what mechanism reduces it?</em></p><p>Bilateral trading answers with paperwork (the ISDA Master Agreement) and posted assets (collateral).</p><p>Central clearing answers by inserting a well-capitalized middleman, the central counterparty (CCP), which becomes everyone's counterparty at once.</p><p>The older and now largely discredited answers (special purpose vehicles, derivatives product companies, monolines, credit derivative product companies) tried to wrap the risk in a supposedly bankruptcy-remote or AAA-rated box. That looked robust until the 2007 to 2009 crisis tested it and, in most cases, broke it.</p>`,

  eli5: `<p>You and a friend bet on next week's weather. If it rains you owe twenty pounds; if it is sunny, they owe you twenty.</p>
  <p>Right now the bet is worth nothing to either of you. That is the important part. By Friday one of you will owe the other, and neither of you knows which yet.</p>
  <p>That is what a derivative is: a contract whose value flips between "they owe me" and "I owe them" as the world moves. And it creates a worry that a loan does not. Whoever ends up being owed has to wonder whether the other side will actually pay.</p>
  <p>Notice which direction that worry runs. You do not fear the bet going against you; you fear it going in your FAVOUR and your friend not settling. A losing position costs you money but carries no credit risk at all.</p>
  <p>Now scale it up. Not a casual bet but a bank and a corporation with a multi-year interest rate swap worth millions, where a handshake will not do.</p>
  <p>There are two answers. Write down in advance exactly what happens if one side cannot pay, which is the ISDA Master Agreement. Or route the bet through a referee who takes a deposit from both sides and becomes the counterparty to each of them, which is a central counterparty.</p>`,
  thinkLike: `<p>A derivatives risk manager's first instinct on seeing any new trade is to ask two separate questions, not one: "how might the value of this contract move against me?" (market risk) and, layered on top, "if it moves in my favour and my counterparty is the one who now owes me, will they actually pay?" (credit risk). The reading trains you to keep those two questions distinct even though they interact, a derivative can have huge potential market-risk swings but low counterparty risk (if it's centrally cleared and fully collateralized), or modest market-risk swings but high counterparty risk (an uncollateralized OTC trade with a thinly capitalized end user). The practitioner move is to classify every trade along two axes at once: standardized vs. customized (exchange-traded vs. OTC), and collateralized vs. not (the four-way split: exchange-traded, OTC centrally cleared, OTC collateralized, OTC uncollateralized), because it's that second axis, not the first, that actually drives how much counterparty risk survives.</p><p>On the exam, this reading is tested less by calculation and more by "which entity/mechanism does X" recall and by classic trap questions: does central clearing eliminate counterparty risk (no, it reduces it, and it introduces new risks such as CCP failure and legal-jurisdiction conflicts); do end users post as much collateral as dealer banks (usually not, which is exactly why their small trades carry outsized relative risk); can you tell VaR, PFE, and ES apart (VaR = worst loss at a confidence level over a short horizon; PFE = the same idea over a long horizon, applied to credit exposure from gains; ES = the average loss beyond VaR). Expect a scenario question that asks you to correctly interpret a stated VaR number (e.g., "no more than $X, 95% of the time", not "an average of $X").</p>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "Four structures the pre-crisis market built to make a counterparty look safer than the firm behind it, none of which survived contact with 2007-2009. A special purpose vehicle ring-fences assets so investors rank ahead of the sponsor's creditors, which swaps counterparty risk for legal risk. A derivatives product company buys a AAA rating with separate capitalization and mirrored trades. A monoline insurer sells a credit wrap that lends its own rating to someone else's bond. A credit derivative product company does the monoline's job with credit derivatives. The last two posted no collateral in normal conditions, which is precisely why their failure hurt.",
  ...
  misconceptions: [
    ...
    { wrong: "\"A bankruptcy-remote SPV or a AAA-rated DPC/monoline guarantees the counterparty risk is gone.\"", right: "All four legacy structures (SPV, DPC, monoline, CDPC) transform counterparty risk into some other risk (legal risk for SPVs/DPCs, or reliance on an untested AAA rating for monolines/CDPCs) rather than eliminating it, and each broke down in real-world stress during 2007 to 2009 (Lehman's SPV flip provision was unenforceable in the US but enforceable in the UK; Lehman/Bear Stearns DPCs failed to unwind in orderly fashion; monolines and CDPCs lost their ratings and were forced to post collateral exactly as their insured losses mounted)." }
  ],
  ...
  quiz: [
    ...
    {
      q: "A monoline insurance company guarantees bond repayments via credit wraps and, in normal conditions, posts no collateral while holding a AAA rating. What happened to monolines during the 2007 to 2009 crisis?",
      options: [
