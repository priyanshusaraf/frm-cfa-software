export default ({
  ...
  visual: `<div class="widget" data-widget="creditvar"></div><div class="widget" data-widget="annotated-table" data-table='{"title":"B-rated bond: marginal vs cumulative default probability (from Figure 25.1)","corner":"B-rated bond","unit":"%","shade":false,"cols":["Year 1","Year 2","Year 3","Year 4"],"rows":[{"label":"Marginal PD","cells":[3.34,4.46,3.95,3.24]},{"label":"Cumulative PD","cells":[3.34,7.80,11.75,14.99]}],"caption":"Marginal (unconditional) default probabilities come straight from the S&amp;P migration data: 3.34, 4.46, 3.95, 3.24 percent for years 1 to 4. Cumulative default probability is their running sum, so by the end of year 4 there is a 14.99 percent chance the bond has defaulted at some point. The marginals rise then fall: for a weak B-rated name the early years are the most dangerous, and a survivor looks safer each year it lasts."}'></div>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      def: "Credit-sensitive instruments are worth what they are worth because of a spread, so any change in that spread changes their value even with no default. Credit spread risk is that exposure, and a Credit VaR number for a mark-to-market book is really a statement about how far spreads could move against you.",
      intuition: "You could try to handle it the way market risk does: run a historical simulation on observed spread changes, get a one-day 99% VaR, and scale it out to a longer horizon. That approach has two specific problems, and both are about the data rather than the math. Credit spreads are not updated daily for most names, so the history you would simulate from is thin and stale. And the companies whose spread history you DO have are, by construction, companies that did not default, so a simulation drawn from them quietly assumes no default going forward, which is precisely the tail you were trying to measure.",
      example: "CreditMetrics is the way around it. Build a rating transition matrix over the horizon you care about, and the historical rating-change data gives you a probability distribution over spread changes for that same horizon. Then run Monte Carlo: each trial samples the matrix to see whether a name holds its rating, migrates, or defaults, applies the credit spread attached to whichever rating it lands in, revalues the portfolio, and the distribution of those portfolio values gives Credit VaR.",
      pitfall: "Correlation still has to be put in by hand, and there are two ways to do it. Either a Gaussian copula links different companies' rating changes, or you assume spread changes WITHIN a rating category are so highly correlated that every instrument of a given rating moves in unison. The second is cruder and is the one an exam stem is more likely to describe than to name.",
      related: [{ r: 29, label: "R29: spread risk and default intensity in depth" }],
      memory: "Spread risk needs no default to hurt you. Simulating spreads directly fails twice: no daily quotes, and a survivor-only sample that assumes nobody defaults."
    },
    {
