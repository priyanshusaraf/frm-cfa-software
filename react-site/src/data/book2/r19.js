export default ({
  ...
  intuition: `<p>Think of the asset classification spectrum as a patient's health chart.</p>
  <p><strong>Standard</strong> is healthy, with no delinquency in question.</p>
  <p><strong>Specially mentioned</strong> is early symptoms: some new weakness that <em>could</em> hurt repayment.</p>
  <p><strong>Substandard</strong> is a diagnosed, well-defined problem. The primary way the bank expected to get repaid has already failed, so it is falling back on secondary sources like collateral.</p>
  <p><strong>Doubtful</strong> is that same diagnosis, but with a poor prognosis for recovery.</p>
  <p><strong>Loss</strong> is a patient who has, for accounting purposes, flatlined. That is not a certainty of zero recovery, but delaying the write-off any further would be dishonest.</p>
  <p>Provisioning intensity should rise as the condition worsens, and that is the whole logic connecting classification to loss reserves. A bank holding mostly standard loans needs a thin cushion. A bank with a pile of doubtful loans needs a thick one.</p>
  <p>Expected loss (EL) is built from three separate questions, and each one is answered by a different piece of evidence.</p>
  <p><strong>Who</strong> did you lend to? That is the probability of default (PD), estimated from the borrower's history, credit scores, rating agency views, or CDS spreads.</p>
  <p><strong>What</strong> product did you lend on? That is loss given default (LGD). A secured mortgage loses far less in default than an unsecured personal loan, because the collateral can be seized.</p>
  <p><strong>How much</strong> is outstanding when default hits? That is exposure at default (EAD). A revolving credit line drawn further down before default has more dollars at risk than one paid down early.</p>
  <p>Multiply the three together and you get the average loss a bank should expect across many similar loans.</p>
  <p>Unexpected loss (UL) is everything EL does not capture: the tail-risk variability around that average, the 99th-percentile bad year rather than the typical one. This reading only defines EL and UL in words, and R20 gives them formulas and standard deviations.</p>`,
  ...
  concepts: [
    ...
    },
    {
      ...
      memory: "Collateral first, consult second, bring in a third party third, liquidate last, but remember the guidance explicitly allows any order."
    },
    {
    ...
    },
    {
      ...
      example: "Board-level questions to expect include: are loans and deposits priced competitively? Are PD, LGD, and EAD estimates historically accurate? How does the bank treat delinquent loans? How frequently is the portfolio stress tested?"
    }
  ],
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: 2,
      why: "Collateral should NOT be considered when sizing risk exposures for concentration purposes, the full $80 million counts. This is separate from asset classification, where full collateralization with cash/cash-equivalents CAN keep a loan in the 'standard' bucket. Confusing the two contexts is the trap here."
    },
    {
    ...
    {
      q: "Which of the following is explicitly excluded from portfolio-level credit risk analysis?",
      options: [
