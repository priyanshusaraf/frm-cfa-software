export default ({
  ...
  thinkLike: `<p>A credit portfolio manager asks "what's my expected loss?" far less often than "what's my TAIL risk, and where is it actually coming from?"</p><p>Two portfolios can carry identical expected loss and wildly different Credit VaR, purely because of how correlated the underlying defaults are.</p><p>So the first move on any new portfolio is to split the loss variance in two. The systematic share is driven by shared exposure to \\(m\\), controlled by each name's \\(\\beta\\). The idiosyncratic share is the part you can diversify away by adding more, smaller, less-correlated names.</p><p>Diversification, or granularity, is a real risk-reduction lever. But it only works against the idiosyncratic share. No amount of adding loans to a portfolio of regional subprime mortgage originators fixes the fact that all of them are exposed to the same housing-market factor.</p>
  <p>On the exam this reading arrives two ways. One is a numeric problem at the correlation extremes: \\(\\rho=1\\) or \\(\\rho=0\\), with a given n, PD and notional, computing expected loss, the loss quantile, and Credit VaR as the difference between them.</p><p>The other is a conceptual trap built on the granularity-versus-correlation distinction, or on the "conditioning reduces variance" property of the single-factor model. When you see "more loans" in an answer choice, ask which kind of risk more loans actually reduces before you pick it.</p>`,
