...
### 8.7 Hard-concept SEQUENCING pages (owner directive, 2026-07-26)

Spec: `docs/superpowers/specs/2026-07-26-hard-concept-sequencing-and-cva-core-concept-design.md`.

A third failure mode, distinct from 8.1-8.4 and from the coverage back-audit. A concept can
be fully covered, correctly explained, dash-clean and readable in every reading it appears
in, and still be ungraspable, because the readings present it in CURRICULUM order and that
is not LEARNING order. The owner's reported case is CVA: it spans R25, R29, R32, R35, R36,
R37, R38 and R62, its inputs are taught twelve readings before the model that consumes them,
a compressed trailer appears before the formula exists, and a term R37 omits on purpose is
supplied downstream in R38. Every one of those readings is locally correct. Nobody assembles
them.

The fix is an authored Core Concept page per qualifying concept (section 6's
`/concept/:slug`, `authoredConcepts.js`, `layer: "core"`, NO new page type), sequenced
problem-first per section 1a rather than in reading order. A concept qualifies when it spans
3+ readings, curriculum order is not learning order for it, and **a student who has read
every constituent reading still cannot say what the model does.** That third test is the one
that matters and it is not automatable, which is why these pages are authored rather than
generated by `build-core-concepts.mjs`.

CVA is the pilot. Four more candidates are recorded in the spec (exposure metrics, Vasicek
/WCDR, the Basel capital stack, the liquidity spiral). Do not fold the Basel III CVA capital
framework (R62) into the CVA page: it is regulatory capital, a separate question, and
including it is part of what makes the concept feel unbounded.
