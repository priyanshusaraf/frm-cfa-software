...
- **R36 (Future Value and Exposure): RESOLVED 2026-07-25** as the content-run pilot. Full
  rewrite landed: validator clean, 0 em/en-dashes, enumerations moved out of the three
  offending `def`s (the lists were already in `breakdown`), problem-first `intuition`, the
  misplaced funding-vs-credit `example` promoted to its own concept, the indeterminate quiz
  Q1 replaced, and the `2.33 × 0.07 × 0.2 = 3.27%` arithmetic error corrected. The durable
  lessons are in `content-guidelines.md` under "R36 (PILOT)". Do not redo.
  - GLOBAL follow-up still live for the content run: every reading that introduces an
    abbreviation must expand it in that file, use exactly one expansion for it, and skip the
    abbreviation entirely when the term appears only two or three times.
